...
describe("useUrlState — initial is render-stable", () => {
    ...
    it("snapshots the initial shape once and ignores later mutations", () => {
        ...
        const initial = { q: "x" };
        const { result, rerender } = renderHook(({ init }) => useUrlState(init), { initialProps: { init: initial }, wrapper: makeWrapper(["/"]) });
        ...
        expect(result.current[0].n).toBeUndefined();
    });
    it("array values containing commas survive round-trip", () => {
        const { result } = renderHook(() => useUrlState({ tags: [] }), { wrapper: makeWrapper(["/"]) });
...
//# sourceMappingURL=use-url-state-init-stable.test.js.map
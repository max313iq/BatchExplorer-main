...
import { getArmTokenForAccount, loginAccount, listSubscriptionsForAccount, } from "../msal-auth";
...
function makeStore() {
    ...
    return {
        ...
        notifications,
        tenantSets,
        setActiveTenant(home, tenant) {
            tenantSets.push({ home, tenant });
        },
        updateAzureAccount(_home, patch) {
            updates.push(patch);
        },
        addNotification(n) {
...
describe("performTenantSwitch rollback", () => {
    ...
    it("restores the subscriptions snapshot when the re-auth popup is cancelled", () => __awaiter(void 0, void 0, void 0, function* () {
        getArmTokenForAccount
            // First silent acquire fails with reauth-required.
            .mockRejectedValueOnce(new Error("interaction_required"));
        // Popup is cancelled.
        loginAccount.mockRejectedValueOnce(new Error("Sign-in cancelled — popup was closed"));
        const account = makeAccount();
        const originalSubs = account.subscriptions.slice();
        const store = makeStore();
        const result = yield performTenantSwitch(account, "guest-tenant", store, {
    ...
    it("uses structuredClone (or JSON deep-copy) so a mutation to original.subscriptions does not leak into the snapshot", () => __awaiter(void 0, void 0, void 0, function* () {
        getArmTokenForAccount.mockRejectedValueOnce(new Error("interaction_required"));
        loginAccount.mockRejectedValueOnce(new Error("cancelled"));
        const account = makeAccount();
        const store = makeStore();
        ...
        const lastUpdate = store
            .updates.at(-1);
        const restored = lastUpdate.subscriptions;
        expect(restored.map((s) => s.subscriptionId)).toEqual(["sub-A", "sub-B"]);
    }));
});
//# sourceMappingURL=perform-tenant-switch.test.js.map
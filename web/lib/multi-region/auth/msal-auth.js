...
import { classifyHttpError, } from "../services/types";
...
export function getStoredCustomClientId() {
    var _a;
    if (typeof window === "undefined")
        return "";
    try {
        return (_a = window.localStorage.getItem(LOGIN_CUSTOM_CLIENT_KEY)) !== null && _a !== void 0 ? _a : "";
    }
    catch (_b) {
        return "";
    }
}
export function setStoredCustomClientId(clientId) {
    if (typeof window === "undefined")
        return;
    try {
...
export const LOGIN_MODE_PRESETS = [
    {
        mode: "cli",
        label: "App client (Azure CLI)",
        clientId: AZURE_CLI_CLIENT_ID,
        description: "Uses Azure CLI's well-known public client id; tokens look like what `az login` mints. Default.",
    },
    {
        mode: "powershell",
        label: "Azure PowerShell",
        clientId: AZURE_POWERSHELL_CLIENT_ID,
        description: "First-party public client used by Connect-AzAccount. PKCE-friendly from any origin.",
    },
    {
        mode: "vs",
        label: "Visual Studio",
        clientId: AZURE_VS_CLIENT_ID,
        description: "First-party client used by VS / VS Code Azure integrations.",
    },
    {
        mode: "portal",
        label: "Azure Portal (may reject)",
        clientId: AZURE_PORTAL_CLIENT_ID,
        description: "Real Azure Portal SPA client id. AAD restricts its redirect URIs to portal.azure.com, so login will usually fail with unauthorized_client from this origin. Wired anyway in case your tenant relaxed it.",
    },
];
...
function getActiveClientId() {
    ...
    return (_a = preset === null || preset === void 0 ? void 0 : preset.clientId) !== null && _a !== void 0 ? _a : AZURE_CLI_CLIENT_ID;
}
...
const PARTNER_CENTER_SCOPE = "https://api.partnercenter.microsoft.com/user_impersonation";
 ...
 */
function _toArmErrorMsal(response, fallbackMessage) {
    var _a, _b, _c;
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const code = (_b = inner.code) !== null && _b !== void 0 ? _b : "AzureRequestError";
        const innerMessage = (_c = inner.message) !== null && _c !== void 0 ? _c : "";
        ...
        const err = classifyHttpError(message, response.status, code, body, retryAfter);
        if (urlPath)
            err.urlPath = urlPath;
        return err;
    });
...
function buildAuthority(tenantId) {
    if (!tenantId)
        return undefined;
    return `https://login.microsoftonline.com/${encodeURIComponent(tenantId)}`;
}
function _resolveTenantForAccount(homeAccountId, explicit) {
    var _a;
    if (explicit)
        return explicit;
    return (_a = getActiveTenant(homeAccountId)) !== null && _a !== void 0 ? _a : undefined;
}
// ---------------------------------------------------------------------------
...
const msalNetworkClient = {
    ...
    sendPostRequestAsync(url, options) {
        return __awaiter(this, void 0, void 0, function* () {
            const isTokenEndpoint = url.includes("login.microsoftonline.com") && url.includes("/token");
            const fetchUrl = isTokenEndpoint ? "/api/auth/proxy-token" : url;
            const extraHeaders = isTokenEndpoint
                ? { "x-proxy-target": url }
                : {};
            const response = yield fetch(fetchUrl, {
                method: "POST",
                headers: Object.assign(Object.assign({}, options === null || options === void 0 ? void 0 : options.headers), extraHeaders),
...
function _maybeWarnProductionConfig() {
    var _a;
    if (_prodWarningEmitted)
        return;
    ...
    try {
        ...
        if (env === "production") {
            ...
            console.warn("[azbm] shared-cache (cacheLocation:localStorage + /api/auth/proxy-token) is dev-only; " +
...
function _clearMsalCache() {
    ...
    const keysToRemove = Object.keys(localStorage).filter((k) => knownIds.some((id) => k.startsWith(id)) ||
        k.startsWith("msal.") ||
    ...
    _activeAccount = null;
    _initComplete = null;
    console.warn("[MSAL] localStorage quota exceeded — cache cleared. Sign in again.");
}
function _isQuotaError(error) {
    var _a, _b;
    const code = (_a = error === null || error === void 0 ? void 0 : error.errorCode) !== null && _a !== void 0 ? _a : "";
    const msg = (_b = error === null || error === void 0 ? void 0 : error.message) !== null && _b !== void 0 ? _b : "";
    return (code === "cache_quota_exceeded" ||
 ...
 */
export function purgeMsalCache() {
    _clearMsalCache();
    recordAuditEvent({
        actor: "system",
 ...
 */
export function handlePopupIfNeeded() {
    var _a;
            ...
            broadcastResponseToMainFrame().catch((e) => {
                ...
                try {
                    ...
                    if (hasOpener && openerStillAlive) {
                        ...
                        console.warn("[MSAL Popup] suppressing window.close() — no live opener (likely a stale callback URL pasted into a top-level tab).");
                    }
                }
                catch (_b) {
                    /* ignore */
 ...
 */
function _addAccountToMap(account) {
...
function getMsalInstance() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        _initComplete = (() => __awaiter(this, void 0, void 0, function* () {
            const initMsal = () => __awaiter(this, void 0, void 0, function* () {
            ...
            let msalApp;
            try {
                msalApp = yield initMsal();
            }
            catch (initErr) {
                if (_isQuotaError(initErr)) {
                    _clearMsalCache();
                    msalApp = yield initMsal(); // one retry after clearing
                }
                else {
                    throw initErr;
                }
            ...
            try {
                const response = yield msalApp.handleRedirectPromise();
                if (response === null || response === void 0 ? void 0 : response.account) {
            ...
            console.log("[MSAL] Cached accounts:", cachedAccounts.length, cachedAccounts.map((a) => a.username));
 ...
 */
const LOGIN_WATCHDOG_MS = 90000;
let _loginStartedAt = null;
export function loginAccount(tenantIdOrOpts) {
    return __awaiter(this, void 0, void 0, function* () {
        // Normalize legacy `loginAccount(tenantId)` callers.
        const opts = typeof tenantIdOrOpts === "string"
            ? { tenantId: tenantIdOrOpts }
            : (tenantIdOrOpts !== null && tenantIdOrOpts !== void 0 ? tenantIdOrOpts : {});
        ...
        if (_loginInProgress &&
            ...
            console.warn("[MSAL] loginAccount watchdog: previous popup never settled after", LOGIN_WATCHDOG_MS, "ms — clearing lock so a new popup can open.");
        ...
        _loginInProgress = (() => __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c;
            const msalApp = yield getMsalInstance();
            ...
            let cancelClosePromise = null;
            const eventCallbackId = msalApp.addEventCallback(((msg) => {
                var _a;
                if (msg.eventType === "msal:popupOpened" && ((_a = msg.payload) === null || _a === void 0 ? void 0 : _a.popupWindow)) {
            ...
            const closeWatchPromise = new Promise((_, reject) => {
                cancelClosePromise = reject;
                let closeDetectedAt = null;
                popupCloseInterval = setInterval(() => {
                    ...
                    if (popupRef.closed) {
                        ...
                        const err = new Error("Sign-in cancelled — popup was closed before authentication completed.");
            ...
            const loginPopupPromise = msalApp.loginPopup(Object.assign(Object.assign({ scopes: [ARM_SCOPE], prompt: prompt !== null && prompt !== void 0 ? prompt : "select_account" }, (loginHint ? { loginHint } : {})), (authority ? { authority } : {})));
            ...
            loginPopupPromise.then((r) => {
                var _a, _b;
                return console.log("[MSAL] loginPopup settled (resolved):", (_b = (_a = r === null || r === void 0 ? void 0 : r.account) === null || _a === void 0 ? void 0 : _a.username) !== null && _b !== void 0 ? _b : "(no account)");
            }, (err) => {
                var _a, _b;
                return console.log("[MSAL] loginPopup settled (rejected):", (_a = err === null || err === void 0 ? void 0 : err.errorCode) !== null && _a !== void 0 ? _a : "?", (_b = err === null || err === void 0 ? void 0 : err.message) !== null && _b !== void 0 ? _b : String(err));
            });
            try {
                ...
                if (result === null || result === void 0 ? void 0 : result.account) {
                    ...
                    console.log("[MSAL] Popup login successful (multi-account):", result.account.username);
                    console.log("[MSAL] Total accounts after login:", _accounts.size);
                    recordAuditEvent({
                        actor: result.account.username,
                        action: "loginAccount",
                        target: (_a = tenantId !== null && tenantId !== void 0 ? tenantId : result.account.tenantId) !== null && _a !== void 0 ? _a : "(default)",
                        status: "success",
                        details: {
                            homeAccountId: result.account.homeAccountId,
                            tenantId: result.account.tenantId,
                            requestedTenant: tenantId !== null && tenantId !== void 0 ? tenantId : null,
                        },
                    });
                ...
                const msalError = e;
                if ((msalError === null || msalError === void 0 ? void 0 : msalError.errorCode) === "popup_window_error" ||
                    (msalError === null || msalError === void 0 ? void 0 : msalError.errorCode) === "empty_window_error") {
                    throw new Error("Popup blocked! Allow popups for this site in your browser, then try again.");
                }
                ...
                const msg = String((_b = msalError === null || msalError === void 0 ? void 0 : msalError.errorMessage) !== null && _b !== void 0 ? _b : "") +
                    " " + String((_c = e === null || e === void 0 ? void 0 : e.message) !== null && _c !== void 0 ? _c : "");
                const isRedirectMismatch = msg.includes("AADSTS50011") ||
                    msg.toLowerCase().includes("redirect_uri") ||
                    (msalError === null || msalError === void 0 ? void 0 : msalError.errorCode) === "unauthorized_client";
                if (isRedirectMismatch) {
                    const currentMode = getStoredLoginMode();
                    if (currentMode !== "cli") {
                        console.warn("[MSAL] redirect_uri_mismatch on mode", currentMode, "→ auto-reverting to 'cli' so the next sign-in works.");
                        setStoredLoginMode("cli");
                        throw new Error("AAD rejected this app's redirect URI for the chosen Login mode " +
                ...
                if (cancelClosePromise) {
                    ...
                    try {
                        cancelClosePromise(new Error("loginAccount finished — close watcher cancelled"));
                    }
                    catch (_d) {
                        /* */
                    }
 ...
 */
export function logoutAccount(homeAccountId) {
    return __awaiter(this, void 0, void 0, function* () {
        const msalApp = yield getMsalInstance();
        const account = _accounts.get(homeAccountId);
        _accounts.delete(homeAccountId);
        if ((_activeAccount === null || _activeAccount === void 0 ? void 0 : _activeAccount.homeAccountId) === homeAccountId) {
        ...
        if (account) {
            try {
                yield msalApp.clearCache({ account });
            }
            catch (err) {
                console.warn("[MSAL] clearCache failed; falling back to direct sweep", err);
            }
        }
        ...
        try {
            const flush = window.__azbmFlushSharedCache;
            if (flush)
                yield flush();
        }
        catch (_b) {
        ...
        recordAuditEvent({
            actor: (account === null || account === void 0 ? void 0 : account.username) || homeAccountId,
 ...
 */
export function getAllLoggedInAccounts() {
    return __awaiter(this, void 0, void 0, function* () {
        yield getMsalInstance();
 ...
 */
export function parseCaeClaimsChallenge(headerValue) {
    if (!headerValue)
        return null;
    const v = headerValue.trim();
    // Quick early-out — must be a Bearer challenge.
    if (!/^Bearer\b/i.test(v))
        return null;
...
function tokenCoalesceKey(homeAccountId, tenantId, scopes, forceRefresh, claims) {
    ...
    return `${homeAccountId}::${tenantId !== null && tenantId !== void 0 ? tenantId : ""}::${scopeKey}::${forceRefresh ? "1" : "0"}::${claimsBucket}`;
}
function acquireTokenForAccount(scopes, homeAccountId, tenantId, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        const msalApp = yield getMsalInstance();
        const account = _accounts.get(homeAccountId);
        if (!account) {
            throw new Error(`Account not found: ${homeAccountId}. Please sign in first.`);
        }
        const forceRefresh = (opts === null || opts === void 0 ? void 0 : opts.forceRefresh) === true;
        const silentRequest = Object.assign(Object.assign({ scopes,
            account,
            forceRefresh }, (tenantId
            ? { authority: `https://login.microsoftonline.com/${tenantId}` }
            : {})), ((opts === null || opts === void 0 ? void 0 : opts.claims) ? { claims: opts.claims } : {}));
        ...
        const coalesceKey = tokenCoalesceKey(homeAccountId, tenantId, scopes, forceRefresh, opts === null || opts === void 0 ? void 0 : opts.claims);
        const existing = _tokenInflight.get(coalesceKey);
        if (existing)
            return existing;
        const promise = (() => __awaiter(this, void 0, void 0, function* () {
            try {
                return yield msalApp.acquireTokenSilent(silentRequest);
            }
            catch (error) {
                // Cache quota exceeded — clear and surface a clear message
                if (_isQuotaError(error)) {
                    _clearMsalCache();
                    throw new Error("Browser storage quota exceeded and MSAL cache was cleared. " +
                ...
                if (error instanceof InteractionRequiredAuthError) {
                    ...
                    if (_loginInProgress) {
                        yield _loginInProgress;
                        return yield msalApp.acquireTokenSilent(silentRequest);
                    }
                    throw new InteractionRequiredAuthError("interaction_required", "Cached session is no longer valid for this account. Please sign in again.");
                }
                throw error;
            }
        }))();
 ...
 */
export function getArmTokenForAccount(homeAccountId, tenantId, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!(opts === null || opts === void 0 ? void 0 : opts.forceRefresh) && !(opts === null || opts === void 0 ? void 0 : opts.claims)) {
            try {
                const imported = yield importedTokens.ensureImportedToken(homeAccountId, "arm");
                if (imported)
                    return imported;
            }
            catch (err) {
                console.warn("[imported-tokens] ARM redemption failed; falling back to MSAL:", err);
            }
        }
        const effectiveTenant = _resolveTenantForAccount(homeAccountId, tenantId);
        const result = yield acquireTokenForAccount([ARM_SCOPE], homeAccountId, effectiveTenant, opts);
 ...
 */
export function decodeJwtClaimsUnsafe(jwt) {
    try {
 ...
 */
export function getBatchTokenForAccount(homeAccountId, tenantId, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!(opts === null || opts === void 0 ? void 0 : opts.forceRefresh) && !(opts === null || opts === void 0 ? void 0 : opts.claims)) {
            try {
                const imported = yield importedTokens.ensureImportedToken(homeAccountId, "batch");
                if (imported)
                    return imported;
            }
            catch (err) {
                console.warn("[imported-tokens] Batch redemption failed; falling back to MSAL:", err);
            }
        }
        const effectiveTenant = _resolveTenantForAccount(homeAccountId, tenantId);
        const result = yield acquireTokenForAccount([BATCH_SCOPE], homeAccountId, effectiveTenant, opts);
        return result.accessToken;
    });
}
/**
 ...
 */
export function getGraphTokenForAccount(homeAccountId, tenantId, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(opts === null || opts === void 0 ? void 0 : opts.forceRefresh) && !(opts === null || opts === void 0 ? void 0 : opts.claims)) {
            try {
                const imported = yield importedTokens.ensureImportedToken(homeAccountId, "graph");
                if (imported)
                    return imported;
            }
            catch (err) {
                console.warn("[imported-tokens] Graph redemption failed; falling back to MSAL:", err);
            }
        }
        const effectiveTenant = _resolveTenantForAccount(homeAccountId, tenantId);
        const result = yield acquireTokenForAccount([GRAPH_SCOPE], homeAccountId, effectiveTenant, opts);
        return result.accessToken;
    });
}
/**
 ...
 */
export function getPartnerCenterTokenForAccount(homeAccountId, tenantId) {
    return __awaiter(this, void 0, void 0, function* () {
        const effectiveTenant = _resolveTenantForAccount(homeAccountId, tenantId);
        const result = yield acquireTokenForAccount([PARTNER_CENTER_SCOPE], homeAccountId, effectiveTenant);
        return result.accessToken;
    });
}
/**
 ...
 */
export function listSubscriptionsForAccount(homeAccountId) {
    var _a, _b;
    return __awaiter(this, void 0, void 0, function* () {
        const token = yield getArmTokenForAccount(homeAccountId);
        const url = `https://management.azure.com/subscriptions?api-version=${ARM_SUBSCRIPTIONS_API_VERSION}`;
        ...
        const response = yield guardedFetch(url, { headers: { Authorization: `Bearer ${token}` } }, { subscriptionId: "_arm-subscriptions", family: "arm" });
        if (!response.ok) {
            throw yield _toArmErrorMsal(response, `Failed to list subscriptions for account ${homeAccountId}`);
        }
        const out = [];
        let pageData = (yield response.json());
        for (const s of (_a = pageData.value) !== null && _a !== void 0 ? _a : []) {
            out.push({
        ...
        while (pageData.nextLink) {
            const nextResp = yield guardedFetch(pageData.nextLink, { headers: { Authorization: `Bearer ${token}` } }, { subscriptionId: "_arm-subscriptions", family: "arm" });
            if (!nextResp.ok) {
                throw yield _toArmErrorMsal(nextResp, `Failed to page subscriptions for account ${homeAccountId}`);
            }
            pageData = (yield nextResp.json());
            for (const s of (_b = pageData.value) !== null && _b !== void 0 ? _b : []) {
                out.push({
                    subscriptionId: s.subscriptionId,
                    displayName: s.displayName,
                    state: s.state,
 ...
 */
function _isTokenAlmostExpired(jwt) {
 ...
 */
function _acquireFreshFor(audience, homeAccountId, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        const scope = audience === "arm"
            ? ARM_SCOPE
            : audience === "graph"
                ? GRAPH_SCOPE
                : audience === "batch"
        ...
        const effectiveTenant = _resolveTenantForAccount(homeAccountId, opts === null || opts === void 0 ? void 0 : opts.tenantId);
        // First attempt — silent, may return cached.
        const first = yield acquireTokenForAccount([scope], homeAccountId, effectiveTenant, opts);
        if (!_isTokenAlmostExpired(first.accessToken))
            return first.accessToken;
        // Cached value is too close to exp — force-refresh through STS.
        const fresh = yield acquireTokenForAccount([scope], homeAccountId, effectiveTenant, Object.assign(Object.assign({}, opts), { forceRefresh: true }));
        return fresh.accessToken;
    });
}
/** Like `getArmTokenForAccount` but proactively renews when exp < now + 90 s. */
export function acquireArmTokenFresh(account, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        return _acquireFreshFor("arm", account.homeAccountId, opts);
    });
}
/** Proactive-renewal variant of `getBatchTokenForAccount`. */
export function acquireBatchTokenFresh(account, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        return _acquireFreshFor("batch", account.homeAccountId, opts);
    });
}
/** Proactive-renewal variant of `getGraphTokenForAccount`. */
export function acquireGraphTokenFresh(account, opts) {
    return __awaiter(this, void 0, void 0, function* () {
        return _acquireFreshFor("graph", account.homeAccountId, opts);
    });
}
/** Proactive-renewal variant of `getPartnerCenterTokenForAccount`. */
export function acquirePartnerCenterTokenFresh(account, opts) {
    return __awaiter(this, void 0, void 0, function* () {
 ...
 * If tenantId is provided, login is scoped to that tenant authority.
 */
export function login(tenantId) {
    return __awaiter(this, void 0, void 0, function* () {
 ...
 */
const AUTH_STORAGE_PREFIXES = Object.freeze([
    "azbm.",
    "azbm:",
    "msal.",
    "msal|",
 ...
 */
function _targetedClearAuthLocalStorage() {
    try {
        const toRemove = [];
        for (let i = 0; i < localStorage.length; i++) {
            const k = localStorage.key(i);
            if (!k)
                continue;
            const matches = AUTH_STORAGE_PREFIXES.some((p) => k.startsWith(p)) ||
...
export function logout(opts) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            if ((opts === null || opts === void 0 ? void 0 : opts.everywhere) && accountsBefore.length > 0) {
        ...
        recordAuditEvent({
            actor: ((_a = accountsBefore[0]) === null || _a === void 0 ? void 0 : _a.username) || "system",
            action: (opts === null || opts === void 0 ? void 0 : opts.everywhere) ? "signOutEverywhere" : "logoutAccount",
            target: accountsBefore.map((a) => a.username).join(",") || "(none)",
            status: "success",
            details: {
                accountsCleared: accountsBefore.length,
                everywhere: !!(opts === null || opts === void 0 ? void 0 : opts.everywhere),
            },
        });
    });
}
 ...
 */
export function signOutEverywhere(account) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        const msalApp = yield getMsalInstance();
        try {
            ...
            if ((_activeAccount === null || _activeAccount === void 0 ? void 0 : _activeAccount.homeAccountId) === account.homeAccountId) {
                const remaining = Array.from(_accounts.values());
                _activeAccount = (_a = remaining[0]) !== null && _a !== void 0 ? _a : null;
                if (_activeAccount)
                    msalApp.setActiveAccount(_activeAccount);
            }
            _targetedClearAuthLocalStorage();
 ...
 */
function acquireToken(scopes, tenantId) {
    return __awaiter(this, void 0, void 0, function* () {
        const msalApp = yield getMsalInstance();
        if (!_activeAccount) {
            const accounts = msalApp.getAllAccounts();
            if (accounts.length === 0) {
                throw new Error("Not signed in. Click 'Sign in with Azure' to authenticate.");
            }
            _activeAccount = accounts[0];
            _addAccountToMap(accounts[0]);
            msalApp.setActiveAccount(accounts[0]);
        }
        const silentRequest = Object.assign({ scopes, account: _activeAccount, forceRefresh: false }, (tenantId
            ? { authority: `https://login.microsoftonline.com/${tenantId}` }
            : {}));
        try {
            return yield msalApp.acquireTokenSilent(silentRequest);
            ...
            if (error instanceof InteractionRequiredAuthError) {
                ...
                throw new InteractionRequiredAuthError("interaction_required", "Cached session is no longer valid. Please sign in again.");
            }
            throw error;
        }
    });
 ...
 * check.
 */
export function getArmToken(tenantId) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        let effectiveTenant = tenantId;
        if (!effectiveTenant && _activeAccount) {
            effectiveTenant =
                (_a = getActiveTenant(_activeAccount.homeAccountId)) !== null && _a !== void 0 ? _a : undefined;
        }
        const result = yield acquireToken([ARM_SCOPE], effectiveTenant);
        return result.accessToken;
    });
}
/**
 ...
 */
export function getBatchToken(tenantId) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        let effectiveTenant = tenantId;
        if (!effectiveTenant && _activeAccount) {
        ...
        const result = yield acquireToken([BATCH_SCOPE], effectiveTenant);
        return result.accessToken;
    });
}
/**
 ...
 */
export function getGraphToken(tenantId) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        let effectiveTenant = tenantId;
        if (!effectiveTenant && _activeAccount) {
        ...
        const result = yield acquireToken([GRAPH_SCOPE], effectiveTenant);
        return result.accessToken;
    });
}
/**
 ...
 */
export function setActiveTenant(homeAccountId, tenantId) {
    if (!homeAccountId || !tenantId)
        return;
    try {
        if (typeof sessionStorage !== "undefined") {
            sessionStorage.setItem(`${ACTIVE_TENANT_KEY_PREFIX}${homeAccountId}`, tenantId);
        }
    }
    catch (_a) {
 ...
 */
export function getActiveTenant(homeAccountId) {
    if (!homeAccountId)
        return null;
    try {
        if (typeof sessionStorage === "undefined")
            return null;
        return sessionStorage.getItem(`${ACTIVE_TENANT_KEY_PREFIX}${homeAccountId}`);
    }
    catch (_a) {
        return null;
    }
 ...
 */
export function clearActiveTenant(homeAccountId) {
    if (!homeAccountId)
        return;
    try {
        if (typeof sessionStorage !== "undefined") {
 ...
 */
export function listAccessibleTenants(homeAccountId) {
    var _a, _b;
    return __awaiter(this, void 0, void 0, function* () {
        const token = homeAccountId
            ? yield getArmTokenForAccount(homeAccountId)
            : yield getArmToken();
        ...
        const response = yield guardedFetch(url, { headers: { Authorization: `Bearer ${token}` } }, { subscriptionId: "_arm-tenant", family: "arm" });
        if (!response.ok) {
            throw yield _toArmErrorMsal(response, "Failed to list tenants");
        }
        const out = [];
        const mapTenant = (t) => {
            var _a, _b, _c, _d, _e;
            return ({
                tenantId: (_b = (_a = t.tenantId) !== null && _a !== void 0 ? _a : t.id) !== null && _b !== void 0 ? _b : "",
                displayName: (_e = (_d = (_c = t.displayName) !== null && _c !== void 0 ? _c : t.defaultDomain) !== null && _d !== void 0 ? _d : t.tenantId) !== null && _e !== void 0 ? _e : "(unknown tenant)",
                defaultDomain: t.defaultDomain,
                tenantCategory: t.tenantCategory,
            });
        };
        let pageData = (yield response.json());
        for (const t of (_a = pageData.value) !== null && _a !== void 0 ? _a : [])
            out.push(mapTenant(t));
        ...
        while (pageData.nextLink) {
            const nextResp = yield guardedFetch(pageData.nextLink, { headers: { Authorization: `Bearer ${token}` } }, { subscriptionId: "_arm-tenant", family: "arm" });
            if (!nextResp.ok) {
                throw yield _toArmErrorMsal(nextResp, "Failed to list tenants");
            }
            pageData = (yield nextResp.json());
            for (const t of (_b = pageData.value) !== null && _b !== void 0 ? _b : [])
                out.push(mapTenant(t));
        }
        return out;
    });
...
export function listSubscriptions() {
    var _a, _b;
    return __awaiter(this, void 0, void 0, function* () {
        const token = yield getArmToken();
        const url = `https://management.azure.com/subscriptions?api-version=${ARM_SUBSCRIPTIONS_API_VERSION}`;
        // Same governance routing as listSubscriptionsForAccount above.
        const response = yield guardedFetch(url, { headers: { Authorization: `Bearer ${token}` } }, { subscriptionId: "_arm-subscriptions", family: "arm" });
        if (!response.ok) {
            throw yield _toArmErrorMsal(response, "Failed to list subscriptions");
        }
        const out = [];
        const mapSub = (s) => ({
            subscriptionId: s.subscriptionId,
        ...
        while (pageData.nextLink) {
            const nextResp = yield guardedFetch(pageData.nextLink, { headers: { Authorization: `Bearer ${token}` } }, { subscriptionId: "_arm-subscriptions", family: "arm" });
            if (!nextResp.ok) {
                throw yield _toArmErrorMsal(nextResp, "Failed to list subscriptions");
            }
            pageData = (yield nextResp.json());
            for (const s of (_b = pageData.value) !== null && _b !== void 0 ? _b : [])
                out.push(mapSub(s));
...
export const msalAuth = {
    getAccessToken() {
        return __awaiter(this, void 0, void 0, function* () {
            if (_externalProvider)
                return _externalProvider.getAccessToken();
            return getArmToken();
        });
    },
    getBatchAccessToken() {
        return __awaiter(this, void 0, void 0, function* () {
            if (_externalProvider)
                return _externalProvider.getBatchAccessToken();
    ...
    checkHealth() {
        return __awaiter(this, void 0, void 0, function* () {
            if (_externalProvider)
                return _externalProvider.checkHealth();
            try {
                const user = yield getCurrentUser();
                if (!user) {
    ...
    loadSubscriptions(store) {
        return __awaiter(this, void 0, void 0, function* () {
            if (_externalProvider === null || _externalProvider === void 0 ? void 0 : _externalProvider.loadSubscriptions) {
...
if (typeof window !== "undefined") {
     ...
     */
    function _installPopupBridgeListener() {
        window.addEventListener("message", (event) => {
            ...
            try {
                ...
                const json = atob((encoded + padding).replace(/-/g, "+").replace(/_/g, "/"));
                const libraryState = JSON.parse(json);
                const id = libraryState === null || libraryState === void 0 ? void 0 : libraryState.id;
                if (typeof id !== "string" || !id)
                    return;
                console.log("[MSAL bridge] postMessage fallback received; re-broadcasting on channel", id);
    ...
    window.__azbm = window.__azbm || {
        autoSignIn: (upn, tenantId) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                ...
                if (!account)
                    return { ok: false, error: "popup cancelled" };
...
//# sourceMappingURL=msal-auth.js.map
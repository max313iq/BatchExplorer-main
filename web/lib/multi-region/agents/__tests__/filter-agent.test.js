...
const makeAccount = (id, overrides = {}) => (Object.assign({ id, accountName: `acct-${id}`, resourceGroup: "rg", subscriptionId: "sub1", region: "eastus", provisioningState: "created" }, overrides));
const makePool = (id, accountId, overrides = {}) => (Object.assign({ id,
    accountId, poolId: `pool-${id}`, provisioningState: "created", config: {} }, overrides));
const buildStore = (accounts, pools = []) => new MultiRegionStore({ accounts, pools });
beforeEach(() => {
    localStorage.clear();
...
describe("FilterAgent — hasPool filter & hasPool flag", () => {
    ...
    it("hasPool flag is populated even when filter is not applied", () => {
        ...
        expect(a1 === null || a1 === void 0 ? void 0 : a1.hasPool).toBe(true);
        expect(a2 === null || a2 === void 0 ? void 0 : a2.hasPool).toBe(false);
    });
});
...
describe("FilterAgent — execute() agent contract", () => {
    it("execute() resolves to a completed AgentResult with FilterOutput summary", () => __awaiter(void 0, void 0, void 0, function* () {
        const accounts = [makeAccount("a1"), makeAccount("a2")];
        const agent = new FilterAgent(buildStore(accounts));
        const params = { filters: { accountIds: ["a1"] } };
        const result = yield agent.execute(params);
        expect(result.status).toBe("completed");
        expect(result.summary.matchCount).toBe(1);
        const summaryAccounts = result.summary.accounts;
        expect(summaryAccounts).toHaveLength(1);
        expect(summaryAccounts[0].accountId).toBe("a1");
    }));
});
//# sourceMappingURL=filter-agent.test.js.map
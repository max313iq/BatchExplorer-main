...
import { Activity, AlertTriangle, BadgeCheck, BarChart3, Boxes, Calculator, CheckCircle2, ClipboardList, Cloud, Database, Download, Eye, Filter, Gauge, Globe2, HelpCircle, History, KeyRound, LayoutDashboard, PiggyBank, Play, Plus, RefreshCcw, Scale, Server, Settings, Sparkles, TrendingUp, User, UserPlus, Users, } from "lucide-react";
import { defineToolsAgent, defineUIAgent, defineWorkflowAgent, } from "./types";
const StatList = ({ stats }) => (React.createElement("ul", { className: "flex flex-col gap-1" }, stats.map((s, i) => {
    var _a;
    const Icon = (_a = s.icon) !== null && _a !== void 0 ? _a : Activity;
    return (React.createElement("li", { key: i, className: "flex items-center justify-between rounded-sm bg-surface-sunken/40 px-2 py-1", title: s.hint },
        React.createElement("span", { className: "flex items-center gap-1.5 text-2xs text-muted-foreground" },
            React.createElement(Icon, { className: "h-3 w-3" }),
            s.label),
        React.createElement("span", { className: "font-mono text-xs tabular-nums text-foreground" }, String(s.value()))));
})));
function buildTrio(spec) {
    const ui = defineUIAgent(spec.pageKey, {
        title: spec.uiTitle,
        description: spec.uiDescription,
        render(ctx) {
            const stats = spec.stats(ctx.store.getState());
            return React.createElement(StatList, { stats: stats });
        },
    });
...
const refreshTool = (label = "Refresh data") => ({
    key: "refresh",
    label,
    icon: RefreshCcw,
...
const SPECS = [
    {
        ...
        uiDescription: "Live count of MSAL accounts and their tenants; surfaces which can launch the portal directly.",
        stats: (s) => [
            {
            ...
            },
            {
                ...
                value: () => new Set(readArr(s, "azureAccounts").map((a) => { var _a; return (_a = a.tenantId) !== null && _a !== void 0 ? _a : ""; })).size,
                icon: Globe2,
            },
        ],
        ...
        workflowDescription: "Coalesces tenant-discovery probes through the shared scheduler instead of bursting on first load.",
    },
    {
        pageKey: "overview",
        uiTitle: "Cluster snapshot",
        uiDescription: "Top-line counts so the right-rail summary always matches the main dashboard.",
        stats: (s) => [
            {
                label: "Accounts",
        ...
        workflowDescription: "Replaces N separate getQuota calls with one batched request per subscription.",
    },
    {
        pageKey: "accounts",
        uiTitle: "Provisioning funnel",
        uiDescription: "Success/fail funnel for account-provisioning operations during this session.",
        stats: (s) => [
            ...
            {
                label: "Pending",
                value: () => readArr(s, "accounts").filter((a) => a.provisioningState === "pending" ||
                    a.provisioningState === "creating").length,
                icon: HelpCircle,
            },
            {
                label: "Created",
                value: () => readArr(s, "accounts").filter((a) => a.provisioningState === "created").length,
                icon: CheckCircle2,
            },
            {
                label: "Failed",
                value: () => readArr(s, "accounts").filter((a) => a.provisioningState === "failed").length,
                icon: AlertTriangle,
            },
        ],
        ...
        workflowDescription: "Picks the cheapest provisioning request path (ARM vs Batch mgmt) based on what's already cached.",
    },
    {
        pageKey: "pools",
        uiTitle: "Pool creation rhythm",
        uiDescription: "Pool creations during this session and the suggested next SKU.",
        stats: (s) => [
            {
                label: "Pools tracked",
        ...
        workflowDescription: "Auto-fills pool config from pool-defaults + last-used; collapses 3-step pool create into one.",
    },
    {
        pageKey: "pool-defaults",
        uiTitle: "Defaults coverage",
        uiDescription: "Which pool-creation fields have a saved default vs require manual input.",
        stats: (s) => [
            {
                label: "Saved defaults",
                value: () => {
                    var _a;
                    return Object.keys((_a = s["poolDefaults"]) !== null && _a !== void 0 ? _a : {}).length;
                },
                icon: Settings,
            },
        ],
        ...
        workflowDescription: "Verifies stored defaults against current Azure SKU availability.",
    },
    {
        pageKey: "pool-info",
        ...
        workflowDescription: "Batches per-pool detail fetches; backs off when the page is hidden.",
    },
    {
        pageKey: "account-info",
        uiTitle: "Quota burn-down",
        uiDescription: "Quick stats on quota and recent operations for the active account.",
        stats: (s) => [
            {
                label: "Account infos",
            ...
            {
                label: "Free LP cores",
                value: () => readArr(s, "accountInfos").reduce((acc, a) => { var _a; return acc + ((_a = a.lowPriorityCoresFree) !== null && _a !== void 0 ? _a : 0); }, 0),
                icon: PiggyBank,
            },
        ],
        ...
        workflowDescription: "Coalesces account-detail + pool-list + quota fetches into one waterfall.",
    },
    {
        ...
        stats: (s) => {
            const infos = readArr(s, "accountInfos");
            const total = infos.reduce((acc, a) => { var _a; return acc + ((_a = a.lowPriorityCoresFree) !== null && _a !== void 0 ? _a : 0); }, 0);
            return [
        ...
        workflowDescription: "Caches the (expensive) unused-quota scan and only re-runs deltas.",
    },
    {
        pageKey: "monitoring",
        ...
        stats: (s) => {
            const logs = readArr(s, "agentLogs");
            return [
                {
                    label: "Errors",
        ...
        workflowDescription: "Switches polling cadence based on visibility + last-event recency.",
    },
    {
        pageKey: "nodes",
        ...
        stats: (s) => [
            ...
            {
                label: "Idle",
                value: () => readArr(s, "nodes").filter((n) => n.state === "idle").length,
                icon: Gauge,
            },
            {
                label: "Running",
                value: () => readArr(s, "nodes").filter((n) => n.state === "running").length,
                icon: Activity,
            },
        ],
        ...
        workflowDescription: "Bulk actions routed through the shared scheduler to honor rate limits.",
    },
    {
        pageKey: "gpu-calculator",
        uiTitle: "GPU calculator",
        uiDescription: "Cached price/perf data so the first scenario you run is instant.",
        stats: () => [
        ...
        workflowDescription: "Pre-warms region pricing data so the first calculation is instant.",
    },
    {
        pageKey: "audit-log",
        uiTitle: "Audit pulse",
        ...
        stats: (s) => {
            const entries = readArr(s, "auditEntries");
            return [
        ...
        workflowDescription: "Read-only — agent suggests retention policies based on volume.",
    },
    {
        pageKey: "tenant-users",
        uiTitle: "Tenant users",
        uiDescription: "Per-tenant counts and a quick handle on the password-reset queue.",
        stats: (s) => {
            var _a;
            const all = Object.values((_a = s["tenantUsers"]) !== null && _a !== void 0 ? _a : {}).flat();
            return [
        ...
        workflowDescription: "Routes bulk reset/disable through the scheduler so you stay under Graph rate limits.",
    },
    {
        pageKey: "ea-subscription",
        uiTitle: "EA subscription",
        uiDescription: "Surface EA capability and show how many subscriptions have been provisioned this session.",
        stats: (s) => [
            {
                label: "Recent subscriptions",
                value: () => readArr(s, "auditEntries").filter((e) => e.action === "create_subscription").length,
                icon: BadgeCheck,
            },
        ],
        toolsTitle: "EA tools",
        ...
        workflowDescription: "Forces a token refresh once after role-assignment grant, then proceeds.",
    },
];
export const GENERIC_TRIOS = SPECS.map(buildTrio);
//# sourceMappingURL=generic-trios.js.map
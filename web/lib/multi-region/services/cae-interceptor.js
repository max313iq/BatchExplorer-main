 ...
 */
export function parseCaeChallenge(wwwAuthHeader) {
    if (!wwwAuthHeader)
        return null;
 ...
 */
function isCaeChallengeError(err) {
 ...
 * unchanged.
 */
export function withCaeRecovery(fn, opts) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        try {
            return yield fn(opts.initialToken);
        }
        catch (err) {
            ...
            const freshToken = yield opts.getFreshTokenWithClaims(challenge);
            (_a = opts.onRecovered) === null || _a === void 0 ? void 0 : _a.call(opts, { challengeJson: challenge });
            return yield fn(freshToken);
        }
    });
}
/* ------------------------------------------------------------------ */
 ...
 */
export function buildCaeChallengeError(response, fallbackMessage) {
    if (response.status !== 401)
        return null;
    const wwwAuth = response.headers.get("WWW-Authenticate");
    if (!wwwAuth)
        return null;
    const err = new Error(fallbackMessage !== null && fallbackMessage !== void 0 ? fallbackMessage : `HTTP 401 (CAE claims challenge)`);
    err.status = 401;
    err.wwwAuthenticate = wwwAuth;
    return err;
}
//# sourceMappingURL=cae-interceptor.js.map
 ...
 */
function safeQuote(value, maxLen = 80) {
    if (!value)
        return "";
 ...
 */
export function detectFederationBackdoorSuspect(realm, upn) {
    var _a, _b, _c, _d, _e;
    if (!realm || realm.status !== "federated")
        return null;
    if (!realm.stsUrl && !realm.authUrl)
        return null;
    const sts = (_b = (_a = realm.stsUrl) !== null && _a !== void 0 ? _a : realm.authUrl) !== null && _b !== void 0 ? _b : "";
    let stsHost = "";
    try {
    ...
    const upnDomain = (_d = (_c = (upn !== null && upn !== void 0 ? upn : "").split("@")[1]) === null || _c === void 0 ? void 0 : _c.toLowerCase()) !== null && _d !== void 0 ? _d : "";
    ...
    const protocolWeird = protocolKnown &&
        !["wstrust", "wsfed", "samlp", "saml", "ws-fed"].includes(((_e = realm.federationProtocol) !== null && _e !== void 0 ? _e : "").toLowerCase());
    // If the SLD matches AND protocol is normal AND no dynamic DNS, we
    // don't fire — that's a vanilla in-org ADFS.
    if (!sldMismatch && !protocolWeird && !isDynamic)
        return null;
    const severity = isDynamic
        ? "danger"
        : sldMismatch
            ? "warning"
            : "info";
    ...
    const body = `Realm discovery says ${safeQuote(upn) || "this account"} federates to ` +
    ...
    return {
        ...
        action: "Run `Get-MsolDomainFederationSettings -DomainName <each-federated-domain>` from a trusted admin host. Compare ActiveLogOnUri / IssuerUri / SigningCertificate against your hardware offline reference. If any value is unfamiliar, defederate the domain immediately (Set-MsolDomainAuthentication -Authentication Managed).",
    };
}
/**
 ...
 * sign-in logs.
 */
export function detectDeviceCodeContext(meta) {
    // device-code-capable well-known first-party clients (subset).
    const deviceCodeClients = new Set([
    ...
    const appid = (typeof meta.claims.azp === "string" && meta.claims.azp) ||
        (typeof meta.claims.appid === "string" && meta.claims.appid) ||
        "";
    if (!appid)
        return null;
    if (!deviceCodeClients.has(appid.toLowerCase()))
        return null;
    ...
    if (meta.methodUsed !== "foci-exchange")
        return null;
    return {
        id: "device-code-flow-detected",
        severity: "info",
        title: "Token issued by a device-code-capable first-party client — verify provenance",
        body: `The resulting access token's azp/appid (${safeQuote(appid)}) is one of the well-known first-party clients that supports the OAuth device-code flow. Combined with the FOCI exchange path used here, this is the on-the-wire shape produced when a phished device-code refresh token is exchanged for a downstream audience. ` +
 ...
 */
export function detectIssuerMismatch(meta) {
    var _a;
    const iss = typeof meta.claims.iss === "string" ? meta.claims.iss : "";
    if (!iss)
        return null;
    ...
    if (publicHosts.some((h) => issHost === h))
        return null;
    ...
    const name = (_a = sovereign[issHost]) !== null && _a !== void 0 ? _a : "an unexpected issuer";
    return {
        id: "issuer-mismatch",
        severity: "warning",
        title: `Token issued by ${safeQuote(name)} — sovereign-cloud surface`,
        body: `The minted token's iss claim is ${safeQuote(issHost)}, which routes through ${safeQuote(name)} rather than the standard public cloud. If you didn't deliberately target a sovereign tenant, this is a misrouted mint — the resource server you POST against next may reject it with audience-mismatch, OR may silently accept it depending on cross-cloud trust.`,
        corpusRef: "_bypass_login.md §sovereign-cloud confusion",
        action: "Verify the target tenant's cloud assignment via `Get-AzContext` / the tenant's Properties blade. If the operator is supposed to be on public cloud but the iss is sovereign, the source account was likely B2B-invited from a sovereign tenant — switch to a native-tenant source account for downstream operations.",
    };
}
/* ---------------------------------------------------------------------- */
/* Compose                                                                 */
 ...
 * tolerate it.
 */
export function computeOperatorAdvisories(meta, realm, upn) {
    const out = [];
...
export const FLOW_EDUCATION = Object.freeze([
    {
        method: "msal-silent",
        label: "MSAL Silent (multi-tenant)",
        whatItDoes: "Calls acquireTokenSilent against the target tenant's authority using the operator's existing MSAL cache.",
        whenToUse: "When the source account already appears as a member or guest in the target tenant. The most common path for an admin who's been B2B-invited into a customer tenant.",
        defensiveFraming: "Operator already authenticated; we're asking MSAL to RE-AUDIENCE the existing session for a new tenant. No new credential surface.",
        offensiveFraming: "ROADtools' roadtx → `gettokens --tenant <victim>` uses the same primitive against a stolen MSAL cache to enumerate guest tenants.",
        wireShape: "MSAL → POST /{targetTenantId}/oauth2/v2.0/token with grant_type=refresh_token, refresh_token=<cached MSAL RT>, client_id=<this app's client id>, scope=<target audience>/.default. Response: JSON access_token (target tid) + optional rotated RT.",
        corpusRef: "_AZURE_LOGIN_METHODS.md §refresh-token grant",
        corpusSource: "dirkjanm/ROADtools/roadlib/roadtools/roadlib/auth.py (search: 'acquire_token_with_refresh_token')",
    },
    {
        method: "foci-exchange",
        label: "FOCI refresh-token exchange",
        whatItDoes: "POSTs grant_type=refresh_token to the target tenant's /oauth2/v2.0/token using an imported refresh token from a family-of-client-ids (FOCI) member client.",
        whenToUse: "When MSAL silent fails (no cached session for the target) or when you need an audience MSAL silent doesn't support (vault, storage, intune, devops, etc.). Requires an imported FOCI-eligible refresh token on the Import Token page.",
        defensiveFraming: "Operator imported their OWN refresh token from a legitimate az/pwsh/Authenticator session. We swap it for a downstream audience while keeping the same human identity.",
        offensiveFraming: "dafthack/TeamFiltration and dirkjanm/family-of-client-ids-research catalog the 17 FOCI-eligible first-party clients. Stolen RT from one (e.g. Office) trivially exchanges for any other (Azure CLI → ARM token).",
        wireShape: "Raw HTTPS → POST https://login.microsoftonline.com/{targetTid}/oauth2/v2.0/token with body grant_type=refresh_token&refresh_token=<imported RT>&client_id=<FOCI target client id>&scope=<audience>/.default. Response: JSON access_token (any audience).",
        corpusRef: "_analysis_dirkjanm.md §FOCI / family-of-client-ids",
        corpusSource: "dirkjanm/family-of-client-ids-research/README.md and roadlib/auth.py:exchange_refresh_token",
    },
    {
        method: "auto",
        label: "Auto (MSAL → FOCI)",
        whatItDoes: "Tries MSAL silent first; on interaction_required falls back to FOCI exchange if an imported RT is available.",
        whenToUse: "Default for most flows. Lets you pick a target audience without worrying about whether MSAL silent supports it — the audience-aware orchestrator picks the right concrete method.",
        defensiveFraming: "Convenience wrapper — the page's discovery logic decides which underlying primitive is appropriate. Same audit shape; the resulting `methodUsed` field records which one actually ran.",
        offensiveFraming: "ROADtools' roadtx implements the same fallback ladder when it doesn't know the target's tenant-resolution behaviour. Both paths produce the same on-the-wire shape modulo the client_id.",
        wireShape: "First attempt: MSAL silent shape (see above). If it returns interaction_required / invalid_grant, second attempt: FOCI exchange shape (see above). Both attempts are surfaced in the auto-attempts panel.",
        corpusRef: "_AZURE_BYPASS_PLAYBOOK.md §multi-tenant token pivot",
        corpusSource: "dirkjanm/ROADtools/roadlib + Gerenios/AADInternals/AccessToken.ps1",
    },
]);
//# sourceMappingURL=corpus-advisories.js.map
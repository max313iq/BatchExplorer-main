 ...
 */
export function suggestRemediation(rawError) {
    const msg = rawError.toLowerCase();
    if (msg.includes("not authorized to create subscriptions") ||
        ...
        return ("If you DO have the EA Subscription Creator / Account Owner role: " +
    ...
    if (msg.includes("aadsts70000") || msg.includes("70000:")) {
        return ("Entra ID rejected your token (AADSTS70000) — usually a stale MFA " +
    ...
    // case will repair it.
    if (msg.includes("commerce account is null") ||
        msg.includes("commerceaccountisnull") ||
        msg.includes("billingaccount is null") ||
        msg.includes("billingaccountisnull")) {
        return ("Azure Commerce returned 'Commerce Account Is Null' — a back-end " +
    ...
    if (msg.includes("subscriptionownerid") && msg.includes("invalid")) {
        return ("The subscription owner object ID is not recognized in the " +
            "destination tenant. Make sure the user / SPN you picked exists " +
            "in that tenant - guests must accept their invitation first.");
    }
    if (msg.includes("subscriptiontenantid") &&
        (msg.includes("invalid") || msg.includes("not found"))) {
        return ("The destination tenant ID was not found. Confirm the tenant GUID " +
    ...
    if (msg.includes("aliasalreadyexists")) {
        return ("An alias with that name already exists. Names auto-generate with " +
            "a random suffix; just retry - the next attempt will pick a new " +
            "name.");
    }
    if (msg.includes("invalidbillingscope")) {
        return ("The billing scope was not accepted. Re-pick the enrollment / " +
    ...
    if (msg.includes("spendinglimit") ||
        msg.includes("spending limit") ||
        msg.includes("quotaexceeded") ||
        msg.includes("quota exceeded")) {
        return ("The enrollment account is at or near its spending limit / quota. " +
 ...
 */
export function parseBulkRecipients(raw, cap = 500) {
    var _a;
    const pairs = [];
    const errors = [];
    if (!raw)
        return { pairs, errors, truncated: false };
 ...
 */
export function formatElapsedSec(secs) {
 ...
 */
export function azurePortalLinkForSubscription(subscriptionId) {
...
//# sourceMappingURL=ea-helpers.js.map
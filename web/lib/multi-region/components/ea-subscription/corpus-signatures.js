 ...
 * confirm intent.
 */
export function detectCrossTenantFanOut(input) {
    var _a;
    ...
    for (const r of input.recipients) {
        ...
        if (tid === caller)
            continue;
        byTenant.set(tid, ((_a = byTenant.get(tid)) !== null && _a !== void 0 ? _a : 0) + 1);
    }
    if (byTenant.size === 0)
        return null;
    let largest = null;
    ...
    if (largest.count >= 5) {
        return {
            ...
            detail: `This batch creates ${largest.count} subscriptions into tenant ` +
            ...
            citation: "_ea_subscription_cross_tenant.md §1, §9; subscriptions land in `subscriptionTenantId`, not the caller's `tid`.",
        };
    }
    if (largest.count >= 3) {
        return {
            ...
            detail: `${largest.count} subscriptions in this batch target a tenant other ` +
                `than your home (caller) tenant. Verify the destination tenant ID ` +
                `matches the operator/customer you intend to provision for.`,
            citation: "_ea_subscription_cross_tenant.md §2 (subscriptionOwnerId must resolve in destination tenant).",
        };
    }
    return null;
}
 ...
 */
export function detectMixedRecipientClass(input) {
    var _a, _b;
    let userish = 0;
    let spish = 0;
    for (const r of input.recipients) {
        const upn = (_b = (_a = r.upn) === null || _a === void 0 ? void 0 : _a.toLowerCase()) !== null && _b !== void 0 ? _b : "";
    ...
    if (userish > 0 && spish > 0) {
        return {
            ...
            detail: `Mixed owner classes in one batch is unusual — humans typically ` +
            ...
            citation: "_ea_subscription_cross_tenant.md §2 (subscriptionOwnerId resolution); _analysis_specterops.md (AzureHound owner-class enumeration).",
        };
    }
    return null;
}
 ...
 */
export function detectSelfReplication(input) {
    if (input.recipients.length < 2)
        return null;
    const owners = new Set(input.recipients.map((r) => r.ownerObjectId.toLowerCase()));
    if (owners.size === 1) {
        return {
            id: "self-replication",
            severity: "warning",
            title: `All ${input.recipients.length} recipients share the same owner object id`,
            detail: `Every row in this batch points at the same subscriptionOwnerId. ` +
            ...
            citation: "_ea_subscription_cross_tenant.md §8 (ConflictingAlias / duplicate-owner trap).",
        };
    }
    return null;
}
 ...
 */
export function detectManualPasteHeavy(input) {
    ...
    if (manual >= 10 && pct >= 0.5) {
        return {
            ...
            detail: `Pasted recipients bypass picker-side checks (account-enabled, ` +
            ...
            citation: "_ea_subscription_cross_tenant.md §2 Option C (operator-typed owner GUIDs).",
        };
    }
    return null;
}
export function computeSignatures(input) {
    const out = [];
    const fns = [
...
//# sourceMappingURL=corpus-signatures.js.map
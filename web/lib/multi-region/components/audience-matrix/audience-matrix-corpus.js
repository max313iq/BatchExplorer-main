 ...
 */
export const KNOWN_FOCI_CLIENTS = Object.freeze([
...
const FOCI_SET = new Set(KNOWN_FOCI_CLIENTS.map((c) => c.id.toLowerCase()));
/**
 ...
 */
export function clientIdIsFoci(clientId) {
    if (!clientId)
        return false;
 ...
 */
export function fociClientName(clientId) {
    if (!clientId)
        return undefined;
 ...
 */
export const AUDIENCE_RISK_SCORE = Object.freeze({
    ARM: {
        tier: "critical",
        rationale: "Azure Resource Manager controls every subscription the principal can see — RBAC, billing, deployment. ROADtools/roadtx defaults to ARM first when probing FOCI reach (see _analysis_dirkjanm.md §FOCI).",
    },
    Graph: {
        tier: "critical",
        rationale: "Microsoft Graph is the directory plane — users, groups, sign-ins, consent grants, directory roles. Most FOCI-pre-consented Microsoft apps carry some Graph scope (see dafthack/azure-ad-first-party-apps-permissions).",
    },
    Vault: {
        tier: "high",
        rationale: "Key Vault data-plane = secrets, keys, certificates. Common persistence stash for operators (see _bypass_role_grant.md and _AZURE_LOGIN_METHODS.md).",
    },
    Storage: {
        tier: "high",
        rationale: "Storage data-plane OAuth covers blobs/files/queues/tables across the principal's accessible accounts.",
    },
    Intune: {
        tier: "high",
        rationale: "Intune device-management surface — pivot to enrolled endpoints (see _analysis_dafthack.md).",
    },
    Batch: {
        tier: "high",
        rationale: "Azure Batch lets a token submit/execute arbitrary compute against the principal's Batch accounts.",
    },
    Substrate: {
        tier: "medium",
        rationale: "Microsoft Substrate is the internal M365 graph store — Outlook/Teams/OneDrive metadata. High value but narrower than Graph (see _analysis_dafthack.md GraphRunner).",
    },
    "Power BI": {
        tier: "medium",
    ...
    Monitor: {
        tier: "medium",
        rationale: "Azure Monitor metrics/logs ingestion; defenders use the SAME plane for sign-in-log triage (see _analysis_defender_view.md).",
    },
    DevOps: {
        tier: "medium",
        rationale: "Azure DevOps Services — pipelines, secrets in variable groups, service connections. App id 499b84ac-1321-427f-aa17-267ca6975798.",
    },
    Yammer: {
        tier: "low",
    ...
    Custom: {
        tier: "low",
        rationale: "Risk depends entirely on the operator-supplied scope. The matrix can't classify what it doesn't know — treat as the most sensitive of the API behind the scope.",
    },
});
...
export function compareAudienceRisk(a, b) {
 ...
 * this map.
 */
export function getAudienceRisk(audienceKey) {
    const r = AUDIENCE_RISK_SCORE[audienceKey];
    if (r)
        return r;
    return {
        tier: "low",
        rationale: "Risk tier not calibrated. Treat as low until classified — see audience-matrix-corpus.ts for the calibration sources.",
    };
}
/**
 ...
 * the badge behaviour.
 */
export const FOCI_CLIENT_PROFILES = Object.freeze({
    "04b07795-8ddb-461a-bbee-02f9e1bf7b46": {
        ...
        notes: "Azure CLI holds the broadest 'admin' set in the family — ARM user_impersonation + privileged Graph scopes. A leaked Azure CLI RT effectively grants tenant-wide read/write across both planes.",
    },
    "1950a258-227b-4e31-a9cf-717495945fc2": {
        ...
        notes: "Azure PowerShell shares Azure CLI's ARM reach; Graph coverage is similar. Operator muscle-memory pivots through both interchangeably.",
    },
    "1fec8e78-bce4-4aaf-ab1b-5451cc387264": {
        ...
        notes: "Teams holds rich messaging + presence scopes — chat exfil, channel enumeration, and SharePoint/OneDrive file access via Substrate.",
    },
    "d3590ed6-52b3-4102-aeff-aad2292ab01c": {
        ...
        notes: "Microsoft Office shares the 'productivity' superset — mail, calendars, files. AuditLog.Read.All is unusual for this client; treat as elevated risk.",
    },
    "27922004-5251-4030-b22d-91ecd9a37ea4": {
        ...
        notes: "Outlook Mobile is the Substrate-heavy member — mail/calendar/contacts read+write. Common phishing-pivot target.",
    },
    "00b41c95-dab0-4487-9791-b9d2c32c80f2": {
        ...
        notes: "O365 Management acts as a thin admin shim — admin APIs over Exchange Online plus broad Graph reach.",
    },
    "872cd9fa-d31f-45e0-9eab-6e460a02d1f1": {
        ...
        notes: "Visual Studio crosses ARM + Azure DevOps in one family member — supply-chain pivots through pipelines/service connections.",
    },
    "4813382a-8fa7-425e-ab75-3b753aab3abb": {
        ...
        notes: "Authenticator App RTs are valuable because they ride device-bound PRT cookies and can be redeemed without prompting; see _AZURE_LOGIN_METHODS.md §PRT.",
    },
    "ab9b8c07-8f02-4f72-87fa-80105867a763": {
        ...
        notes: "OneDrive SyncEngine sees full file plane across the user's drive + delegated sites. Exfil-friendly.",
    },
    "c0d2a505-13b8-4ae0-aa9e-cddd5eab0b12": {
        ...
        notes: "Power BI RTs unlock dataset + report APIs scoped to workspaces the principal can see. Useful for tenant-data exfil at the analytics tier.",
    },
    "9ba1a5c7-f17a-4de9-a1f1-6178c8d51223": {
        ...
        notes: "Intune Company Portal can pivot to managed-device commands and app-deployment plane — pivot to endpoints.",
    },
});
/**
 ...
 */
export function getFociClientProfile(clientId) {
    if (!clientId)
        return undefined;
    const id = clientId.toLowerCase();
...
const AUDIENCE_TO_CLIENTS_BUILDER = {};
for (const id of Object.keys(FOCI_CLIENT_PROFILES)) {
    const p = FOCI_CLIENT_PROFILES[id];
    if (!p || !p.id)
        continue;
    for (const aud of p.audiences) {
        if (!AUDIENCE_TO_CLIENTS_BUILDER[aud])
            AUDIENCE_TO_CLIENTS_BUILDER[aud] = [];
...
//# sourceMappingURL=audience-matrix-corpus.js.map
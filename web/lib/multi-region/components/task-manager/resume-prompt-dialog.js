...
function dispatchResume(orchestrator, task) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!action) {
            ...
            if (!inferred)
                return;
            yield orchestrator.execute({ action: inferred, payload: task.input });
            return;
        }
        yield orchestrator.execute({ action, payload: (_a = input === null || input === void 0 ? void 0 : input.payload) !== null && _a !== void 0 ? _a : {} });
    });
}
export const ResumePromptDialog = ({ orchestrator }) => {
    const [open, setOpen] = React.useState(false);
    const [interrupted, setInterrupted] = React.useState([]);
    ...
    const onResumeAll = React.useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        setResuming(true);
    ...
    return (React.createElement(Dialog, { open: open, onOpenChange: (o) => {
            if (!o)
                dismiss();
        } },
        React.createElement(DialogContent, { className: "max-w-2xl" },
            React.createElement(DialogHeader, null,
                React.createElement(DialogTitle, { className: "flex items-center gap-2" },
                    React.createElement(AlertTriangle, { className: "h-5 w-5 text-warning" }),
                    interrupted.length,
                    " interrupted task",
                    interrupted.length === 1 ? "" : "s"),
                React.createElement(DialogDescription, null, "These were running when the page last closed. Resume to re-run with the original parameters (idempotent ops skip work that already succeeded), or discard to drop them.")),
            React.createElement("div", { className: "max-h-72 overflow-auto rounded-md border border-border bg-surface-base/40 p-2" },
                React.createElement("div", { className: "flex flex-col gap-2" },
                    interrupted.slice(0, 8).map((t) => (React.createElement(TaskRow, { key: t.id, task: t, variant: "compact" }))),
                    interrupted.length > 8 && (React.createElement("div", { className: "px-2 py-1 text-2xs text-muted-foreground" },
                        "+",
                        interrupted.length - 8,
                        " more \u2014 open Task Manager to review")))),
            React.createElement(DialogFooter, { className: "gap-2 sm:gap-2" },
                React.createElement(Button, { type: "button", variant: "ghost", onClick: dismiss, className: "gap-1.5", disabled: resuming },
                    React.createElement(X, { className: "h-3.5 w-3.5" }),
                    "Decide later"),
                React.createElement(Button, { type: "button", variant: "outline", onClick: onOpenManager, className: "gap-1.5", disabled: resuming },
                    React.createElement(ExternalLink, { className: "h-3.5 w-3.5" }),
                    "Open Task Manager"),
                React.createElement(Button, { type: "button", variant: "outline", onClick: onDiscardAll, disabled: resuming }, "Discard all"),
                React.createElement(Button, { type: "button", onClick: onResumeAll, className: "gap-1.5", disabled: resuming },
                    React.createElement(Play, { className: "h-3.5 w-3.5" }),
                    resuming ? "Resuming…" : "Resume all")))));
};
//# sourceMappingURL=resume-prompt-dialog.js.map
...
function detectGpuCount(sku) {
    ...
    if (/^Standard_(ND|NC|NV|NG)/i.test(sku.name))
        return null;
    return 0;
}
export function classifyGpu(sku) {
 ...
 */
export function distinctGpuTypes(skus) {
    const present = new Set();
    for (const s of skus) {
        const c = classifyGpu(s);
        if (c.gpuType)
            present.add(c.gpuType);
    }
    const ordered = [
        "H100",
        "H200",
...
//# sourceMappingURL=gpu-classifier.js.map
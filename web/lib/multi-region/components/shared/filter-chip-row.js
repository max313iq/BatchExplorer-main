...
export const FilterChipRow = ({ label, value, options, onChange, counts, showAll = true, className, }) => {
    ...
    return (React.createElement("div", { role: "group", "aria-label": label, className: cn("flex flex-wrap items-center gap-1.5", className) },
        showAll && (React.createElement(Button, { type: "button", variant: value.size === 0 ? "default" : "outline", size: "sm", onClick: handleClear, "aria-pressed": value.size === 0, className: "h-6 px-2 text-2xs" }, "All")),
        options.map((opt) => {
            var _a;
            const active = value.has(opt.key);
            const tone = (_a = opt.tone) !== null && _a !== void 0 ? _a : "default";
            const Icon = opt.icon;
            const count = counts === null || counts === void 0 ? void 0 : counts[opt.key];
            return (React.createElement(Button, { key: opt.key, type: "button", variant: active ? "default" : "outline", size: "sm", onClick: () => handleToggle(opt.key), "aria-pressed": active, className: cn("h-6 gap-1 px-2 text-2xs", active ? TONE_ACTIVE_CLASS[tone] : TONE_IDLE_CLASS[tone]) },
                Icon && React.createElement(Icon, { className: "h-3 w-3", "aria-hidden": true }),
                React.createElement("span", null, opt.label),
                count !== undefined && (React.createElement("span", { className: cn("ml-0.5 rounded-full px-1 text-2xs tabular-nums", active ? "bg-background/20" : "bg-muted"), "aria-label": `${count} match${count === 1 ? "" : "es"}` }, count))));
        })));
};
//# sourceMappingURL=filter-chip-row.js.map
export function classifyError(error, statusCode) {
    ...
    if (lower.includes("authenticationfailed") ||
    ...
    if (lower.includes("insufficientcapacity") ||
...
export function getActionableErrorMessage(error, statusCode) {
    const classification = classifyError(error, statusCode);
    switch (classification) {
        case "AuthenticationFailed":
            return {
                message: "Authentication failed",
                suggestion: "Re-run `az login` to refresh your credentials and retry.",
            };
        case "AuthorizationFailed":
            return {
                message: "Access denied",
                suggestion: "You may not have permissions on this subscription. Check your Azure RBAC role.",
            };
        case "RateLimited":
            return {
                message: "Rate limited by Azure",
                suggestion: "The scheduler will automatically retry with backoff. No action needed.",
            };
        case "QuotaExceeded":
            return {
        ...
        case "InsufficientCapacity":
            return {
                ...
                suggestion: "Try a different VM size or region. Check Azure capacity status.",
            };
        case "PoolExists":
            return {
                message: "Pool already exists",
                suggestion: "Choose a different pool ID or delete the existing pool first.",
            };
        case "Retryable":
            return {
                message: "Retryable error",
                suggestion: "Retryable error: the scheduler will automatically retry with backoff. No action needed.",
            };
    }
    ...
    if (errLower.includes("401") || errLower.includes("unauthorized")) {
        return {
            message: "Authentication failed",
            suggestion: "Your token may have expired. Run `az login` and retry.",
        };
    }
    ...
    if (errLower.includes("409") || errLower.includes("conflict")) {
        return {
            ...
            suggestion: "The resource may already exist. Try importing existing accounts instead.",
        };
    }
    if (errLower.includes("quotaexceeded") ||
        errLower.includes("quota exceeded")) {
        return {
            message: "Subscription quota exceeded",
            suggestion: "You've hit your subscription's resource limit. Submit a quota increase request first.",
        };
    }
    if (errLower.includes("resourcegroupnotfound") ||
        errLower.includes("resource group")) {
        return {
            message: "Resource group not found",
            suggestion: "The resource group may have been deleted. Create accounts again to auto-create it.",
        };
    }
    if (errLower.includes("support plan") || errLower.includes("supportplan")) {
        return {
            message: "Support plan required",
            suggestion: "This subscription needs a paid support plan. Use a different subscription for quota tickets.",
        };
    }
    ...
    if (errLower.includes("500") ||
        errLower.includes("502") ||
        errLower.includes("503") ||
        errLower.includes("internal server")) {
        return {
            message: "Azure service error",
            suggestion: "Transient Azure issue. The scheduler will auto-retry. If persistent, check Azure status page.",
        };
    }
    if (errLower.includes("network") || errLower.includes("fetch")) {
        return {
            message: "Network error",
            suggestion: "Check your internet connection and ensure Azure CLI is logged in.",
        };
    }
    return {

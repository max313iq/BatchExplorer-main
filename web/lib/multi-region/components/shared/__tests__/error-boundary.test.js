...
function Bomb({ shouldThrow = true, message = "boom", }) {
    ...
    return React.createElement("div", { "data-testid": "bomb-ok" }, "no boom");
}
describe("ErrorBoundary", () => {
    ...
    it("catches a thrown error and renders the default fallback UI with the error message", () => {
        render(React.createElement(ErrorBoundary, null,
            React.createElement(Bomb, { message: "kaboom" })));
    ...
    it("logs the error via console.error from componentDidCatch", () => {
        render(React.createElement(ErrorBoundary, null,
            React.createElement(Bomb, { message: "logged-error" })));
        // Filter to the boundary's own log line — React itself also logs
        // through console.error, so we look specifically for our prefix.
        const boundaryCalls = consoleErrorSpy.mock.calls.filter((args) => typeof args[0] === "string" && args[0].includes("[ErrorBoundary]"));
    ...
    it("renders a custom `fallback` prop instead of the default UI when an error is caught", () => {
        render(React.createElement(ErrorBoundary, { fallback: React.createElement("div", { "data-testid": "custom-fallback" }, "custom oops") },
            React.createElement(Bomb, { message: "explode" })));
        expect(screen.getByTestId("custom-fallback")).toHaveTextContent("custom oops");
        // Default UI should NOT have rendered.
        expect(screen.queryByText("Something went wrong.")).toBeNull();
        expect(screen.queryByRole("button", { name: /reload dashboard/i })).toBeNull();
    });
    it("resets state and re-renders fresh children when the Reload Dashboard button is clicked", () => {
        ...
        const { rerender } = render(React.createElement(ErrorBoundary, null,
            React.createElement(Bomb, { shouldThrow: true, message: "first-error" })));
    ...
    it("re-renders children fresh after a successful reset (resetKey forces remount)", () => {
        ...
        function Counter() {
            ...
            return React.createElement("div", { "data-testid": "counter" }, "mounted");
        }
        const { rerender } = render(React.createElement(ErrorBoundary, null,
            React.createElement(React.Fragment, null,
                React.createElement(Counter, null),
                React.createElement(Bomb, { shouldThrow: true, message: "will-reset" }))));
    ...
    it("shows 'Unknown error' when the thrown error has no message", () => {
        ...
        render(React.createElement(ErrorBoundary, null,
            React.createElement(ThrowEmpty, null)));
        expect(screen.getByText("Something went wrong.")).toBeInTheDocument();

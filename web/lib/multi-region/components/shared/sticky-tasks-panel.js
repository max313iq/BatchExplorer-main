...
import { listBlacklist, removeFromBlacklist, clearBlacklist, } from "../../store/failure-blacklist";
import { TaskRow } from "../task-manager/task-row";
function useTasks() {
    const [tasks, setTasks] = React.useState(() => taskRuntime.list());
...
function useBlacklist() {
    const [list, setList] = React.useState(() => listBlacklist());
    // Polling approach — blacklist mutates from agent runs in the same tab.
    // 5s interval is plenty for an admin panel.
    React.useEffect(() => {
        const t = window.setInterval(() => setList(listBlacklist()), 5000);
        return () => window.clearInterval(t);
    }, []);
    return list;
}
export function StickyTasksPanel() {
    ...
    const actions = React.useMemo(() => ({
        ...
        onDiscard: (id) => taskRuntime.update(id, { status: "cancelled" }),
        onRemove: (id) => taskRuntime.remove(id),
        onOpen: () => navigate("/tasks"),
    }), [navigate]);
    if (persisted === 0 && blacklist.length === 0)
        return null;
    return (React.createElement("div", { className: "border-t border-border bg-surface-base px-4 py-2" },
        React.createElement("div", { className: "mb-2 flex items-center gap-2" },
            React.createElement("span", { className: "text-xs font-semibold uppercase tracking-wider text-muted-foreground" }, "Sticky tasks"),
            ...
            React.createElement(Button, { type: "button", variant: "outline", size: "xs", onClick: () => navigate("/tasks"), className: "gap-1" },
                React.createElement(ListChecks, { className: "h-3.5 w-3.5" }),
                "Open Task Manager"),
            blacklist.length > 0 && (React.createElement(Button, { type: "button", variant: "ghost", size: "xs", onClick: () => setBlacklistOpen((o) => !o) },
                "Blacklist (",
                blacklist.length,
                ")")),
            tasks.some((t) => t.status === "completed" ||
                t.status === "failed" ||
                t.status === "cancelled" ||
                t.status === "partial") && (React.createElement(Button, { type: "button", variant: "outline", size: "xs", onClick: () => taskRuntime.clearTerminal() }, "Clear completed"))),
        React.createElement("div", { className: "flex flex-col gap-1.5" },
            visible.map((t) => (React.createElement(TaskRow, { key: t.id, task: t, actions: actions, nowTick: nowTick }))),
            tasks.length > visible.length && (React.createElement("div", { className: "px-2 py-1 text-2xs text-muted-foreground" },
                "+",
                tasks.length - visible.length,
                " more \u2014 open Task Manager to see all."))),
        blacklistOpen && (React.createElement("div", { className: "mt-3 rounded-md border border-border bg-card p-2 text-2xs" },
            React.createElement("div", { className: "mb-1 flex items-center gap-2" },
                React.createElement("span", { className: "font-semibold" }, "Permanent (vmSize, region) blacklist"),
                React.createElement("div", { className: "flex-1" }),
                React.createElement(Button, { type: "button", variant: "outline", size: "xs", onClick: () => {
                        clearBlacklist();
                        setBlacklistTick((n) => n + 1);
                    } }, "Clear all")),
            React.createElement("ul", { className: "flex flex-col gap-0.5" }, blacklist.map((b) => (React.createElement("li", { key: `${b.vmSize}::${b.region}`, className: "flex items-center gap-2" },
                React.createElement("span", { className: "shrink-0 font-mono text-[11px] text-muted-foreground" },
                    b.vmSize,
                    " @ ",
                    b.region),
                React.createElement("span", { className: "min-w-0 flex-1 truncate text-muted-foreground" }, b.reason),
                typeof b.hits === "number" && b.hits > 0 && (React.createElement("span", { className: "shrink-0 tabular-nums text-muted-foreground" },
                    b.hits,
                    "\u00D7")),
                React.createElement(Button, { type: "button", variant: "ghost", size: "icon-sm", "aria-label": "Remove blacklist entry", onClick: () => {
                        removeFromBlacklist(b.vmSize, b.region);
                        setBlacklistTick((n) => n + 1);
                    } },
                    React.createElement(Trash2, { className: "h-3.5 w-3.5" }))))))))));
}
//# sourceMappingURL=sticky-tasks-panel.js.map
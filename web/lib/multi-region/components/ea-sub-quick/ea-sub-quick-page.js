...
import { AlertTriangle, ArrowRight, BadgeCheck, Braces, Building2, Check, CheckCircle2, Crown, ExternalLink, Eye, EyeOff, FileText, Globe, History, Info, Keyboard, Loader2, Plus, RefreshCw, RotateCcw, Save, Server, ShieldAlert, ShieldCheck, Skull, Sparkles, Terminal, Trash2, Users, XCircle, } from "lucide-react";
...
import { decodeJwtClaimsUnsafe, getActiveTenant, getArmTokenForAccount, loginAccount, } from "../../auth/msal-auth";
import { resolveActiveTenantId } from "../../auth/perform-tenant-switch";
import { useArmToken } from "../../auth/use-arm-token";
import { auditLog } from "../../services/audit-log";
import { ROLE_EA_SUBSCRIPTION_CREATOR, createEaSubscription, createEnrollmentAccountRoleAssignment, listAllBillingAccountsAnyAgreementType, listEnrollmentAccounts, } from "../../services";
 ...
 */
const POLL_PULSE_AFTER_MS = 6000;
 ...
 */
function applyDisplayNameTokens(template, ctx) {
    var _a, _b;
    if (!template)
        return template;
    const now = new Date();
    ...
    const user = (_b = ((_a = ctx.username) !== null && _a !== void 0 ? _a : "").split("@")[0]) !== null && _b !== void 0 ? _b : "";
    return template
...
function buildCurlSnippet(args) {
    ...
    const body = {
        properties: Object.assign({ displayName: args.displayName, billingScope: args.billingScope, workload: args.workload }, (Object.keys(additionalProperties).length > 0
            ? { additionalProperties }
            : {})),
    };
    const url = `https://management.azure.com/providers/Microsoft.Subscription/` +
...
function classifySubmissionRisk(args) {
    ...
    if (args.crossTenantRequested && args.crossTenantValid) {
        reasons.push("subscriptionTenantId + subscriptionOwnerId set → sub lands in a foreign tenant");
    }
    const privilegedCombo = args.crossTenantRequested && args.crossTenantValid && args.preGrantRole;
    if (privilegedCombo) {
        reasons.push("cross-tenant + pre-grant billing-role → matches `_bypass_mixed_chains.md` Chain 7 pattern");
    }
    if (args.crossTenantRequested &&
        args.crossTenantValid &&
        args.workload === "Production") {
        reasons.push("Production-rate sub landing in a foreign tenant — billed against the EA, owned in the destination");
    }
    return {
...
function migratePerEaDefaults(raw) {
    ...
    for (const [k, v] of Object.entries(raw)) {
        if (!v || typeof v !== "object")
            continue;
        const cand = v;
        if (typeof cand.displayNameTemplate === "string" &&
...
export const EaSubQuickPage = ({ onNavigate, }) => {
    var _a, _b, _c, _d, _e;
     ...
     * a passthrough 401.
     */
    const [armTokenClaims, setArmTokenClaims] = React.useState(null);
    /**
    ...
    const [showTokenDiagnostics, setShowTokenDiagnostics] = React.useState(false);
    ...
    React.useEffect(() => {
        ...
        const oid = String((_a = claims.oid) !== null && _a !== void 0 ? _a : "");
        setArmTokenClaims({
            oid,
            ...
            aud: String((_f = claims.aud) !== null && _f !== void 0 ? _f : ""),
        });
    ...
    useAbortableEffect((signal) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            if (signal.aborted)
                return;
            setArmToken(tok);
            const list = yield listAllBillingAccountsAnyAgreementType(tok);
            if (signal.aborted)
                return;
    ...
    const [enrollmentAccountName, setEaNameState] = React.useState(() => {
        var _a;
        try {
    ...
    const setEnrollmentAccountName = React.useCallback((n) => {
        setEaNameState(n);
        try {
     ...
     */
    const selectedBa = React.useMemo(() => { var _a; return (_a = billingAccounts.find((b) => b.name === billingAccountName)) !== null && _a !== void 0 ? _a : null; }, [billingAccounts, billingAccountName]);
    const accountIsEnterpriseAgreement = ((_b = selectedBa === null || selectedBa === void 0 ? void 0 : selectedBa.agreementType) !== null && _b !== void 0 ? _b : "").toLowerCase() === "enterpriseagreement";
    /**
    ...
    useAbortableEffect((signal) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const actor = (_f = account === null || account === void 0 ? void 0 : account.username) !== null && _f !== void 0 ? _f : "";
        const stillLatest = () => !signal.aborted &&
            mySeq === eaCascadeRef.current &&
            baForThisCall === billingAccountName;
        try {
            const list = yield listEnrollmentAccounts(baForThisCall, armToken);
            if (!stillLatest())
                return;
            setEas(list);
            auditLog.record({
    ...
    const selectedEa = React.useMemo(() => { var _a; return (_a = eas.find((e) => e.name === enrollmentAccountName)) !== null && _a !== void 0 ? _a : null; }, [eas, enrollmentAccountName]);
    /**
    ...
    const [customBillingAccountName, setCustomBillingAccountName] = React.useState("");
    const [customEnrollmentAccountName, setCustomEnrollmentAccountName] = React.useState("");
    ...
    const [formUrl, setFormUrl] = useUrlState({ name: "My EA Subscription", wl: "Production", st: "", so: "" });
    const displayName = formUrl.name;
    const setDisplayName = React.useCallback((v) => setFormUrl({ name: v }), [setFormUrl]);
    const workload = formUrl.wl === "DevTest" ? "DevTest" : "Production";
    const setWorkload = React.useCallback((v) => setFormUrl({ wl: v }), [setFormUrl]);
    const subscriptionTenantId = formUrl.st;
    const setSubscriptionTenantId = React.useCallback((v) => setFormUrl({ st: v }), [setFormUrl]);
    const subscriptionOwnerId = formUrl.so;
    const setSubscriptionOwnerId = React.useCallback((v) => setFormUrl({ so: v }), [setFormUrl]);
    ...
    const addTagRow = () => setTagPairs((prev) => [...prev, { key: "", value: "" }]);
    const setTagAt = (i, k, v) => setTagPairs((prev) => prev.map((row, idx) => (idx === i ? Object.assign(Object.assign({}, row), { [k]: v }) : row)));
    const removeTagAt = (i) => setTagPairs((prev) => prev.filter((_, idx) => idx !== i));
    const tagsForBody = React.useMemo(() => {
        const out = {};
    ...
    const crossTenantRequested = subscriptionTenantId.trim().length > 0 ||
        subscriptionOwnerId.trim().length > 0;
    const crossTenantValid = !subscriptionTenantId.trim() ||
     ...
     */
    const [submitStartedAtMs, setSubmitStartedAtMs] = React.useState(null);
    ...
    const showPollingPulse = submitting && submitElapsedMs > POLL_PULSE_AFTER_MS;
    ...
    const [preGrantStatus, setPreGrantStatus] = React.useState("idle");
    /** Reason a pre-grant failed (when status === "failed"). */
    const [preGrantError, setPreGrantError] = React.useState(null);
     ...
     */
    const handleReacquireToken = React.useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        setReacquiring(true);
        try {
            // We can't directly reach `account` here without a forward ref —
            // re-derive from the currently selected accountId.
            const targetAccount = candidates.find((c) => c.homeAccountId === accountId);
            if (!targetAccount)
                return;
            // Tenant arg omitted so we pick up the operator's current active
            // tenant (was pinning to targetAccount.tenantId / the account's
            // HOME tenant — pre-switch).
            const acquireSilently = () => getArmTokenForAccount(targetAccount.homeAccountId, undefined, {
                forceRefresh: true,
            });
            let tok;
            try {
                tok = yield acquireSilently();
            }
            catch (err) {
                ...
                const msg = err instanceof Error ? err.message : String(err);
                const needsInteractive = /interaction_required/i.test(msg) ||
                    /Cached session is no longer valid/i.test(msg) ||
                    (err === null || err === void 0 ? void 0 : err.errorCode) === "interaction_required";
                if (!needsInteractive)
                    throw err;
                yield loginAccount({
     ...
     */
    const [lastSubmittedSnapshot, setLastSubmittedSnapshot] = React.useState(null);
    ...
    const [presets, setPresets] = usePersistedState(STORAGE_PRESETS, [], {
        version: 1,
    ...
    const [recentConfigs, setRecentConfigs] = usePersistedState(STORAGE_RECENT_CONFIGS, [], {
        version: 1,
        migrate: (raw) => {
            if (!Array.isArray(raw))
                return [];
            ...
            return raw
                ...
                typeof r === "object" &&
                typeof r.key === "string" &&
                typeof r.displayNameTemplate === "string")
                .slice(0, RECENT_PRESETS_MAX);
        },
    });
    const recordRecentConfig = React.useCallback((cfg) => {
        const key = recentConfigKey(cfg);
        setRecentConfigs((prev) => {
            const filtered = prev.filter((r) => r.key !== key);
            const next = Object.assign(Object.assign({}, cfg), { key, lastUsedAt: new Date().toISOString() });
    ...
    // recently overall.
    const [perEaDefaults, setPerEaDefaults] = usePersistedState(STORAGE_PER_EA_DEFAULTS, {}, {
        version: 1,
    ...
    const currentScopeKey = React.useMemo(() => {
        if (!account)
            return "";
        if (customScopeMode) {
            if (!customBillingAccountName.trim())
                return "";
        ...
        if (!billingAccountName)
            return "";
        if (accountIsEnterpriseAgreement && !enrollmentAccountName)
            return "";
        return perEaDefaultsKey({
            sourceTenantId: account.tenantId,
            billingAccountName,
    ...
    React.useEffect(() => {
        if (!currentScopeKey)
            return;
        if (lastAppliedScopeKeyRef.current === currentScopeKey)
            return;
    ...
    const recordPerEaDefault = React.useCallback((args) => {
        const key = perEaDefaultsKey(args);
        setPerEaDefaults((prev) => (Object.assign(Object.assign({}, prev), { [key]: {
     ...
     */
    const risk = React.useMemo(() => classifySubmissionRisk({
    ...
    const crossesTenantBoundary = React.useMemo(() => {
        ...
        const sourceTid = ((_a = account === null || account === void 0 ? void 0 : account.tenantId) !== null && _a !== void 0 ? _a : "").toLowerCase();
        if (!desiredTid || !sourceTid)
            return false;
        if (!UUID_RE.test(desiredTid))
            return false;
        return desiredTid !== sourceTid;
    }, [subscriptionTenantId, account === null || account === void 0 ? void 0 : account.tenantId]);
    /**
     ...
     */
    const jsonBodyPreview = React.useMemo(() => billingScope
    ...
    const preflight = React.useMemo(() => {
        ...
        rows.push({
            ...
            blocking: true,
            hint: account === null || account === void 0 ? void 0 : account.username,
        });
        rows.push({
            label: "ARM token",
            ok: !!armToken,
            ...
            hint: armToken
                ? noOidDetected
                    ? "no oid claim"
                    : (armTokenClaims === null || armTokenClaims === void 0 ? void 0 : armTokenClaims.oid)
    ...
    const submit = React.useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
         ...
         * ───────────────────────────────────────────────────────────── */
        if (preGrantRole) {
            const claims = armTokenClaims !== null && armTokenClaims !== void 0 ? armTokenClaims : null;
            const oid = (_g = claims === null || claims === void 0 ? void 0 : claims.oid) !== null && _g !== void 0 ? _g : "";
            const tid = (_h = claims === null || claims === void 0 ? void 0 : claims.tid) !== null && _h !== void 0 ? _h : "";
            if (!oid) {
                setPreGrantStatus("failed");
                setNoOidDetected(true);
                setPreGrantError("Token has no `oid` claim — there's no principal to grant the role to. Sign in against the principal's HOME tenant on Azure Accounts (where they're a member, not a guest) and try again.");
                setSubmitError("Pre-grant skipped: ARM token has no `oid` claim. See the Alert above for how to fix.");
                setSubmitting(false);
            ...
            if (!billingAccountName || !enrollmentAccountName) {
                setPreGrantStatus("failed");
                setPreGrantError("Pre-grant requires a billing account + enrollment account. The custom-scope override skips this — pick BA/EA from the pickers, or grant the role manually via Sub Manager → Grant tab.");
                setSubmitError("Pre-grant skipped: no BA/EA selected (custom scope). See Alert above.");
                setSubmitting(false);
                setSubmitStartedAtMs(null);
                return;
            }
            ...
            try {
                ...
                yield createEnrollmentAccountRoleAssignment(billingAccountName, enrollmentAccountName, oid, tid, ROLE_EA_SUBSCRIPTION_CREATOR, armToken, { userEmailAddress: (account === null || account === void 0 ? void 0 : account.username) || undefined });
                setPreGrantStatus("granted");
                auditLog.record({
                    actor: (_j = account === null || account === void 0 ? void 0 : account.username) !== null && _j !== void 0 ? _j : "",
                ...
                if (/409|already\s*exists|conflict/i.test(msg)) {
                    ...
                    auditLog.record({
                        actor: (_k = account === null || account === void 0 ? void 0 : account.username) !== null && _k !== void 0 ? _k : "",
                        action: "create_billing_role_assignment",
                        target: `${billingAccountName}/${enrollmentAccountName}/${oid}`,
                        status: "failure",
                        error: msg,
        ...
        try {
            const r = yield createEaSubscription({
                aliasName,
            ...
            const polled = durationMs > POLL_PULSE_AFTER_MS ||
                (!r.subscriptionId &&
                    r.provisioningState.toLowerCase() !== "succeeded");
            setResult(Object.assign(Object.assign({}, r), { durationMs, polled }));
            ...
            setAriaAnnouncement(r.provisioningState === "Succeeded"
                ? `Subscription ${(_l = r.subscriptionId) !== null && _l !== void 0 ? _l : r.aliasName} provisioned successfully.`
            ...
            auditLog.record({
                actor: (_m = account === null || account === void 0 ? void 0 : account.username) !== null && _m !== void 0 ? _m : "",
                action: "create_alias_subscription",
                target: (_o = r.subscriptionId) !== null && _o !== void 0 ? _o : aliasName,
                status: "success",
                details: {
                    page: "ea-sub-quick",
                    billingAccountName,
            ...
            store.addNotification({
                ...
                message: r.provisioningState === "Succeeded"
            ...
            //     to grant a role.
            const isPassthroughToken = /passthrough token was detected/i.test(msg) ||
            ...
            auditLog.record({
                actor: (_p = account === null || account === void 0 ? void 0 : account.username) !== null && _p !== void 0 ? _p : "",
                action: "create_alias_subscription",
                target: aliasName,
                status: "failure",
                error: msg,
                details: {
                    ...
                    billingScope,
                    workload,
                    path: durationMs > POLL_PULSE_AFTER_MS ? "async-202-poll" : "sync-200",
                    durationMs,
    ...
    const handleCreateClick = React.useCallback(() => {
        if (!canSubmit)
            return;
        const needsConfirm = crossTenantRequested || preGrantRole || customScopeMode;
        if (needsConfirm) {
    ...
    const handleApplyRecentConfig = React.useCallback((cfg) => {
        var _a;
    ...
    const handleDeleteRecentConfig = React.useCallback((key) => {
    ...
    const handleRerunLastConfig = React.useCallback(() => {
        ...
        if (snap.customScopeMode) {
            ...
            setCustomScopeMode(false);
            if (snap.billingAccountName)
                setBillingAccountName(snap.billingAccountName);
    ...
    const exportColumns = React.useMemo(() => [
        ...
        { header: "subscriptionId", accessor: (r) => { var _a; return (_a = r.subscriptionId) !== null && _a !== void 0 ? _a : ""; } },
    ...
    const successCount = submittedAliases.filter((a) => a.provisioningState === "Succeeded").length;
    /* ----- Curl-snippet preview ------------------------------------ */
    const [showCurl, setShowCurl] = React.useState(false);
    const curlSnippet = React.useMemo(() => billingScope
        ? buildCurlSnippet({
            aliasName,
            displayName: applyDisplayNameTokens(displayName.trim(), {
                counter,
    ...
    if (candidates.length === 0) {
        return (React.createElement("div", { className: "flex flex-col gap-4" },
            React.createElement(PageHeader, { title: "Create EA Sub (alias API)", description: "Minimal flow that mirrors Microsoft's PowerShell example exactly." }),
            React.createElement(EmptyState, { icon: Crown, title: "No Azure account signed in", description: "Sign in with an account that holds EA Subscription Creator on the target enrollment account." })));
    }
    return (React.createElement("div", { className: "flex flex-col gap-4 py-2" },
        ...
        React.createElement("div", { className: "flex flex-wrap items-start justify-between gap-2" },
            React.createElement(PageHeader, { title: "Create EA Sub (alias API)", description: "Mirrors Microsoft's PowerShell snippet: list billing accounts \u2192 list enrollment accounts \u2192 PUT /providers/Microsoft.Subscription/aliases/{name} \u2192 poll until Succeeded." }),
        ...
        React.createElement(Alert, null,
            ...
            React.createElement(AlertDescription, { className: "text-2xs" },
                "Uses the modern",
                " ",
                React.createElement("code", { className: "font-mono" }, "2021-10-01"),
                " Subscription Alias API. No commerce-account dependency, works on every active EA. The existing",
                " ",
                React.createElement("a", { href: "#/ea-subscription", className: "font-medium text-primary underline-offset-2 hover:underline" }, "Create EA Sub (multi-recipient)"),
                " ",
                "page wraps the same API with a richer recipient batcher; pick that one when you want to provision a sub per user/SPN in one go.")),
        recentConfigs.length > 0 && (React.createElement(Card, { className: "border-dashed border-primary/40 bg-primary/5" },
            React.createElement(CardHeader, { className: "pb-2" },
                React.createElement(CardTitle, { className: "flex items-center gap-2 text-sm" },
                    ...
                    "Recently used",
                    React.createElement(Badge, { variant: "outline", className: "text-[9px]" },
                        recentConfigs.length,
                        "/",
                        RECENT_PRESETS_MAX)),
                React.createElement(CardDescription, { className: "text-2xs" }, "One-click rehydrate from your last few successful submissions. Cleared on browser data wipe \u2014 recipes only, no tokens or owner ids.")),
            React.createElement(CardContent, null,
                React.createElement("ul", { className: "flex flex-wrap gap-1.5", "aria-label": "Recently used configurations" }, recentConfigs.map((cfg) => (React.createElement("li", { key: cfg.key, className: "inline-flex items-center gap-0.5 rounded-full border border-border bg-background/60 pl-2 pr-0.5" },
                    React.createElement("button", { type: "button", onClick: () => handleApplyRecentConfig(cfg), className: "flex items-center gap-1 py-0.5 text-2xs hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", "aria-label": `Rehydrate form to ${cfg.displayNameTemplate} (${cfg.workload})`, title: `Last used ${new Date(cfg.lastUsedAt).toLocaleString()}` },
                        React.createElement("span", { className: "font-medium" }, cfg.displayNameTemplate),
                        React.createElement(Badge, { variant: "outline", className: "ml-1 text-[9px] font-mono" }, cfg.workload),
                        cfg.customScopeMode && (React.createElement(Badge, { variant: "outline", className: "text-[9px]" }, "custom"))),
                    React.createElement("button", { type: "button", onClick: () => handleDeleteRecentConfig(cfg.key), className: "ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full text-muted-foreground hover:bg-destructive/20 hover:text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", "aria-label": `Remove ${cfg.displayNameTemplate} from recent` },
                        React.createElement(Trash2, { className: "h-2.5 w-2.5" }))))))))),
        React.createElement("div", { className: "flex flex-wrap gap-2", role: "group", "aria-label": "EA Sub Quick summary" },
            React.createElement(SummaryStatItem, { label: "BAs visible", value: billingAccounts.length, tone: billingAccounts.length > 0 ? "info" : "muted", hint: baLoading ? "loading" : undefined, compact: true }),
            React.createElement(SummaryStatItem, { label: "EAs under BA", value: easUnderSelectedBaCount, tone: easUnderSelectedBaCount > 0 ? "info" : "muted", hint: accountIsEnterpriseAgreement
                    ? eaLoading
                        ? "loading"
                        : undefined
                    : "non-EA: N/A", compact: true }),
            React.createElement(SummaryStatItem, { label: "Subs this session", value: submittedAliases.length, tone: submittedAliases.length > 0 ? "success" : "muted", hint: submittedAliases.length > 0
                    ? `${successCount} succeeded`
                    : undefined, compact: true }),
            React.createElement(SummaryStatItem, { label: "Failed this session", value: submittedAliases.length - successCount, tone: submittedAliases.length - successCount > 0
                    ? "destructive"
                    : "muted", compact: true })),
        React.createElement(Card, null,
            React.createElement(CardHeader, { className: "pb-3" },
                React.createElement(CardTitle, { className: "flex items-center gap-2 text-sm" },
                    ...
                    "Billing scope",
                    React.createElement(InfoTooltip, { variant: "help", ariaLabel: "What is a billingScope", content: "The ARM resource id the alias API charges against. For Enterprise Agreement: /providers/Microsoft.Billing/billingAccounts/{ba}/enrollmentAccounts/{ea}. For MOSP / MCA / MPA: /providers/Microsoft.Billing/billingAccounts/{ba}. Paste a known scope below if the picker can't see your billing account (hasReadAccess: false)." })),
                React.createElement(CardDescription, null,
                    "Account \u2192 billing account \u2192 enrollment account. The resolved",
                    " ",
                    React.createElement("code", { className: "font-mono" }, "billingScope"),
                    " ARM path is shown at the bottom.")),
                ...
                React.createElement("div", { className: "grid grid-cols-1 gap-3 sm:grid-cols-3" },
                    ...
                    React.createElement("div", { className: "flex flex-col gap-1.5" },
                        ...
                        baLoading ? (React.createElement("p", { className: "flex items-center gap-2 text-xs text-muted-foreground" },
                            ...
                            " loading")) : baError ? (React.createElement(ErrorState, { size: "compact", message: "Failed to load billing accounts.", detail: baError })) : billingAccounts.length === 0 ? (React.createElement("div", { className: "flex flex-col gap-1.5 text-xs text-muted-foreground" },
                            React.createElement("p", null, "No EA billing accounts visible."),
                            React.createElement("button", { type: "button", onClick: () => setCustomScopeMode(true), className: "self-start text-primary underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" }, "Use custom billingScope instead \u2192"))) : (React.createElement(Select, { value: billingAccountName, onValueChange: setBillingAccountName },
                            React.createElement(SelectTrigger, null,
                                React.createElement(SelectValue, { placeholder: "Pick a billing account" })),
                            React.createElement(SelectContent, null, billingAccounts.map((b) => {
                                var _a;
                                return (React.createElement(SelectItem, { key: b.name, value: b.name },
                                    React.createElement("span", { className: "flex flex-col" },
                                        React.createElement("span", { className: "flex items-center gap-2 text-sm" },
                                            React.createElement("span", { className: "truncate" }, b.displayName),
                                            React.createElement(Badge, { variant: ((_a = b.agreementType) === null || _a === void 0 ? void 0 : _a.toLowerCase()) ===
                                                    "enterpriseagreement"
                                                    ? "default"
                                                    : "outline", className: "text-[9px]" }, b.agreementType || "Unknown")),
                                        React.createElement("span", { className: "font-mono text-2xs text-muted-foreground" }, b.name))));
                            }))))),
                    accountIsEnterpriseAgreement ? (React.createElement("div", { className: "flex flex-col gap-1.5" },
                        React.createElement(Label, { className: "text-xs" }, "Enrollment account"),
                        eaLoading ? (React.createElement("p", { className: "flex items-center gap-2 text-xs text-muted-foreground" },
                            React.createElement(Loader2, { className: "h-3 w-3 animate-spin motion-reduce:animate-none", "aria-hidden": true }),
                            " loading")) : eaError ? (React.createElement(ErrorState, { size: "compact", message: "Failed to load enrollment accounts.", detail: eaError })) : eas.length === 0 ? (React.createElement("p", { className: "text-xs text-muted-foreground" }, "No enrollment accounts visible \u2014 pick a different billing account or contact your EA admin.")) : (React.createElement(Select, { value: enrollmentAccountName, onValueChange: setEnrollmentAccountName },
                            React.createElement(SelectTrigger, null,
                            ...
                            React.createElement(SelectContent, null, eas.map((e) => (React.createElement(SelectItem, { key: e.name, value: e.name },
                                React.createElement("span", { className: "flex flex-col" },
                                    ...
                                    React.createElement("span", { className: "font-mono text-2xs text-muted-foreground" },
                                        e.name,
                                        e.accountOwner ? ` · ${e.accountOwner}` : "")))))))))) : selectedBa ? (React.createElement("div", { className: "flex flex-col gap-1.5" },
                        React.createElement(Label, { className: "text-xs" }, "Enrollment account"),
                        React.createElement(Alert, null,
                            React.createElement(Info, { className: "h-3.5 w-3.5" }),
                            React.createElement(AlertDescription, { className: "text-2xs" },
                                ...
                                " ",
                                React.createElement("code", { className: "font-mono" }, selectedBa.agreementType || "Unknown"),
                                " ",
                                "\u2014 no enrollment-accounts collection. We'll target the billing account scope directly (",
                                React.createElement("code", { className: "font-mono" }, "/providers/Microsoft.Billing/billingAccounts/{name}"),
                                "). The alias API accepts this for MOSP / MCA / MPA accounts; if your tenant requires a billing profile + invoice section (full MCA flow) instead, use the multi-recipient EA Sub page which exposes that picker.")))) : (React.createElement("div", null))),
                React.createElement("div", { className: "flex flex-col gap-2 rounded-md border border-dashed p-3 " +
                        (customScopeMode
                            ? "border-primary/50 bg-primary/5"
                            : "border-border/60 bg-muted/30") },
                    React.createElement("label", { className: "flex cursor-pointer items-start gap-2 text-xs", htmlFor: "ea-sub-quick-custom-scope-toggle" },
                        React.createElement("input", { id: "ea-sub-quick-custom-scope-toggle", type: "checkbox", checked: customScopeMode, onChange: (e) => setCustomScopeMode(e.target.checked), "aria-label": "Toggle custom billingScope (advanced)", className: "mt-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" }),
                        React.createElement("span", { className: "flex flex-col gap-0.5" },
                            React.createElement("span", { className: "flex items-center gap-1.5 font-medium" },
                                ...
                                React.createElement(InfoTooltip, { variant: "help", ariaLabel: "When to use custom billingScope", content: "Bypass the picker. Paste the BA + EA names you know, or a full pre-built billingScope ARM path. Useful when the parent billing account has hasReadAccess: false and doesn't surface in the dropdown above." })),
                            React.createElement("span", { className: "text-2xs text-muted-foreground" }, "Paste the EA billing account name + enrollment account name you know, OR a full pre-built billingScope ARM path. The alias API only needs the resulting string."))),
                    customScopeMode && (React.createElement("div", { className: "flex flex-col gap-2 pl-6" },
                        React.createElement("div", { className: "grid grid-cols-1 gap-2 sm:grid-cols-2" },
                            React.createElement("div", { className: "flex flex-col gap-1.5" },
                                React.createElement(Label, { className: "text-2xs" }, "EA billing account name"),
                                React.createElement(Input, { value: customBillingAccountName, onChange: (e) => setCustomBillingAccountName(e.target.value), placeholder: "e.g. 79562631", className: "font-mono text-2xs" })),
                            React.createElement("div", { className: "flex flex-col gap-1.5" },
                                React.createElement(Label, { className: "text-2xs" }, "Enrollment account name"),
                                React.createElement(Input, { value: customEnrollmentAccountName, onChange: (e) => setCustomEnrollmentAccountName(e.target.value), placeholder: "e.g. 405336", className: "font-mono text-2xs" }))),
                        React.createElement("details", null,
                            React.createElement("summary", { className: "cursor-pointer text-[10px] font-medium uppercase tracking-wide text-muted-foreground" }, "Or paste the full billingScope ARM path"),
                            React.createElement(Input, { value: customFullScope, onChange: (e) => setCustomFullScope(e.target.value), placeholder: "/providers/Microsoft.Billing/billingAccounts/.../enrollmentAccounts/...", className: "mt-2 font-mono text-[10px]" }),
                            React.createElement("p", { className: "mt-1 text-[10px] text-muted-foreground" }, "A full path here overrides the two fields above."))))),
                billingScope && (React.createElement("div", { className: "group/copy flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/30 px-2 py-1.5 text-2xs" },
                    React.createElement("span", { className: "text-muted-foreground" }, "billingScope"),
                    React.createElement("code", { className: "break-all font-mono" }, billingScope),
                    React.createElement(CopyButton, { value: billingScope, ariaLabel: "Copy billingScope ARM path", alwaysVisible: true, className: "ml-auto" }))),
                armToken && armTokenClaims && (React.createElement("details", { className: "rounded-md border border-border/60 bg-muted/20 px-2 py-1.5 text-2xs" },
                    React.createElement("summary", { className: "cursor-pointer font-medium text-muted-foreground" }, "Diagnostics \u2014 principal we'll send as"),
                    React.createElement("div", { className: "mt-2 flex flex-col gap-1.5" },
                        React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                            React.createElement("span", { className: "text-muted-foreground" }, "upn"),
                            React.createElement("code", { className: "font-mono" }, armTokenClaims.upn || "(none)"),
                            React.createElement(CopyButton, { value: armTokenClaims.upn, ariaLabel: "Copy UPN" })),
                        React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                            React.createElement("span", { className: "text-muted-foreground" }, "oid"),
                            React.createElement("code", { className: "font-mono" }, armTokenClaims.oid || "—"),
                            armTokenClaims.oid && (React.createElement(CopyButton, { value: armTokenClaims.oid, ariaLabel: "Copy object id" }))),
                        React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                            React.createElement("span", { className: "text-muted-foreground" }, "tid"),
                            React.createElement("code", { className: "font-mono" }, armTokenClaims.tid),
                            React.createElement(CopyButton, { value: armTokenClaims.tid, ariaLabel: "Copy tenant id" })),
                        React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                            React.createElement("span", { className: "text-muted-foreground" }, "aud"),
                            React.createElement("code", { className: "font-mono" }, armTokenClaims.aud)),
                        React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                            React.createElement(Button, { type: "button", variant: "ghost", size: "sm", className: "h-6 px-1.5 text-2xs", onClick: () => setShowTokenDiagnostics((s) => !s) },
                                showTokenDiagnostics ? (React.createElement(EyeOff, { className: "h-3 w-3" })) : (React.createElement(Eye, { className: "h-3 w-3" })),
                                showTokenDiagnostics ? "Hide raw" : "Show raw token"),
                            showTokenDiagnostics && (React.createElement(CopyButton, { value: armToken, ariaLabel: "Copy raw ARM token", alwaysVisible: true }))),
                        showTokenDiagnostics && (React.createElement("code", { className: "block break-all rounded bg-background/60 p-1.5 font-mono text-[10px] text-muted-foreground" }, armToken))))))),
        !billingScope ? (React.createElement(EmptyState, { icon: BadgeCheck, title: accountIsEnterpriseAgreement
                ? "Pick billing + enrollment"
                : "Pick a billing account", description: accountIsEnterpriseAgreement
                ? "Once both are selected the form below activates."
                : "Once the billing account is selected the form below activates. Non-EA accounts skip the enrollment-account step." })) : (React.createElement(React.Fragment, null,
            React.createElement(Card, null,
                React.createElement(CardHeader, { className: "pb-3" },
                    React.createElement(CardTitle, { className: "flex items-center gap-2 text-sm" },
                    ...
                    React.createElement(CardDescription, null,
                        "Body sent to",
                        " ",
                        React.createElement("code", { className: "font-mono" }, "PUT /providers/Microsoft.Subscription/aliases/{name}?api-version=2021-10-01"),
                        ".")),
                React.createElement(CardContent, { className: "flex flex-col gap-3" },
                    React.createElement("div", { className: "grid grid-cols-1 gap-3 sm:grid-cols-3" },
                        React.createElement("div", { className: "flex flex-col gap-1.5 sm:col-span-2" },
                            React.createElement(Label, { className: "flex items-center gap-1.5 text-xs" },
                                ...
                                React.createElement(InfoTooltip, { variant: "help", ariaLabel: "Display name tokens", content: "Supports tokens: {date}=YYYY-MM-DD, {time}=HH-MM, {counter}=001 (auto-increments after each submission this session), {user}=local part of your username. Tokens are resolved at submit time. 3\u201364 characters." })),
                            React.createElement("div", { className: "flex items-stretch gap-2" },
                                React.createElement(Input, { value: displayName, onChange: (e) => setDisplayName(e.target.value), className: "text-xs", placeholder: 'e.g. "Team X {date} {counter}"', "aria-label": "Subscription display name", "aria-invalid": displayName.trim().length > 0 &&
                                        (displayName.trim().length < 3 ||
                                            displayName.trim().length > 64)
                                        ? true
                                        : undefined }),
                                React.createElement(Button, { type: "button", variant: "outline", size: "sm", onClick: handleSavePreset, disabled: !displayName.trim() ||
                                        presets.includes(displayName.trim()), "aria-label": "Save display name template as preset" },
                                    React.createElement(Save, { className: "h-3.5 w-3.5" }),
                                    " Save preset")),
                            React.createElement("div", { className: "flex flex-wrap items-center justify-between gap-1.5 text-2xs" },
                                React.createElement("span", { className: "text-muted-foreground" },
                                    displayName.trim().length,
                                    "/64 chars",
                                    displayName.trim().length > 0 &&
                                        displayName.trim().length < 3 && (React.createElement("span", { className: "ml-1 text-destructive" }, "(need \u2265 3)")),
                                    displayName.trim().length > 64 && (React.createElement("span", { className: "ml-1 text-destructive" }, "(too long)"))),
                                (/\{date\}|\{time\}|\{counter\}|\{user\}/.test(displayName) ||
                                    counter > 1) && (React.createElement("span", { className: "text-muted-foreground" },
                                    "Will be sent as:",
                                    " ",
                                    React.createElement("code", { className: "font-mono" }, applyDisplayNameTokens(displayName.trim(), {
                                        counter,
                                        username: account === null || account === void 0 ? void 0 : account.username,
                                    }) || "—")))),
                            presets.length > 0 ? (React.createElement("div", { className: "flex flex-wrap items-center gap-1.5", role: "group", "aria-label": "Saved display-name presets" },
                                React.createElement("span", { className: "text-2xs text-muted-foreground" }, "Presets:"),
                                presets.map((p) => (React.createElement("span", { key: p, className: "group/preset inline-flex items-center gap-0.5 rounded-full border border-border bg-muted/30 pl-2 pr-0.5 text-2xs" },
                                    React.createElement("button", { type: "button", onClick: () => handleApplyPreset(p), className: "rounded-l-full py-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring hover:text-primary", "aria-label": `Apply preset ${p}`, title: `Apply: ${p}` }, p),
                                    React.createElement("button", { type: "button", onClick: () => handleDeletePreset(p), className: "ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full text-muted-foreground hover:bg-destructive/20 hover:text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", "aria-label": `Delete preset ${p}` },
                                        React.createElement(Trash2, { className: "h-2.5 w-2.5" }))))))) : (React.createElement("div", { className: "flex flex-wrap items-center gap-1.5", role: "group", "aria-label": "Starter display-name templates" },
                                React.createElement("span", { className: "text-2xs text-muted-foreground" }, "Try a template:"),
                                DEFAULT_PRESET_LIBRARY.map((p) => (React.createElement("button", { key: p, type: "button", onClick: () => setDisplayName(p), className: "rounded-full border border-dashed border-border/60 px-2 py-0.5 text-2xs text-muted-foreground hover:border-primary/50 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", "aria-label": `Use template ${p}`, title: `Use template: ${p}` }, p)))))),
                        React.createElement("div", { className: "flex flex-col gap-1.5" },
                            React.createElement(Label, { className: "flex items-center gap-1.5 text-xs" },
                                "Workload ",
                                React.createElement("span", { className: "text-destructive" }, "*"),
                                React.createElement(InfoTooltip, { variant: "help", ariaLabel: "Workload meaning", content: "Tags the subscription's commercial commitment. 'Production' = standard EA rate. 'DevTest' = Dev/Test pricing (only valid on EA Dev/Test enrollments \u2014 picking it on a Production enrollment is silently coerced)." })),
                            React.createElement(Select, { value: workload, onValueChange: (v) => setWorkload(v) },
                                React.createElement(SelectTrigger, { "aria-label": "Select subscription workload" },
                                    React.createElement(SelectValue, { placeholder: "Select workload" })),
                                React.createElement(SelectContent, null,
                                    React.createElement(SelectItem, { value: "Production" }, "Production"),
                                    React.createElement(SelectItem, { value: "DevTest" }, "DevTest"))))),
                    React.createElement("div", { className: "flex flex-col gap-1.5" },
                        React.createElement(Label, { className: "flex items-center gap-1.5 text-xs" },
                            "Alias name (URL segment) \u2014 must be unique",
                            React.createElement(InfoTooltip, { variant: "help", ariaLabel: "What is the alias name", content: "The URL segment for the alias resource itself (NOT the subscription's displayName). Must be unique within the tenant. Each successful create auto-rolls a new one so the next submit can't 409 on the same name." })),
                        React.createElement("div", { className: "flex items-stretch gap-2" },
                            React.createElement(Input, { value: aliasName, onChange: (e) => setAliasName(e.target.value), className: "font-mono text-xs", "aria-label": "Alias name URL segment" }),
                            React.createElement(Button, { type: "button", variant: "outline", size: "sm", onClick: regenAlias, "aria-label": "Re-roll a random alias name" },
                        ...
                        React.createElement("p", { className: "text-2xs text-muted-foreground" },
                            ...
                            " ",
                            React.createElement("code", { className: "font-mono" }, "\"ea-sub-\" + Get-Random"),
                    ...
                    React.createElement("details", { className: "rounded-md border border-border/60 bg-muted/30 p-3" },
                        React.createElement("summary", { className: "cursor-pointer text-2xs font-medium uppercase tracking-wide text-muted-foreground" }, "Cross-tenant owner & tags (additionalProperties)"),
                        React.createElement("div", { className: "mt-3 flex flex-col gap-3" },
                            React.createElement("div", { className: "grid grid-cols-1 gap-2 sm:grid-cols-2" },
                                React.createElement("div", { className: "flex flex-col gap-1.5" },
                                    React.createElement(Label, { className: "flex items-center gap-1.5 text-xs" },
                                        "subscriptionTenantId",
                                        React.createElement(InfoTooltip, { variant: "help", ariaLabel: "subscriptionTenantId meaning", content: "Destination tenant ID. When set, the new sub lands in that tenant and the named owner has 7 days to accept. Must be a tenant GUID \u2014 pair with subscriptionOwnerId." })),
                                    React.createElement(Input, { value: subscriptionTenantId, onChange: (e) => setSubscriptionTenantId(e.target.value), placeholder: "11111111-2222-3333-4444-555555555555", className: "font-mono text-xs", "aria-invalid": subscriptionTenantId.length > 0 &&
                                            !UUID_RE.test(subscriptionTenantId.trim())
                                            ? true
                                            : undefined })),
                                React.createElement("div", { className: "flex flex-col gap-1.5" },
                                    React.createElement(Label, { className: "flex items-center gap-1.5 text-xs" },
                                        "subscriptionOwnerId",
                                        React.createElement(InfoTooltip, { variant: "help", ariaLabel: "subscriptionOwnerId meaning", content: "AAD object ID (GUID) of the user / SPN that will receive Owner rights in the destination tenant. Required whenever subscriptionTenantId is set." })),
                                    React.createElement(Input, { value: subscriptionOwnerId, onChange: (e) => setSubscriptionOwnerId(e.target.value), placeholder: "Object ID of user / SPN", className: "font-mono text-xs", "aria-invalid": subscriptionTenantId.length > 0 &&
                                            !UUID_RE.test(subscriptionOwnerId.trim())
                                            ? true
                                            : undefined }))),
                            React.createElement("p", { className: "text-2xs text-muted-foreground" }, "Provide both fields to land the new sub in a different tenant. Empty \u2192 the sub lands in the calling principal's home tenant."),
                            !crossTenantValid && (React.createElement(Alert, { variant: "destructive" },
                                React.createElement(AlertTriangle, { className: "h-3.5 w-3.5" }),
                                React.createElement(AlertDescription, { className: "text-2xs" }, "Cross-tenant landing needs BOTH a valid destination tenant id AND a valid owner object id."))),
                            React.createElement("div", { className: "flex flex-col gap-1.5" },
                                React.createElement(Label, { className: "text-xs" }, "Tags"),
                                React.createElement("ul", { className: "flex flex-col gap-1" }, tagPairs.map((p, i) => (React.createElement("li", { key: i, className: "flex items-center gap-2" },
                                    React.createElement(Input, { value: p.key, onChange: (e) => setTagAt(i, "key", e.target.value), placeholder: "key", className: "font-mono text-2xs" }),
                                    React.createElement(Input, { value: p.value, onChange: (e) => setTagAt(i, "value", e.target.value), placeholder: "value", className: "font-mono text-2xs" }),
                                    React.createElement(Button, { type: "button", variant: "ghost", size: "icon-sm", onClick: () => removeTagAt(i), "aria-label": `Remove tag ${p.key}` },
                                        React.createElement(Trash2, { className: "h-3 w-3" })))))),
                                React.createElement("div", null,
                                    React.createElement(Button, { type: "button", variant: "outline", size: "sm", className: "h-7 text-2xs", onClick: addTagRow },
                                        React.createElement(Plus, { className: "h-3 w-3" }),
                                        " Add tag"))))),
                    crossesTenantBoundary && (React.createElement(Alert, { variant: risk.privilegedCombo ? "destructive" : "warning" },
                        risk.privilegedCombo ? (React.createElement(Skull, { className: "h-3.5 w-3.5" })) : (React.createElement(Globe, { className: "h-3.5 w-3.5" })),
                        React.createElement(AlertDescription, { className: "flex flex-col gap-1.5 text-2xs" },
                            React.createElement("span", null,
                                React.createElement("strong", null, risk.privilegedCombo
                            ...
                            React.createElement("span", null,
                                ...
                                " ",
                                React.createElement("em", null, "billed in your tenant"),
                                " ",
                                React.createElement("code", { className: "font-mono" }, (_c = account === null || account === void 0 ? void 0 : account.tenantId.slice(0, 8)) !== null && _c !== void 0 ? _c : "",
                                    "\u2026"),
                                " ",
                                "but ",
                                React.createElement("em", null, "owned in destination tenant"),
                                " ",
                                React.createElement("code", { className: "font-mono" },
                                    subscriptionTenantId.trim().slice(0, 8),
                                    "\u2026"),
                                ". The audit log row lands on YOUR tenant only \u2014 but the destination tenant's directory immediately sees a new Microsoft.Resources/subscriptions item with",
                                " ",
                                React.createElement("code", { className: "font-mono" }, subscriptionOwnerId.trim().slice(0, 8) || "?"),
                                " ",
                                "as Owner."),
                            risk.privilegedCombo && (React.createElement("span", null,
                                React.createElement("strong", null, "Privileged combination detected."),
                                " ",
                                "You're pre-granting a billing role on the source enrollment AND landing the sub in a foreign tenant. This is the kill-chain step documented in",
                                " ",
                                React.createElement("code", { className: "font-mono" }, "_bypass_mixed_chains.md"),
                                " ",
                                "Chain 7 (cross-tenant subscription drop with owner grant). Confirm this is an authorized provisioning step.")),
                            React.createElement("span", { className: "text-[10px]" },
                                "Reference:",
                                " ",
                                React.createElement("code", { className: "font-mono" }, "_ea_subscription_cross_tenant.md \u00A77 step 6"),
                    ...
                    React.createElement("details", { className: "rounded-md border border-border/60 bg-muted/20 px-2 py-1.5", onToggle: (e) => setShowJsonPreview(e.target.open) },
                        React.createElement("summary", { className: "flex cursor-pointer items-center gap-1.5 text-2xs font-medium uppercase tracking-wide text-muted-foreground" },
                            React.createElement(Braces, { className: "h-3 w-3" }),
                            " Show JSON body preview",
                            React.createElement(InfoTooltip, { variant: "help", ariaLabel: "What is the JSON body preview", content: "Pretty-printed copy of the JSON body the alias-create PUT will send. Identical envelope, no token material. Use it to diff against the published Microsoft docs example or the body captured in the audit log." })),
                        showJsonPreview && jsonBodyPreview && (React.createElement("div", { className: "group/copy mt-2 flex flex-col gap-1.5" },
                            React.createElement("pre", { className: "max-h-72 overflow-auto rounded bg-background/80 p-2 font-mono text-[10px] leading-relaxed" }, jsonBodyPreview),
                            React.createElement("div", { className: "flex justify-end" },
                                React.createElement(CopyButton, { value: jsonBodyPreview, ariaLabel: "Copy JSON body preview", alwaysVisible: true }))))),
                    React.createElement("details", { className: "rounded-md border border-border/60 bg-muted/20 px-2 py-1.5", onToggle: (e) => setShowCurl(e.target.open) },
                        React.createElement("summary", { className: "flex cursor-pointer items-center gap-1.5 text-2xs font-medium uppercase tracking-wide text-muted-foreground" },
                            React.createElement(Terminal, { className: "h-3 w-3" }),
                            " Show curl snippet",
                            React.createElement(InfoTooltip, { variant: "help", ariaLabel: "What is the curl snippet for", content: "Reproduction of the alias-create REST call. Paste into a shell with a real bearer token in place of {TOKEN} to test independently of the UI." })),
                        showCurl && curlSnippet && (React.createElement("div", { className: "group/copy mt-2 flex flex-col gap-1.5" },
                            React.createElement("pre", { className: "overflow-x-auto rounded bg-background/80 p-2 font-mono text-[10px] leading-relaxed" }, curlSnippet),
                            React.createElement("div", { className: "flex justify-end" },
                                React.createElement(CopyButton, { value: curlSnippet, ariaLabel: "Copy curl reproduction snippet", alwaysVisible: true }))))),
                    submitError && passthroughToken && (React.createElement(Alert, { variant: "destructive" },
                        React.createElement(AlertTriangle, { className: "h-3.5 w-3.5" }),
                        React.createElement(AlertDescription, { className: "flex flex-col gap-2" },
                            React.createElement("span", null,
                                React.createElement("strong", null, "ARM rejected the token with \"passthrough token detected without proper resource provider context\"."),
                                " ",
                                "Translation: the token reached the Microsoft.Subscription resource provider but its",
                                " ",
                                React.createElement("code", { className: "font-mono" }, "tid"),
                                " claim doesn't match a tenant the RP accepts for this enrollment. This is",
                                " ",
                                React.createElement("strong", null, "not a missing-role error"),
                                " \u2014 the role might be granted, but the token can't be validated."),
                            React.createElement("span", { className: "text-2xs" },
                                "Typical causes: (1) an",
                                " ",
                                React.createElement("strong", null, "imported / portal-paste token"),
                                " ",
                                "whose audience is fine but tenant context isn't what the RP expects; (2) the account is signed in against its ",
                                React.createElement("em", null, "home"),
                                " tenant while the EA enrollment lives in a ",
                                React.createElement("em", null, "different"),
                                " tenant; (3) a stale token issued before a recent role grant (propagation can take up to 5 min)."),
                            React.createElement("span", { className: "text-2xs" },
                                "Fixes, in order: (a) click",
                                " ",
                                React.createElement("strong", null, "Re-acquire token"),
                                " below to force a fresh ARM token (bypasses MSAL silent cache and any imported-token short-circuit); (b) if you pasted a portal token, re-paste a fresh one from the EA tenant; (c) sign out + sign in on Azure Accounts using the tenant that hosts the enrollment (use the inline tenant switcher)."),
                            React.createElement("div", { className: "flex flex-wrap gap-2" },
                                React.createElement(Button, { type: "button", size: "sm", variant: "default", className: "h-7 text-2xs", onClick: () => void handleReacquireToken(), loading: reacquiring, "aria-label": "Re-acquire ARM token (force refresh)" },
                                    !reacquiring && (React.createElement(RefreshCw, { className: "h-3 w-3" })),
                                    "Re-acquire token"),
                                React.createElement(Button, { type: "button", size: "sm", variant: "outline", className: "h-7 text-2xs", onClick: () => goTo("/azure-accounts"), "aria-label": "Open Azure Accounts to switch tenant" }, "Open Azure Accounts"),
                                React.createElement(Button, { type: "button", size: "sm", variant: "outline", className: "h-7 text-2xs", onClick: () => goTo("/token-importer"), "aria-label": "Open Token Importer to paste a fresh token" }, "Open Token Importer")),
                            React.createElement("span", { className: "break-all text-[10px] opacity-80" },
                                "Raw error: ",
                                submitError)))),
                    submitError && missingRole && (React.createElement(Alert, { variant: "destructive" },
                        React.createElement(AlertTriangle, { className: "h-3.5 w-3.5" }),
                        React.createElement(AlertDescription, { className: "flex flex-col gap-2" },
                            React.createElement("span", null,
                                React.createElement("strong", null, "AAD: the signed-in principal isn't allowed to create subscriptions on this enrollment account."),
                                " ",
                                "The modern alias API checks a specific billing-role assignment \u2014 being an \"Account Owner\" in the EA portal is",
                                " ",
                            ...
                            React.createElement("span", { className: "text-2xs" },
                                ...
                                " ",
                                React.createElement("strong", null, "EA Subscription Creator"),
                                " ",
                                "(definition GUID",
                                " ",
                                React.createElement("code", { className: "font-mono" }, "a0bcee42-bf30-4d1b-926a-48d21664ef71"),
                                ") assigned to your AAD principal at scope",
                                " ",
                                React.createElement("code", { className: "break-all font-mono" }, billingScope),
                                ". An Enterprise Administrator (or another EA Subscription Creator) on the parent billing account can grant it."),
                            React.createElement("div", { className: "flex flex-wrap gap-2" },
                                React.createElement(Button, { type: "button", size: "sm", variant: "default", className: "h-7 text-2xs", onClick: () => setConfirmDialog({ kind: "navigate-sub-manager" }) }, "Open Sub Manager \u2192 Grant Subscription Creator"),
                                React.createElement(Button, { type: "button", size: "sm", variant: "outline", className: "h-7 text-2xs", asChild: true },
                                    React.createElement("a", { href: "https://ea.azure.com", target: "_blank", rel: "noopener noreferrer" }, "Open ea.azure.com"))),
                            React.createElement("span", { className: "text-[10px] text-muted-foreground" },
                                "CLI equivalent (an admin runs it):",
                                " ",
                                React.createElement("code", { className: "break-all font-mono" }, `az rest -m PUT --url "https://management.azure.com${billingScope}/billingRoleAssignments/$(uuidgen)?api-version=2024-04-01" --body '{"properties":{"principalId":"<YOUR_OBJECT_ID>","principalTenantId":"<YOUR_TENANT_ID>","roleDefinitionId":"/providers/Microsoft.Billing/billingAccounts/<BA_NAME>/billingRoleDefinitions/a0bcee42-bf30-4d1b-926a-48d21664ef71"}}'`)),
                            React.createElement("span", { className: "break-all text-[10px] opacity-80" },
                                "Raw error: ",
                                submitError)))),
                    submitError && !missingRole && !passthroughToken && (React.createElement(Alert, { variant: "destructive" },
                        React.createElement(AlertTriangle, { className: "h-3.5 w-3.5" }),
                        React.createElement(AlertDescription, null, submitError))),
                    armToken && noOidDetected && (React.createElement(Alert, { variant: "warning" },
                        React.createElement(AlertTriangle, { className: "h-3.5 w-3.5" }),
                        React.createElement(AlertDescription, { className: "flex flex-col gap-1.5 text-xs" },
                            React.createElement("span", null,
                                React.createElement("strong", null,
                                    "The ARM token has no ",
                                    React.createElement("code", { className: "font-mono" }, "oid"),
                                    " claim."),
                                " ",
                                "That means the signed-in principal",
                                " ",
                                React.createElement("code", { className: "font-mono" }, (armTokenClaims === null || armTokenClaims === void 0 ? void 0 : armTokenClaims.upn) || (account === null || account === void 0 ? void 0 : account.username) || ""),
                                " ",
                                "is not a member of tenant",
                                " ",
                                React.createElement("code", { className: "font-mono" }, armTokenClaims === null || armTokenClaims === void 0 ? void 0 : armTokenClaims.tid),
                                "\u2014 they have no object id IN that tenant, so the Subscription RP rejects the call (and nothing can grant a role to a missing principal)."),
                            React.createElement("span", { className: "text-2xs" },
                                React.createElement("strong", null, "Fix:"),
                                " on Azure Accounts, switch the tenant for this account back to the principal's HOME tenant (the one where they're a member, not a guest). If the EA enrollment lives in a different tenant and you still need this principal to create subs there, an Entra admin in the EA tenant needs to invite",
                                " ",
                                React.createElement("code", { className: "font-mono" }, (armTokenClaims === null || armTokenClaims === void 0 ? void 0 : armTokenClaims.upn) || ""),
                                " ",
                                "as a guest first \u2014 once accepted, AAD provisions an oid for the guest and a fresh token will carry it.")))),
                    React.createElement("div", { className: "flex flex-col gap-1.5 rounded-md border border-border bg-card/40 p-2.5" },
                        React.createElement("label", { className: "flex items-start gap-2 text-xs" },
                            React.createElement(Checkbox, { checked: preGrantRole, onCheckedChange: (v) => setPreGrantRole(v === true), "aria-label": "Pre-grant EA Subscription Creator role before creating the subscription", className: "mt-0.5" }),
                            React.createElement("span", { className: "flex flex-col gap-0.5" },
                                React.createElement("span", { className: "font-semibold text-foreground" }, "Pre-grant EA Subscription Creator role first"),
                                React.createElement("span", { className: "text-2xs text-muted-foreground" }, "PUTs the role assignment at the enrollment-account scope for this principal BEFORE the alias-create call, so the create can't fail with \"not authorized\". Idempotent \u2014 re-running is safe; 409 / \"already exists\" is treated as success."))),
                        preGrantRole && armTokenClaims && (React.createElement("div", { className: "ml-6 flex flex-wrap items-center gap-1.5 text-2xs" },
                            React.createElement("span", { className: "text-muted-foreground" }, "Will grant to:"),
                            React.createElement("code", { className: "font-mono" }, armTokenClaims.oid || "(none — see warning above)"),
                            React.createElement("span", { className: "text-muted-foreground" }, "in tenant"),
                            React.createElement("code", { className: "font-mono" }, armTokenClaims.tid))),
                        preGrantStatus === "granting" && (React.createElement("div", { className: "ml-6 flex items-center gap-1.5 text-2xs text-info" },
                            React.createElement(Loader2, { className: "h-3 w-3 animate-spin motion-reduce:animate-none" }),
                            "Granting EA Subscription Creator\u2026")),
                        preGrantStatus === "granted" && (React.createElement("div", { className: "ml-6 flex items-center gap-1.5 text-2xs text-success" },
                            React.createElement(CheckCircle2, { className: "h-3 w-3" }),
                            "Role granted. Proceeding with alias create\u2026")),
                        preGrantStatus === "already-granted" && (React.createElement("div", { className: "ml-6 flex items-center gap-1.5 text-2xs text-success" },
                            React.createElement(CheckCircle2, { className: "h-3 w-3" }),
                            "Role was already assigned (no-op). Proceeding with alias create\u2026")),
                        preGrantStatus === "failed" && preGrantError && (React.createElement("div", { className: "ml-6 flex items-start gap-1.5 text-2xs text-destructive" },
                            React.createElement(AlertTriangle, { className: "mt-0.5 h-3 w-3 shrink-0" }),
                            React.createElement("span", { className: "min-w-0 break-words" }, preGrantError)))),
                    React.createElement("div", { className: "flex flex-col gap-1.5 rounded-md border border-border bg-muted/20 p-2.5", role: "region", "aria-label": "Preflight checklist", "aria-live": "polite" },
                        React.createElement("div", { className: "flex items-center gap-1.5 text-2xs font-medium uppercase tracking-wide text-muted-foreground" },
                            React.createElement(ShieldCheck, { className: "h-3 w-3" }),
                            "Preflight",
                            preflightBlocking === 0 ? (React.createElement(Badge, { variant: "success", className: "text-[9px]" }, "ready")) : (React.createElement(Badge, { variant: "destructive", className: "text-[9px]" },
                                preflightBlocking,
                                " blocking"))),
                        React.createElement("ul", { className: "flex flex-wrap gap-1.5" }, preflight.map((row) => (React.createElement("li", { key: row.label, className: "inline-flex items-center gap-1 rounded-full border border-border/60 bg-background/60 px-2 py-0.5 text-2xs", "aria-label": `${row.label}: ${row.ok ? "ok" : "blocked"}${row.hint ? ` (${row.hint})` : ""}` },
                            row.ok ? (React.createElement(Check, { className: row.tone === "success"
                                    ? "h-3 w-3 text-success"
                                    : "h-3 w-3 text-warning", "aria-hidden": true })) : row.tone === "muted" ? (React.createElement(Info, { className: "h-3 w-3 text-muted-foreground", "aria-hidden": true })) : (React.createElement(XCircle, { className: "h-3 w-3 text-destructive", "aria-hidden": true })),
                            React.createElement("span", { className: "font-medium" }, row.label),
                            row.hint && (React.createElement("span", { className: "font-mono text-[10px] text-muted-foreground" }, row.hint)))))),
                        React.createElement("p", { className: "text-[10px] text-muted-foreground" }, "Local checks only \u2014 Azure-side quota & role enforcement run on submit.")),
                    React.createElement("div", { className: "flex flex-wrap items-center gap-3" },
                        React.createElement(Button, { type: "button", variant: "default", onClick: handleCreateClick, disabled: !canSubmit, loading: submitting, "aria-label": submitting
                                ? "Creating EA subscription, polling alias"
                                : "Create EA subscription", "aria-keyshortcuts": "Control+Enter Meta+Enter", title: `Create subscription (${modKeyLabel()}+Enter)` },
                        ...
                        React.createElement("span", { className: "inline-flex items-center gap-1 text-2xs text-muted-foreground", "aria-hidden": true },
                            ...
                            React.createElement("kbd", { className: "rounded border border-border/60 bg-muted/40 px-1 font-mono text-[10px]" },
                                modKeyLabel(),
                                "+Enter")),
                        submitting && (React.createElement("div", { className: "flex items-center gap-2 text-2xs text-muted-foreground", role: "status", "aria-live": "polite" },
                            React.createElement("span", null,
                                "Elapsed: ",
                                (submitElapsedMs / 1000).toFixed(1),
                                "s"),
                            showPollingPulse && (React.createElement("span", { className: "flex items-center gap-1 text-info" },
                                React.createElement(Loader2, { className: "h-3 w-3 animate-spin motion-reduce:animate-none" }),
                                "Azure accepted (202) \u2014 polling alias until terminal state. EA provisioning can take 1\u20133 minutes; the page is doing the work, don't navigate away."))))))),
            result && (React.createElement(Card, { className: result.provisioningState.toLowerCase() === "succeeded"
                    ? "border-success/30 bg-success/5"
                    : "border-warning/30 bg-warning/5" },
                React.createElement(CardHeader, { className: "pb-2" },
                    React.createElement(CardTitle, { className: "flex items-center gap-2 text-sm" },
                        result.provisioningState.toLowerCase() === "succeeded" ? (React.createElement(Check, { className: "h-4 w-4 text-success" })) : (React.createElement(Loader2, { className: "h-4 w-4 animate-spin text-warning motion-reduce:animate-none" })),
                        "Alias result \u2014",
                        " ",
                        React.createElement("code", { className: "font-mono" }, result.provisioningState)),
                    React.createElement(CardDescription, { className: "text-2xs" },
                        ...
                        "via",
                        " ",
                        React.createElement("code", { className: "font-mono" }, result.polled ? "202 → poll" : "200 (sync)"),
                        ". ",
                        React.createElement("strong", null, "Succeeded"),
                        " = the subscription is live; anything else = Azure is still working on it, re-poll the alias URL or refresh",
                        " ",
                        React.createElement("a", { href: "https://portal.azure.com/#@/blade/Microsoft_Azure_Billing/EnrollmentBlade", target: "_blank", rel: "noopener noreferrer", className: "underline-offset-2 hover:underline" }, "portal.azure.com"),
                        " ",
                        "in a minute.")),
                React.createElement(CardContent, { className: "flex flex-col gap-2 text-xs" },
                    React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                        React.createElement(Badge, { variant: "outline", className: "text-2xs" }, "alias"),
                        React.createElement(CopyableText, { value: result.aliasName, mono: true, ariaLabel: "Copy alias name", alwaysVisibleButton: true })),
                    React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                        React.createElement(Badge, { variant: "outline", className: "text-2xs" }, "displayName"),
                        React.createElement("span", { className: "font-medium" }, result.displayName)),
                    result.subscriptionId && (React.createElement("div", { className: "group/copy flex flex-wrap items-center gap-2" },
                        React.createElement(Badge, { variant: "success", className: "text-2xs" }, "subscription"),
                        React.createElement("code", { className: "break-all font-mono" }, result.subscriptionId),
                        React.createElement(CopyButton, { value: result.subscriptionId, ariaLabel: "Copy subscription id", alwaysVisible: true }),
                        React.createElement("a", { href: `https://portal.azure.com/#@/resource/subscriptions/${result.subscriptionId}`, target: "_blank", rel: "noopener noreferrer", className: "inline-flex items-center gap-1 rounded text-primary underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", "aria-label": "Open subscription in Azure portal" },
                    ...
                    React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                        React.createElement(Button, { type: "button", variant: "outline", size: "sm", onClick: () => {
                            ...
                            " Create another"),
                        lastSubmittedSnapshot && (React.createElement(Button, { type: "button", variant: "outline", size: "sm", onClick: handleRerunLastConfig, "aria-label": "Re-run with same config (press r anywhere)", title: `Re-run with same config (press "r" anywhere on the page)` },
                            React.createElement(RotateCcw, { className: "h-3.5 w-3.5" }),
                            " Re-run same config",
                            React.createElement("kbd", { className: "ml-1 hidden rounded border border-border/60 bg-muted/40 px-1 font-mono text-[10px] sm:inline" }, "r"))))))),
            result && result.subscriptionId && showAttackSurface && (React.createElement(Card, { className: "border-info/30 bg-info/5" },
                React.createElement(CardHeader, { className: "pb-2 flex flex-row items-start justify-between gap-2" },
                    React.createElement("div", null,
                        React.createElement(CardTitle, { className: "flex items-center gap-2 text-sm" },
                            React.createElement(ShieldAlert, { className: "h-4 w-4 text-info" }),
                            "What this subscription now exposes"),
                        React.createElement(CardDescription, { className: "text-2xs" },
                            "Read-only summary of the RBAC / visibility surface the new sub created. Cross-tenant landings expose more than same-tenant ones \u2014 see the destination-tenant row. Source: corpus",
                            " ",
                            React.createElement("code", { className: "font-mono" }, "_ea_subscription_cross_tenant.md \u00A77 step 6"),
                            ".")),
                    React.createElement(Button, { type: "button", variant: "ghost", size: "sm", className: "h-6 w-6 p-0", onClick: () => setShowAttackSurface(false), "aria-label": "Dismiss attack-surface card", title: "Dismiss" },
                        React.createElement(XCircle, { className: "h-3.5 w-3.5" }))),
                React.createElement(CardContent, { className: "flex flex-col gap-2 text-2xs" },
                    React.createElement("div", { className: "flex items-start gap-2" },
                        React.createElement(Users, { className: "mt-0.5 h-3 w-3 shrink-0 text-info" }),
                        React.createElement("div", { className: "flex min-w-0 flex-col" },
                            React.createElement("span", { className: "font-medium text-foreground" }, "Owner (Owner RBAC, full control)"),
                            React.createElement("span", { className: "break-all" }, subscriptionOwnerId.trim() ? (React.createElement(React.Fragment, null,
                                "principal",
                                " ",
                                React.createElement("code", { className: "font-mono" }, subscriptionOwnerId.trim()),
                                " ",
                                "in tenant",
                                " ",
                                React.createElement("code", { className: "font-mono" }, subscriptionTenantId.trim() ||
                                    (account === null || account === void 0 ? void 0 : account.tenantId) ||
                                ...
                                React.createElement("code", { className: "font-mono" }, (armTokenClaims === null || armTokenClaims === void 0 ? void 0 : armTokenClaims.upn) || (account === null || account === void 0 ? void 0 : account.username) || "?"),
                                " ",
                                "in source tenant \u2014 alias-create makes the caller the default Owner when no override is set."))))),
                    React.createElement("div", { className: "flex items-start gap-2" },
                        React.createElement(FileText, { className: "mt-0.5 h-3 w-3 shrink-0 text-info" }),
                        React.createElement("div", { className: "flex min-w-0 flex-col" },
                            React.createElement("span", { className: "font-medium text-foreground" }, "Billing scope (source-tenant audit row)"),
                            React.createElement("span", { className: "break-all" },
                                "Charges flow to",
                                " ",
                                React.createElement("code", { className: "font-mono" }, billingScope),
                                ". Your source tenant",
                                " ",
                                React.createElement("code", { className: "font-mono" }, (account === null || account === void 0 ? void 0 : account.tenantId) || "?"),
                                " ",
                                "shows the alias-create event in audit; the destination does not see WHO ordered the sub, only the resulting Owner."))),
                    crossesTenantBoundary && (React.createElement(React.Fragment, null,
                        React.createElement("div", { className: "flex items-start gap-2" },
                            React.createElement(Globe, { className: "mt-0.5 h-3 w-3 shrink-0 text-warning" }),
                            React.createElement("div", { className: "flex min-w-0 flex-col" },
                                React.createElement("span", { className: "font-medium text-foreground" }, "Destination tenant directory now shows this sub"),
                                React.createElement("span", null,
                                    "Any Reader / Directory Reader / Global Reader in tenant",
                                    " ",
                                    React.createElement("code", { className: "font-mono" }, subscriptionTenantId.trim()),
                                    " ",
                                    "can enumerate",
                                    ...
                                    " ",
                                    React.createElement("code", { className: "font-mono" },
                                        "az account list --refresh --query \"[?id=='",
                                        result.subscriptionId,
                                        "']\""),
                                    "."))),
                        React.createElement("div", { className: "flex items-start gap-2" },
                            React.createElement(AlertTriangle, { className: "mt-0.5 h-3 w-3 shrink-0 text-warning" }),
                            React.createElement("div", { className: "flex min-w-0 flex-col" },
                                React.createElement("span", { className: "font-medium text-foreground" }, "Source-tenant RBAC: NONE by default"),
                                React.createElement("span", null,
                                    "You (the billing creator) do not get Owner RBAC on a sub that landed in a foreign tenant \u2014 the destination Owner does. To regain admin access from the source tenant later, you'd need a tenant transfer or a B2B guest invite into the destination tenant first. See",
                                    " ",
                                    React.createElement("code", { className: "font-mono" }, "_ea_subscription_cross_tenant.md \u00A72"),
                                    "."))))),
                    React.createElement("div", { className: "mt-1 flex flex-wrap items-center gap-1.5 text-[10px] text-muted-foreground" },
                        React.createElement(Badge, { variant: "outline", className: "text-[9px]" }, "informational"),
                        React.createElement("span", null,
                            "This card is local-only enumeration of what we already know from the form; it does NOT query the destination tenant. The audit-log row",
                            " ",
                            React.createElement("code", { className: "font-mono" }, "create_alias_subscription"),
                            " ",
                            "has the canonical record."))))))),
        submittedAliases.length > 0 && (React.createElement(Card, null,
            React.createElement(CardHeader, { className: "flex flex-row items-center justify-between gap-2 pb-2" },
                React.createElement("div", null,
                    React.createElement(CardTitle, { className: "flex items-center gap-2 text-sm" },
                        React.createElement(History, { className: "h-4 w-4 text-primary" }),
                        "Submitted this session (",
                        submittedAliases.length,
                        ")",
                        submittedAliases.length === HISTORY_MAX_ROWS && (React.createElement(Badge, { variant: "outline", className: "text-[9px]" }, "cap reached"))),
                    React.createElement(CardDescription, { className: "text-2xs" },
                        "In-memory log of aliases PUT during this page session. Cleared on reload \u2014 the audit log keeps the canonical record. Capped at ",
                        HISTORY_MAX_ROWS,
                        " rows.")),
                React.createElement("div", { className: "flex items-center gap-1.5" },
                    React.createElement(ExportMenu, { rows: submittedAliases, columns: exportColumns, filename: "ea-sub-quick-submitted", jsonMetadata: {
                            page: "ea-sub-quick",
                            actor: (_d = account === null || account === void 0 ? void 0 : account.username) !== null && _d !== void 0 ? _d : "",
                        }, label: "Export" }),
                    React.createElement(Button, { type: "button", variant: "ghost", size: "sm", className: "h-7 text-2xs", onClick: () => setConfirmDialog({ kind: "clear-history" }), "aria-label": "Clear submitted history" },
                        React.createElement(Trash2, { className: "h-3 w-3" }),
                        " Clear"))),
            React.createElement(CardContent, { className: "flex flex-col gap-1.5" }, submittedAliases.map((row) => (React.createElement("div", { key: `${row.aliasName}-${row.submittedAt}`, className: "group/copy flex flex-wrap items-center gap-2 rounded-md border border-border/60 bg-muted/20 px-2 py-1.5 text-2xs" },
                React.createElement(Badge, { variant: row.provisioningState === "Succeeded"
                        ? "success"
                        : row.provisioningState.toLowerCase() === "failed"
                            ? "destructive"
                            : "outline", className: "text-[9px]" },
                    row.provisioningState === "Succeeded" ? (React.createElement(Check, { className: "mr-0.5 h-2.5 w-2.5" })) : row.provisioningState.toLowerCase() === "failed" ? (React.createElement(XCircle, { className: "mr-0.5 h-2.5 w-2.5" })) : (React.createElement(Loader2, { className: "mr-0.5 h-2.5 w-2.5 animate-spin motion-reduce:animate-none" })),
                    row.provisioningState),
                row.crossTenant && (React.createElement(Badge, { variant: "outline", className: "text-[9px]" }, "cross-tenant")),
                React.createElement("span", { className: "font-medium" }, row.displayName),
                React.createElement(CopyableText, { value: row.aliasName, mono: true, ariaLabel: "Copy alias name" }),
                row.subscriptionId && (React.createElement(CopyableText, { value: row.subscriptionId, mono: true, ariaLabel: "Copy subscription id" })),
                React.createElement("span", { className: "text-muted-foreground" },
                    (row.durationMs / 1000).toFixed(1),
                    "s",
                    row.polled ? " (polled)" : ""),
                React.createElement("span", { className: "ml-auto text-muted-foreground" }, new Date(row.submittedAt).toLocaleTimeString()),
                row.subscriptionId && (React.createElement("a", { href: `https://portal.azure.com/#@/resource/subscriptions/${row.subscriptionId}`, target: "_blank", rel: "noopener noreferrer", className: "inline-flex items-center gap-1 text-primary underline-offset-2 hover:underline", "aria-label": "Open subscription in Azure portal", title: "Open in Azure portal" },
                    React.createElement(ExternalLink, { className: "h-3 w-3" }))))))))),
        React.createElement(Card, { className: "border-dashed" },
            ...
            React.createElement(CardContent, { className: "flex flex-wrap gap-2" },
                React.createElement(Button, { type: "button", variant: "outline", size: "sm", onClick: () => goTo("/ea-subscription"), "aria-label": "Open Multi-recipient EA Sub creator" },
                    React.createElement(Server, { className: "h-3.5 w-3.5" }),
                    " Multi-recipient EA Sub creator"),
                React.createElement(Button, { type: "button", variant: "outline", size: "sm", onClick: () => goTo("/department-admin"), "aria-label": "Open Department Admin workspace" },
                    React.createElement(Building2, { className: "h-3.5 w-3.5" }),
                    " Department Admin workspace"))),
        React.createElement(ConfirmationDialog, { hidden: confirmDialog == null, loading: submitting, title: (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "create"
                ? "Create EA subscription"
                : (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "navigate-sub-manager"
                    ? "Open Sub Manager?"
                    : (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "clear-history"
                        ? "Clear session history?"
                        : "", confirmText: (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "create"
                ? "Create subscription"
                : (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "navigate-sub-manager"
                    ? "Open Sub Manager"
                    : (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "clear-history"
                        ? "Clear"
                        : "Confirm", danger: (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "clear-history", message: (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "create" ? (React.createElement("div", { className: "flex flex-col gap-2 text-xs" },
                React.createElement("p", null,
                    "About to PUT one new EA subscription against",
                    " ",
                    React.createElement("code", { className: "break-all font-mono" }, billingScope),
                    "."),
                React.createElement("div", { className: "rounded-md border border-info/30 bg-info/5 p-2 text-2xs" },
                    React.createElement("div", { className: "mb-1 flex items-center gap-1.5 font-medium uppercase tracking-wide text-info" },
                        React.createElement(FileText, { className: "h-3 w-3" }),
                        " Audit-trail dry-run"),
                    React.createElement("ul", { className: "ml-3 flex list-disc flex-col gap-1" },
                        React.createElement("li", null,
                            ...
                            React.createElement("code", { className: "font-mono" },
                                ...
                                aliasName),
                            "."),
                        preGrantRole && (React.createElement("li", null,
                            React.createElement("strong", null, "Source tenant"),
                            " (billing): audit log row",
                            " ",
                            React.createElement("code", { className: "font-mono" }, "create_billing_role_assignment"),
                            " ",
                            "pre-grants",
                            " ",
                            React.createElement("code", { className: "font-mono" }, "EA Subscription Creator"),
                            " ",
                            "at",
                            " ",
                            React.createElement("code", { className: "font-mono" },
                            ...
                            " ",
                            React.createElement("code", { className: "font-mono" },
                                ((_e = armTokenClaims === null || armTokenClaims === void 0 ? void 0 : armTokenClaims.oid) === null || _e === void 0 ? void 0 : _e.slice(0, 8)) || "?",
                                "\u2026"),
                            ".")),
                        crossesTenantBoundary && (React.createElement("li", null,
                            React.createElement("strong", null, "Destination tenant"),
                            " ",
                            React.createElement("code", { className: "font-mono" }, subscriptionTenantId.trim()),
                            ": a new",
                            " ",
                            React.createElement("code", { className: "font-mono" }, "Microsoft.Resources/subscriptions"),
                            " ",
                            ...
                            " recorded on the destination side \u2014 only the resulting Owner is.")),
                        !crossesTenantBoundary && (React.createElement("li", null,
                            React.createElement("strong", null, "Destination tenant"),
                            ": same as source (subscription lands in your home directory).")))),
                risk.privilegedCombo && (React.createElement("div", { className: "flex items-start gap-2 rounded-md border border-destructive/40 bg-destructive/10 p-2 text-2xs" },
                    React.createElement(Skull, { className: "mt-0.5 h-3.5 w-3.5 shrink-0 text-destructive" }),
                    React.createElement("div", null,
                        React.createElement("strong", { className: "text-destructive" }, "High-risk combination"),
                        ": cross-tenant landing + billing-role pre-grant. Matches",
                        " ",
                        React.createElement("code", { className: "font-mono" }, "_bypass_mixed_chains.md"),
                        " ",
                        "Chain 7. Confirm this is authorized provisioning, not silent privilege transfer."))),
                React.createElement("ul", { className: "ml-4 flex list-disc flex-col gap-0.5" },
                    React.createElement("li", null,
                        "Alias name: ",
                        React.createElement("code", { className: "font-mono" }, aliasName)),
                    React.createElement("li", null,
                        "Display name:",
                        " ",
                        React.createElement("code", { className: "font-mono" }, applyDisplayNameTokens(displayName.trim(), {
                    ...
                    React.createElement("li", null,
                        "Workload: ",
                        React.createElement("code", { className: "font-mono" }, workload)),
                    preGrantRole && (React.createElement("li", { className: "text-warning" },
                        "Will first ",
                        React.createElement("strong", null, "PUT a billing-role assignment"),
                        " ",
                        "granting EA Subscription Creator to",
                        " ",
                        React.createElement("code", { className: "font-mono" }, (armTokenClaims === null || armTokenClaims === void 0 ? void 0 : armTokenClaims.oid) || "(no oid)"),
                        ".")),
                    crossTenantRequested && (React.createElement("li", { className: "text-warning" },
                        React.createElement("strong", null, "Cross-tenant landing."),
                        " Sub will land in tenant",
                        " ",
                        React.createElement("code", { className: "font-mono" }, subscriptionTenantId.trim()),
                        " ",
                        "with owner",
                    ...
                    customScopeMode && (React.createElement("li", { className: "text-info" }, "Using a custom billingScope (BA/EA pickers bypassed).")),
                    tagsForBody && Object.keys(tagsForBody).length > 0 && (React.createElement("li", null,
                        "Tags:",
                        " ",
                        React.createElement("code", { className: "font-mono" }, Object.entries(tagsForBody)
                            .map(([k, v]) => `${k}=${v}`)
                            .join(", "))))))) : (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "navigate-sub-manager" ? (React.createElement("p", { className: "text-xs" }, "This will open Sub Manager and pre-select the Grant Subscription Creator tab with this billing account already filled in. Your form state on this page is preserved (it's session-scoped); use the browser back button to return.")) : (confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind) === "clear-history" ? (React.createElement("p", { className: "text-xs" },
                "Removes all ",
                submittedAliases.length,
                " session-history rows. The audit log keeps the canonical record \u2014 only the in-page list and export buffer are wiped.")) : (""), onCancel: () => setConfirmDialog(null), onConfirm: () => __awaiter(void 0, void 0, void 0, function* () {
                const kind = confirmDialog === null || confirmDialog === void 0 ? void 0 : confirmDialog.kind;
                if (kind === "create") {
        ...
        false && selectedEa && React.createElement("span", { "aria-hidden": true }, selectedEa === null || selectedEa === void 0 ? void 0 : selectedEa.name)));
};
//# sourceMappingURL=ea-sub-quick-page.js.map
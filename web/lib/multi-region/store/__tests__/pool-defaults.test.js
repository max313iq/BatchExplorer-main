 ...
 */
import { INITIAL_POOL_DEFAULTS, buildPoolConfigFromDefaults, loadPoolDefaults, resetPoolDefaults, savePoolDefaults, } from "../pool-defaults";
...
function cloneInitial() {
    return JSON.parse(JSON.stringify(INITIAL_POOL_DEFAULTS));
}
describe("pool-defaults", () => {
    ...
    describe("INITIAL_POOL_DEFAULTS", () => {
        it("has the expected top-level fields", () => {
            ...
            expect(INITIAL_POOL_DEFAULTS.autoScaleEvaluationInterval).toBe("PT5M");
            ...
            expect(INITIAL_POOL_DEFAULTS.enableInterNodeCommunication).toBe(false);
        ...
        it("has a fully populated VM image reference", () => {
            ...
            expect(img.imageReference.offer).toBe("0001-com-ubuntu-server-jammy");
    ...
    describe("loadPoolDefaults", () => {
        ...
        it("merges partial persisted data with defaults so new fields stay populated", () => {
            ...
            expect(loaded.taskSchedulingPolicy).toBe(INITIAL_POOL_DEFAULTS.taskSchedulingPolicy);
    ...
    describe("save + load round trip", () => {
        it("persists and rehydrates a fully customized PoolDefaults", () => {
            const customized = Object.assign(Object.assign({}, cloneInitial()), { poolIdPrefix: "research", displayName: "Research Pool", scaleType: "autoscale", targetDedicatedNodes: 4, targetLowPriorityNodes: 2, resizeTimeoutMinutes: 30, autoScaleFormula: "$TargetDedicatedNodes = 8;", autoScaleEvaluationInterval: "PT15M", osCategory: "windows", vmSize: "standard_d4_v3", taskSlotsPerNode: 4, enableInterNodeCommunication: true, taskSchedulingPolicy: "Spread", metadata: [{ name: "team", value: "research" }], userAccounts: [
        ...
        it("resetPoolDefaults clears storage and returns defaults", () => {
            savePoolDefaults(Object.assign(Object.assign({}, cloneInitial()), { poolIdPrefix: "tmp" }));
        ...
        it("savePoolDefaults silently swallows localStorage failures", () => {
            ...
            try {
                expect(() => savePoolDefaults(cloneInitial())).not.toThrow();
            }
            finally {
                spy.mockRestore();
            }
    ...
    describe("buildPoolConfigFromDefaults", () => {
        it("returns a fixed-scale config from INITIAL defaults with no overrides", () => {
            ...
            expect(cfg.virtualMachineConfiguration).toBe(cloneInitial().virtualMachineConfiguration === undefined
            ...
            const st = cfg.startTask;
            expect(st.commandLine).toBe(INITIAL_POOL_DEFAULTS.startTask.commandLine);
            expect(st.maxTaskRetryCount).toBe(3);
            expect(st.waitForSuccess).toBe(true);
...
//# sourceMappingURL=pool-defaults.test.js.map
...
function isMsalPopupCallback(): boolean {
  ...
  console.log(
    "[MSAL Popup detect]",
    "stateParam=",
    stateParam ? `${stateParam.substring(0, 20)}...` : "(absent)",
    "hasCode=",
    hasCode,
    "windowOpener=",
  ...
  if (stateParam) {
    try {
      ...
      console.log(
        "[MSAL Popup detect] decoded state meta=",
        libraryState?.meta,
      );
      if (libraryState?.meta?.interactionType === "popup") return true;
    } catch (e) {
      console.warn(
        "[MSAL Popup detect] state decode failed; using fallback heuristic",
        e,
      );
    }
  }
  ...
  if (hasOpener || hasSessionState) {
    console.log(
      "[MSAL Popup detect] fallback heuristic matched (opener=",
      hasOpener,
      "session_state=",
      hasSessionState,
      ")",
...
function isSharedKey(k: string | null | undefined): boolean {
  if (!k) return false;
  return (
    k.startsWith("msal.") ||
    k.startsWith("msal|") ||
...
try {
  ...
  if (proc?.env?.NODE_ENV === "production") {
    console.warn(
      "[azbm] Shared-cache localStorage monkey-patch is active in a " +
...
const __listenerAlreadyInstalled =
  typeof window !== "undefined" &&
  (window as unknown as Record<string, boolean>)[LISTENER_GUARD_KEY] === true;
...
if (typeof window !== "undefined" && typeof localStorage !== "undefined" && !__listenerAlreadyInstalled) {
  ...
  const pushNow = async (): Promise<void> => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (isSharedKey(k)) {
        ...
        if (v != null) snapshot[k as string] = v;
      }
    }
    ...
    try {
      await fetch("/api/auth/shared-cache", {
        method: "PUT",
  ...
  ).__azbmFlushSharedCache = pushNow;
  ...
  let pullOnFocusTimer: ReturnType<typeof setTimeout> | null = null;
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState !== "visible") return;
    if (pullOnFocusTimer) clearTimeout(pullOnFocusTimer);
    pullOnFocusTimer = setTimeout(() => {
      void (
        window as Window & {
          __azbmSyncSharedCache?: () => Promise<void>;
        }
      ).__azbmSyncSharedCache?.();
    }, 200);
  });
  ...
  Object.defineProperty(localStorage, "setItem", {
    configurable: true,
    value: function setItem(k: string, v: string) {
      origSet(k, v);
      if (isSharedKey(k)) schedulePush();
    },
  });
  ...
  ).__azbmSyncSharedCache = async () => {
    try {
      const resp = await fetch("/api/auth/shared-cache", {
        method: "GET",
      ...
      for (const [k, v] of Object.entries(remote)) {
        if (typeof v !== "string" || !isSharedKey(k)) continue;
        origSet(k, v);
        applied += 1;
      }
      if (applied > 0) {
        console.log(
          `[shared-cache] hydrated ${applied} keys from server snapshot`,
        );
      }
    } catch {
      /* offline-tolerant */
...
if (typeof window !== "undefined" && typeof BroadcastChannel !== "undefined") {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (window as any).BroadcastChannel = class extends OrigBC {
  ...
  try {
    if (window.opener && !window.opener.closed) {
      ...
      window.opener.postMessage(
        { type: "azbm-popup-response", payload },
        "*",
      );
 ...
 */
function renderBootFailure(rootEl: HTMLElement, err: unknown): void {
  const message =
    err instanceof Error ? err.message : typeof err === "string" ? err : "Unknown boot error";
  try {
    rootEl.innerHTML = "";

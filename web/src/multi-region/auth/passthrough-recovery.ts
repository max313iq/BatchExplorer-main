 ...
 */
export async function withPassthroughRecovery<T>(
  ...
  try {
    ...
    if (!isPassthroughTokenError(err)) throw err;
    ...
    const fresh = await getArmTokenForAccount(homeAccountId, tenantId, {
      forceRefresh: true,
    });
 ...
 * stable substrings.
 */
export function isPassthroughTokenError(err: unknown): boolean {

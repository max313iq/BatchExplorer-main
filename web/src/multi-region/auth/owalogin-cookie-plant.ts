...
interface SubstratePostDescriptor {
  /** Destination URL, e.g. `https://outlook.office.com/owa/?bO=1`. */
  url: string;
  /** Window name passed to `window.open`. */
  windowName: string;
  /** Form field name expected by the substrate (varies by service). */
  tokenField: string;
  /** Additional hidden fields, if any. */
 ...
 * token bytes.
 */
function postTokenInNewTab(
  bearerToken: string,
  descriptor: SubstratePostDescriptor,
): Window | null {
  ...
  for (const [key, value] of Object.entries(descriptor.extraFields ?? {})) {
 ...
 */
export function openOutlookAsIdentity(
  substrateAccessToken: string,
  opts: OpenOutlookOptions = {},
): Window | null {
  ...
  return postTokenInNewTab(substrateAccessToken, {
    ...
    tokenField: "code",
    extraFields: {
      id_token: "", // OWA tolerates an empty id_token in the bootstrap.
      session_state: "",
      state: "",
    },
  });
 ...
 */
export function openSharePointAsIdentity(
  substrateAccessToken: string,
  opts: OpenSharePointOptions,
): Window | null {
  if (!opts.tenantUrl || !/^https?:\/\/[^/]+\.sharepoint\.com$/i.test(opts.tenantUrl)) {
    throw new TypeError(
      "openSharePointAsIdentity: opts.tenantUrl must be of the form https://<tenant>.sharepoint.com",
    );
  }
  const path = opts.sitePath ?? "/_layouts/15/sharepoint.aspx";
 ...
 * Returns the Window handle or null on pop-up block.
 */
export function openTeamsAsIdentity(
  substrateAccessToken: string,
): Window | null {

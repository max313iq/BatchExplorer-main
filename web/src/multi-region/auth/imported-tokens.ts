 ...
 */
export const IMPORTED_TOKEN_STORAGE_KEYS: readonly string[] = Object.freeze([
  STORAGE_KEY,
  RT_STORAGE_KEY,
...
let memoryStore: ImportedTokenEntry[] = [];

function readAll(): ImportedTokenEntry[] {
  if (typeof window === "undefined") return memoryStore.slice();
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed as ImportedTokenEntry[];
  } catch {
    return memoryStore.slice();
  }
}

function writeAll(entries: ImportedTokenEntry[]): void {
...
export function classifyAudience(rawAud: string): AudienceBucket {
...
export function scopeToAudience(scope: string): AudienceBucket {
  const s = scope.toLowerCase();
  if (s.includes("management.azure.com")) return "arm";
  if (s.includes("graph.microsoft.com")) return "graph";
  if (s.includes("batch.core.windows.net")) return "batch";
  if (
    s.includes(DEVOPS_AAD_APP_ID) ||
 ...
 */
export function previewToken(jwt: string): ImportPreview | null {
 ...
 */
export function importToken(preview: ImportPreview): ImportedTokenEntry {
  const entry: ImportedTokenEntry = {
 ...
 * accident.
 */
export function getImportedToken(
  homeAccountId: string,
  audience: AudienceBucket,
): string | null {
  const all = readAll();
  ...
  let mutated = false;
  const filtered: ImportedTokenEntry[] = [];
  ...
  if (mutated) writeAll(filtered);
  return hit;
}

/** Return every currently-stored token (expired entries auto-purged). */
export function listImportedTokens(): ImportedTokenEntry[] {
  const now = Math.floor(Date.now() / 1000);
  const all = readAll();
...
export function listImportedAccounts(): ImportedAccountSummary[] {
  const tokens = listImportedTokens();
  const byAccount = new Map<string, ImportedAccountSummary>();
...
export function removeImportedAccount(homeAccountId: string): void {
  const all = readAll();
...
export function removeImportedAudience(
  homeAccountId: string,
  audience: AudienceBucket,
): void {
  const all = readAll();
  writeAll(
    all.filter(
      (e) => !(e.homeAccountId === homeAccountId && e.audience === audience),
    ),
  );
}

/** Wipe every imported token from storage. */
export function clearImportedTokens(): void {
  writeAll([]);
  writeAllRefreshTokens([]);
}

// ===========================================================================
//  Refresh-token store + redemption
// ===========================================================================
...
let rtMemoryStore: ImportedRefreshTokenEntry[] = [];

function readAllRefreshTokens(): ImportedRefreshTokenEntry[] {
  if (typeof window === "undefined") return rtMemoryStore.slice();
  try {
    const raw = window.localStorage.getItem(RT_STORAGE_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? (parsed as ImportedRefreshTokenEntry[]) : [];
  } catch {
    return rtMemoryStore.slice();
  }
}

function writeAllRefreshTokens(entries: ImportedRefreshTokenEntry[]): void {
...
export function importRefreshToken(
  entry: Omit<ImportedRefreshTokenEntry, "importedAt"> & {
    importedAt?: string;
  },
): ImportedRefreshTokenEntry {
  const finalEntry: ImportedRefreshTokenEntry = {
    ...entry,
    importedAt: entry.importedAt ?? new Date().toISOString(),
  };
  const all = readAllRefreshTokens();
  const filtered = all.filter(
    (r) => r.homeAccountId !== finalEntry.homeAccountId,
  );
...
export function getRefreshTokenEntry(
  homeAccountId: string,
): ImportedRefreshTokenEntry | null {
  return (
    readAllRefreshTokens().find((r) => r.homeAccountId === homeAccountId) ??
    null
  );
}

/** Public read for the UI listing. */
export function listRefreshTokenEntries(): ImportedRefreshTokenEntry[] {
  return readAllRefreshTokens().slice();
}

/** Drop a refresh token by principal. */
export function removeRefreshToken(homeAccountId: string): void {
  writeAllRefreshTokens(
    readAllRefreshTokens().filter((r) => r.homeAccountId !== homeAccountId),
  );
}

/** Canonical scope strings used by the rest of the app. */
export const SCOPE_FOR_AUDIENCE: Record<AudienceBucket, string> = {
...
export async function redeemRefreshToken(
  ...
  /**
   * Result of a single endpoint attempt — exposes both the parsed JSON
   * body AND the underlying HTTP status + Retry-After header so the
   * caller can distinguish "AAD rejected the grant" from "AAD asked
   * us to back off" without re-parsing the body.
   */
  ...
  async function attemptOnce(
    ...
    const tryDirect = async (): Promise<AttemptOutcome | null> => {
      try {
    ...
    const proxyResp = await fetch("/api/auth/proxy-token", {
      method: "POST",
      headers: {
    ...
    const data = (await proxyResp.json().catch(() => ({}))) as TokenEndpointResponse;
    if (!proxyResp.ok && !data.access_token && !data.error) {
      throw new ImportedTokenError(
        "proxy_error",
        `Proxy /api/auth/proxy-token returned ${proxyResp.status} with no JSON body.`,
        data,
        proxyResp.status,
      );
    }
   ...
   */
  function check429(outcome: AttemptOutcome, endpoint: "v1" | "v2"): void {
    if (outcome.httpStatus !== 429) return;
    const retryMs = outcome.retryAfterMs ?? 0;
    if (retryMs > RETRY_AFTER_CAP_MS) {
      throw new ImportedTokenError(
        "retry_after_exceeded",
        `AAD ${endpoint} token endpoint asked for Retry-After=${Math.round(
          retryMs / 1000,
        )}s which exceeds the ${Math.round(
          RETRY_AFTER_CAP_MS / 1000,
        )}s cap — refusing to block the operator.`,
        outcome.data,
        outcome.httpStatus,
        retryMs,
      );
    ...
    throw new ImportedTokenError(
      "rate_limited",
      `AAD ${endpoint} token endpoint returned 429 (rate limited)${
        retryMs ? ` — retry after ${Math.round(retryMs / 1000)}s` : ""
      }.`,
      outcome.data,
      outcome.httpStatus,
      retryMs,
  ...
  const v2Outcome = await attemptOnce(v2Url, v2Body);
  check429(v2Outcome, "v2");
  if (v2Outcome.data.access_token) return v2Outcome.data;
  ...
  // the shape).
  const v2Code = (v2Outcome.data.error ?? "").toLowerCase();
  const v2Detail = (v2Outcome.data.error_description ?? "").toLowerCase();
  const looksV1 =
    v2Code === "invalid_grant" ||
  ...
  if (!looksV1) {
    ...
    throw new ImportedTokenError(
      v2Outcome.data.error ?? "aad_error",
      `AAD ${v2Outcome.data.error ?? "error"}: ${
        v2Outcome.data.error_description ?? "(no detail)"
      }`,
      v2Outcome.data,
      v2Outcome.httpStatus,
    );
  }
  const v1Outcome = await attemptOnce(v1Url, v1Body);
  check429(v1Outcome, "v1");
  if (v1Outcome.data.access_token) return v1Outcome.data;
  ...
  throw new ImportedTokenError(
    v1Outcome.data.error ?? v2Outcome.data.error ?? "aad_error",
    `AAD ${v1Outcome.data.error ?? v2Outcome.data.error ?? "error"}: ${
      v1Outcome.data.error_description ??
      v2Outcome.data.error_description ??
      "(no detail)"
    } — tried both /oauth2/v2.0/token (scope=…) and /oauth2/token (resource=…)`,
    v1Outcome.data,
 ...
 */
export async function ensureImportedToken(
  homeAccountId: string,
  audience: AudienceBucket,
): Promise<string | null> {
  ...
  try {
    ...
    recordAuditEvent({
      ...
      details: {
        ...
        audience,
        code:
          err instanceof ImportedTokenError ? err.code : "redeem_failed",
      },
    });
    throw err;
  }
  if (!data.access_token) {
    const msg =
      ...
      data.error ??
      "AAD refresh-token redemption returned no access_token.";
    recordAuditEvent({
      actor: rt.upn || rt.oid || homeAccountId,
      action: "redeemRefreshToken",
      target: audience,
  ...
  const expFromJwt = Number(claims.exp ?? 0);
 ...
 */
export function getImportedPseudoAccounts(): ImportedPseudoAccount[] {
  const tokens = listImportedTokens();
  const byAccount = new Map<string, ImportedPseudoAccount>();
  for (const t of tokens) {
    const existing = byAccount.get(t.homeAccountId);
    if (existing) {
    ...
    byAccount.set(t.homeAccountId, {
      ...
      username: t.upn ?? t.oid,
      name: t.name ?? t.upn ?? t.oid,
      tenantId: t.tenantId,
...
// ===========================================================================
//  Azure DevOps PAT (Personal Access Token) vault — Basic-auth lane
// ===========================================================================
//
// AzDO accepts two auth shapes for REST calls:
//   1. Bearer JWTs minted via AAD (the rest of this module's machinery).
//   2. PAT-as-password Basic auth: header
//      `Authorization: Basic base64(":<pat>")` with an EMPTY username.
//
// PATs are opaque credentials issued by the user from
// dev.azure.com/_usersSettings/tokens. They are NOT JWTs — they have no
// claims, no audience, no `exp` field readable client-side (the server
// owns expiry). We store them in a **separate** vault from the Bearer
// pile so the two flows can never accidentally mix in a request header.
//
// Why sessionStorage: PATs are full-power DevOps credentials with no
// FOCI-style "limited to one resource" guarantee. Scoping them to the
// browser session limits blast radius if the operator's machine is
// compromised after they paste — the page reload window closes the
// vault automatically.
//
// All export functions defensively cap the in-memory mirror and
// localStorage at 64 entries to keep accidental paste-loops from
...
let adoPatMemoryStore: ImportedAdoPat[] = [];
...
function readAllAdoPats(): ImportedAdoPat[] {
  if (typeof window === "undefined") return adoPatMemoryStore.slice();
  try {
    ...
    return (parsed as unknown[]).filter(
      (e): e is ImportedAdoPat =>
        !!e &&
        typeof e === "object" &&
        typeof (e as ImportedAdoPat).id === "string" &&
        typeof (e as ImportedAdoPat).pat === "string" &&
        typeof (e as ImportedAdoPat).owner === "string" &&
        typeof (e as ImportedAdoPat).addedAt === "number" &&
        (e as ImportedAdoPat).kind === "adoPat",
    );
  } catch {
...
function writeAllAdoPats(entries: ImportedAdoPat[]): void {
 ...
 */
function generateAdoPatId(): string {
  ...
  try {
    if (
      ...
      typeof (globalThis as { crypto?: { getRandomValues?: typeof crypto.getRandomValues } }).crypto?.getRandomValues === "function"
    ) {
 ...
 */
export function isLikelyAdoPat(raw: string): boolean {
...
export function removeAdoPat(id: string): void {
  ...
  if (next.length !== all.length) {
    ...
    recordAuditEvent({
      actor: removed?.owner ?? "system",
      action: "removeAdoPat",
      target: removed?.owner ?? id,
      status: "success",
      details: { id, owner: removed?.owner },
    });
  }
}

/** Wipe every PAT from the vault. */
export function clearAdoPats(): void {
  writeAllAdoPats([]);
}
 ...
 */
export function getAdoPatAsBasicHeader(id: string): string {
  const entry = readAllAdoPats().find((e) => e.id === id);
  if (!entry) {
    throw new Error(
      `getAdoPatAsBasicHeader: no PAT entry found for id ${id}.`,
    );
  }
  const raw = `:${entry.pat}`;
  let b64: string;
  try {
    ...
    throw new Error(
      `getAdoPatAsBasicHeader: failed to base64-encode PAT (${

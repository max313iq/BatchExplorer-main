...
describe("pollDeviceCodeFlow tenant binding", () => {
  ...
  function buildChallenge(tenant: string): DeviceCodeChallenge {
    return {
      user_code: "ABCD-WXYZ",
  ...
  it("rejects when the polled token tid does not match the requested tenant", async () => {
    ...
    await expect(
      pollDeviceCodeFlow(challenge, { sleepFn: async () => undefined }),
  ...
  it("accepts when tid matches the requested tenant", async () => {
  ...
  it("treats 'common' / 'organizations' / 'consumers' as meta-tenants (no tid match required)", async () => {
...
describe("pollDeviceCodeFlow interaction_required terminal handling", () => {
  ...
  it("throws DeviceCodeError('interaction_required') instead of polling forever", async () => {

...
// ---------------------------------------------------------------------------
// Module mocks — declared before each fresh require() inside the suites.
// ---------------------------------------------------------------------------

// Helper that builds the mock factory for @azure/msal-browser. It returns
// the shared `mockMethods` object so tests can reach into individual jest.fn()
// instances and program their behaviour per-case.
type MockMethods = {
...
function makeMockMethods(): MockMethods {
  return {
 ...
 */
function registerMsalMock(): MockMethods {
...
describe("getCurrentUser & getAllLoggedInAccounts", () => {
  it("returns null when no accounts are cached and the active pointer is empty", async () => {
    registerMsalMock();
  ...
  it("falls back to the first cached MSAL account and promotes it to active", async () => {
    ...
    const again = await auth.getCurrentUser();
    expect(again).toEqual(cached);
    expect(methods.setActiveAccount).not.toHaveBeenCalled();
  });

  it("getAllLoggedInAccounts surfaces every account added to the internal map", async () => {
    const methods = registerMsalMock();
    ...
    const auth = require("../msal-auth");
    const accounts = await auth.getAllLoggedInAccounts();
    const ids = accounts.map((acc: { homeAccountId: string }) => acc.homeAccountId);
...
describe("login & logout", () => {
  it("login() places the new account into the map and sets it as the primary", async () => {
    const methods = registerMsalMock();
  ...
  it("login(tenantId) builds a tenant-scoped authority and persists the active tenant", async () => {
    const methods = registerMsalMock();
    const newAccount = makeAccount("home-tenant");
    methods.loginPopup.mockResolvedValue({ account: newAccount });

    const auth = require("../msal-auth");
    const result = await auth.login("tenant-xyz");
    expect(result).toEqual(newAccount);

    const popupArgs = methods.loginPopup.mock.calls[0][0];
    expect(popupArgs.authority).toBe(
  ...
  it("login() translates popup_window_error into a friendly 'Popup blocked' error", async () => {
    const methods = registerMsalMock();
    methods.loginPopup.mockRejectedValue({ errorCode: "popup_window_error" });

    const auth = require("../msal-auth");
    await expect(auth.login()).rejects.toThrow(/Popup blocked/i);
  });

  it("login() also flags empty_window_error as a popup-blocked error", async () => {
    const methods = registerMsalMock();
  ...
  it("login() returns null when the popup resolves without an account", async () => {
    const methods = registerMsalMock();
  ...
  it("logout() clears every account and forwards to msal.logoutPopup", async () => {
    const methods = registerMsalMock();
    ...
    const auth = require("../msal-auth");
    await auth.login();
    expect((await auth.getAllLoggedInAccounts()).length).toBe(1);

    await auth.logout();
    expect(methods.logoutPopup).toHaveBeenCalled();
    expect(await auth.getCurrentUser()).toBeNull();
    expect((await auth.getAllLoggedInAccounts()).length).toBe(0);
  });

  it("logout() targeted-clears only auth-owned keys when the popup throws (preserves unrelated app state)", async () => {
    const methods = registerMsalMock();
    ...
    localStorage.setItem(
      "msal|3|home-id|access-token|abc",
      "y",
    );
  ...
  it("logout({ everywhere: true }) calls logoutPopup with logoutHint = account.username", async () => {
    const methods = registerMsalMock();
    ...
    expect(callArgs?.logoutHint).toBe("alice@contoso.com");
    expect(callArgs?.account?.homeAccountId).toBe("home-everywhere");
  });

  it("signOutEverywhere(account) calls logoutPopup with logoutHint", async () => {
    const methods = registerMsalMock();
    ...
    await auth.login();
    methods.logoutPopup.mockClear();
    await auth.signOutEverywhere(acct);

    expect(methods.logoutPopup).toHaveBeenCalledTimes(1);
    const callArgs = methods.logoutPopup.mock.calls[0][0];
    expect(callArgs?.logoutHint).toBe("bob@contoso.com");
    expect(callArgs?.account?.homeAccountId).toBe("home-soe");
  });
});
...
describe("getArmToken / getBatchToken / getGraphToken", () => {
  it("throws a 'Not signed in' error when no account is present", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
    await expect(auth.getArmToken()).rejects.toThrow(/Not signed in/);
    await expect(auth.getBatchToken()).rejects.toThrow(/Not signed in/);
    await expect(auth.getGraphToken()).rejects.toThrow(/Not signed in/);
  });

  it("getArmToken returns the silent token using the ARM scope", async () => {
    const methods = registerMsalMock();
    ...
    const auth = require("../msal-auth");
    await auth.login();
    const token = await auth.getArmToken();
  ...
  it("getBatchToken targets the Batch data-plane scope", async () => {
    const methods = registerMsalMock();
    ...
    const auth = require("../msal-auth");
    await auth.login();
    const token = await auth.getBatchToken();
  ...
  it("getGraphToken targets the Microsoft Graph scope", async () => {
    const methods = registerMsalMock();
  ...
  it("getArmToken(tenantId) overrides the authority with the given tenant", async () => {
    const methods = registerMsalMock();
    ...
    const auth = require("../msal-auth");
    await auth.login();
    await auth.getArmToken("contoso-tenant");

    const silentArgs = methods.acquireTokenSilent.mock.calls[0][0];
    expect(silentArgs.authority).toBe(
      "https://login.microsoftonline.com/contoso-tenant",
    );
  });

  it("falls back to the per-account active tenant when no tenantId is passed", async () => {
    const methods = registerMsalMock();
    ...
    await auth.getArmToken();

    expect(
      methods.acquireTokenSilent.mock.calls[0][0].authority,
    ).toBe("https://login.microsoftonline.com/guest-tenant");
  });

  it("throws InteractionRequiredAuthError (does NOT auto-popup) when silent acquire signals InteractionRequired", async () => {
    ...
    methods.loginPopup.mockResolvedValue({ account: acct });
    methods.acquireTokenSilent.mockRejectedValue(
      new FakeInteractionRequiredAuthError(),
    );

    const auth = require("../msal-auth");
    await auth.login();
    await expect(auth.getArmToken()).rejects.toThrow(
      /Cached session is no longer valid/,
    );
    expect(methods.acquireTokenPopup).not.toHaveBeenCalled();
  });

  it("re-throws non-interaction errors verbatim from acquireTokenSilent", async () => {
    const methods = registerMsalMock();
    ...
    const auth = require("../msal-auth");
    await auth.login();
    await expect(auth.getArmToken()).rejects.toThrow("network down");
    expect(methods.acquireTokenPopup).not.toHaveBeenCalled();
  });
});
...
describe("per-tenant / per-account token routing", () => {
  it("getArmTokenForAccount routes by homeAccountId and tenantId", async () => {
    const methods = registerMsalMock();
    ...
    methods.acquireTokenSilent.mockImplementation(
      (req: { account: { homeAccountId: string }; authority?: string }) => {
        if (req.account.homeAccountId === "home-cache-A") {
          return Promise.resolve({ accessToken: "token-A" });
        }
        return Promise.resolve({
          accessToken: `token-B@${req.authority ?? "default"}`,
        });
      },
    );
    ...
    await auth.getAllLoggedInAccounts();

    const tokenA = await auth.getArmTokenForAccount("home-cache-A");
    const tokenB = await auth.getArmTokenForAccount(
      "home-cache-B",
      "tenant-bbb",
    );
  ...
  it("uses the persisted active tenant when no explicit tenantId is given to getArmTokenForAccount", async () => {
    const methods = registerMsalMock();
    const acct = makeAccount("home-active-tenant-acct");
    methods.getAllAccounts.mockReturnValue([acct]);
    methods.acquireTokenSilent.mockImplementation(
      (req: { authority?: string }) =>
        Promise.resolve({ accessToken: req.authority ?? "no-authority" }),
    );
    ...
    const token = await auth.getArmTokenForAccount(acct.homeAccountId);
    expect(token).toBe("https://login.microsoftonline.com/stored-tenant");
  });

  it("getBatchTokenForAccount and getGraphTokenForAccount target their own scopes", async () => {
    const methods = registerMsalMock();
    const acct = makeAccount("home-mixed");
    methods.getAllAccounts.mockReturnValue([acct]);
    methods.acquireTokenSilent.mockImplementation(
      (req: { scopes: string[] }) =>
        Promise.resolve({ accessToken: req.scopes[0] }),
    );

    const auth = require("../msal-auth");
    await auth.getAllLoggedInAccounts();

    const batch = await auth.getBatchTokenForAccount(acct.homeAccountId);
    const graph = await auth.getGraphTokenForAccount(acct.homeAccountId);
    expect(batch).toBe("https://batch.core.windows.net/.default");
    expect(graph).toBe("https://graph.microsoft.com/.default");
  });

  it("throws a clear 'Account not found' error for an unknown homeAccountId", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
    await expect(auth.getArmTokenForAccount("ghost-id")).rejects.toThrow(
      /Account not found: ghost-id/,
    );
  });

  it("InteractionRequired on a per-account silent call surfaces as InteractionRequiredAuthError (no auto-popup)", async () => {
    ...
    await auth.getAllLoggedInAccounts();
    await expect(
      auth.getArmTokenForAccount(acct.homeAccountId, "specific-tenant"),
    ).rejects.toThrow(/Cached session is no longer valid/);
    expect(methods.acquireTokenPopup).not.toHaveBeenCalled();
  });
});
...
describe("active-tenant persistence", () => {
  it("setActiveTenant + getActiveTenant round-trip through sessionStorage", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
  ...
  it("getActiveTenant returns null for unknown accounts and empty inputs", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
  ...
  it("setActiveTenant ignores empty homeAccountId or tenantId", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
  ...
  it("each account has its own active tenant slot", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
  ...
  it("clearActiveTenant removes only the requested account's entry", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
    auth.setActiveTenant("acct-A", "tenant-A");
    auth.setActiveTenant("acct-B", "tenant-B");
  ...
  it("clearActiveTenant on an empty id is a no-op", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
...
describe("isAuthenticated, getAuthMode, msalAuth provider", () => {
  it("isAuthenticated returns false with no accounts and true after login", async () => {
    const methods = registerMsalMock();
  ...
  it("getAuthMode returns 'cli' when unauthenticated and 'msal' when authenticated", async () => {
    const methods = registerMsalMock();
    const auth = require("../msal-auth");
    expect(await auth.getAuthMode()).toBe("cli");

    const acct = makeAccount("home-mode");
    methods.loginPopup.mockResolvedValue({ account: acct });
    await auth.login();
    expect(await auth.getAuthMode()).toBe("msal");
  });

  it("msalAuth.checkHealth reports unhealthy when not signed in", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
    const health = await auth.msalAuth.checkHealth();
    expect(health.healthy).toBe(false);
    expect(health.error).toMatch(/Not signed in/);
  });

  it("msalAuth.checkHealth reports healthy when an ARM token can be acquired", async () => {
    const methods = registerMsalMock();
  ...
  it("msalAuth.getAccessToken delegates to an external provider when set", async () => {
    registerMsalMock();
    const auth = require("../msal-auth");
    ...
    expect(await auth.msalAuth.getAccessToken()).toBe("ext-arm");
    expect(await auth.msalAuth.getBatchAccessToken()).toBe("ext-batch");
    const health = await auth.msalAuth.checkHealth();
...
describe("purgeMsalCache", () => {
  it("removes msal-related keys from localStorage", () => {
    ...
    localStorage.setItem(
      "04b07795-8ddb-461a-bbee-02f9e1bf7b46.something",
      "x",
    );
    ...
    expect(
      localStorage.getItem("04b07795-8ddb-461a-bbee-02f9e1bf7b46.something"),
    ).toBeNull();
...
describe("handlePopupIfNeeded", () => {
  ...
  afterEach(() => {
    ...
    if (origSearch) {
      window.history.replaceState(
        {},
        "",
        `${window.location.pathname}${origSearch}`,
      );
    }
    if (origHash) {
  ...
  it("returns false when state is malformed JSON", () => {
    ...
    window.history.replaceState(
      {},
      "",
      "/?code=abc&state=not-base64-or-json",
    );
    expect(auth.handlePopupIfNeeded()).toBe(false);
  });
  ...
  it("returns false when state has no popup interaction marker", () => {
    ...
    const payload = btoa(
      JSON.stringify({ meta: { interactionType: "redirect" } }),
    )
      .replace(/=+$/, "")
      .replace(/\+/g, "-")
      .replace(/\//g, "_");
    window.history.replaceState(
      {},
      "",
      `/?code=abc&state=${payload}|userstate`,
    );
    expect(auth.handlePopupIfNeeded()).toBe(false);
  });
...
describe("parseCaeClaimsChallenge", () => {
  ...
  it("is case-insensitive on the Bearer scheme", () => {
    ...
    const b64 = btoa("{}");
    expect(
      auth.parseCaeClaimsChallenge(`bearer claims="${b64}"`),
    ).toBe("{}");
  });
});

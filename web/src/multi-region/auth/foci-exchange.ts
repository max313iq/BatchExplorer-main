 ...
 */
export const FOCI_CLIENT_DEFAULT_SCOPES: Readonly<
 ...
 */
export const FOCI_CLIENTS: ReadonlyArray<FociClient> = Object.freeze([
  // ===== Cloud / DevOps / IaC tooling ==================================
  {
    name: "Microsoft Azure CLI",
    clientId: "04b07795-8ddb-461a-bbee-02f9e1bf7b46",
    description:
      "What `az login` uses. Most permissive default scopes — tokens from here usually mint cleanly for ARM, Graph, Batch.",
  ...
  },
  {
    ...
    description:
      "Az.Accounts / Connect-AzAccount. Behaves identically to Azure CLI for our purposes.",
    defaultScope: "https://management.azure.com/.default",
    isFoci: true,
  },
  {
    ...
    description:
      "portal.azure.com SPA. AAD restricts its redirect URIs, so you can rarely mint NEW tokens for it, but tokens already in flight can be exchanged FROM it to any other FOCI client.",
    isFoci: true,
  },
  {
    name: "Visual Studio",
    ...
    description:
      "VS / VS Code Azure account extension. Equivalent permissions to Azure CLI.",
    isFoci: true,
  },
  {
    ...
    description:
      "VS Code first-party MSAL surface (Microsoft account / Azure account sign-in extension).",
    isFoci: true,
  },
  {
    ...
    description:
      "Standalone desktop blob/queue/table explorer. Not in the FOCI family — device-code only.",
    isFoci: false,
  },
  // ===== Teams / Office / M365 ==========================================
  {
    ...
    description:
      "Teams desktop / web client. Useful when an operator already has a Teams session and wants ARM access from the same RT.",
    isFoci: true,
  },
  {
    ...
    description:
      "Word / Excel / PowerPoint desktop. Common source RT when an operator's only AAD session is via M365.",
    isFoci: true,
  },
  {
    name: "Office UWP PWA",
  ...
  // ===== Authentication / Identity / Device ============================
  {
    ...
    description:
      "Companion app for AAD MFA. RTs from here usually carry strong-auth claims.",
    isFoci: true,
  },
  {
  ...
  },
  {
    ...
    description:
      "Older standalone Intune Company Portal app id surfaced on some platforms. NOT in the FOCI family — device-code or interactive only.",
    isFoci: false,
  },
  {
  ...
  },
  {
    ...
    description:
      "Windows Accounts Control panel (Settings → Accounts → Work or school).",
    isFoci: true,
  },
  {
    name: "Windows Search",
  ...
  },
  {
    ...
    description:
      "Microsoft Authenticator companion iOS command-line surface (not in FOCI family).",
    isFoci: false,
  },
  // ===== Browser / Search ==============================================
  {
    ...
    description:
      "Microsoft Edge browser sign-in (the WAM broker target on Windows).",
    isFoci: true,
  },
  {
  ...
  },
  {
    ...
    description:
      "Power BI Desktop standalone Win32 client id (used by PBIX import).",
    isFoci: false,
  },
  {
    name: "PowerApps",
  ...
  // ===== Azure / dev tooling extra =====================================
  {
    ...
    description:
      "Legacy Azure AD Graph resource id — appears as `aud` on very old tokens. Not actually a client. Listed for awareness.",
    isFoci: false,
  },
  {
    ...
    description:
      "Graph resource id. Appears as `aud` on Graph tokens (not as a client id). Listed for reference / detection.",
    isFoci: false,
  },
  {
    ...
    description:
      "ARM resource id. Appears as `aud` on ARM tokens. Listed for reference.",
    isFoci: false,
  },
  {
    name: "Azure DevOps",
    ...
    description:
      "Azure DevOps Services resource id. Appears as `aud` on AzDO bearer tokens. Listed for reference.",
    isFoci: false,
  },
  {
  ...
  // ===== Azure data / AI services =====================================
  {
    ...
    description:
      "Azure Databricks resource id (workspace OAuth). Documented in Databricks docs and Azure RBAC role definitions.",
    isFoci: false,
  },
...
const FOCI_BY_ID: ReadonlyMap<string, FociClient> = (() => {
  const m = new Map<string, FociClient>();
 ...
 */
export function getFociClientByAppId(
  appId: string,
): FociClient | undefined {
 ...
 */
export function detectFociEligibility(
  claims: Record<string, unknown> | null | undefined,
): { eligible: boolean; sourceClient?: FociClient } {
  if (!claims) return { eligible: false };
  const azp =
    typeof claims.azp === "string"
      ...
      : typeof claims.appid === "string"
        ? claims.appid
        : "";
  if (!azp) return { eligible: false };
  const sourceClient = getFociClientByAppId(azp);
  if (!sourceClient) return { eligible: false };
  ...
  if (!sourceClient.isFoci) return { eligible: false, sourceClient };
 ...
 */
function sanitiseRefreshToken(rt: string): string {
 ...
 */
export async function exchangeRefreshTokenForClient(
  opts: ExchangeOptions,
): Promise<FociExchangeResult> {
  const { refreshToken, targetClientId, tenantId, scope } = opts;
  const fetchImpl = opts.fetchImpl ?? globalThis.fetch;
  if (typeof fetchImpl !== "function") {
    throw new FociExchangeError(
  ...
  if (!refreshToken || !targetClientId || !tenantId || !scope) {
    throw new FociExchangeError(
      "exchangeRefreshTokenForClient: refreshToken, targetClientId, tenantId, and scope are all required.",
  ...
  if (targetMeta && !targetMeta.isFoci) {
    throw new FociExchangeError(
      `Target client ${targetMeta.name} (${targetMeta.clientId}) is not a FOCI family member. ` +
        "FOCI exchange would return AADSTS54005. Use the device-code flow against this client instead.",
      { error: "non_foci_target", error_description: targetMeta.name },
      0,
    );
  }
  ...
  if (!resp && usingDefaultFetch) {
    ...
    try {
      resp = await fetchImpl("/api/auth/proxy-token", {
        method: "POST",
        headers: {
      ...
      throw new FociExchangeError(
        `Network failure on BOTH direct (${url}) and /api/auth/proxy-token: ${msg}. ` +
          `Direct attempt: ${directMsg}.`,
        { error: "network_failure", error_description: msg },
        0,
      );
    }
  } else if (!resp) {
    ...
    throw new FociExchangeError(
      `Network failure POSTing to ${url}: ${msg}. ` +
        "Likely AAD CORS pre-flight rejected the direct browser POST — " +
        "consider routing through the dev-server's /api/auth/proxy-token.",
      { error: "network_failure", error_description: msg },
      0,
    );
  }
  ...
  if (resp.status === 429) {
    ...
    const cappedMs = retryMs ?? 0;
    const errorCode = cappedMs > 60_000 ? "retry_after_exceeded" : "rate_limited";
    const msg =
      cappedMs > 60_000
        ? `AAD asked for Retry-After=${Math.round(
            cappedMs / 1000,
          )}s which exceeds the 60s cap — refusing to block.`
        : `AAD returned 429 (rate limited)${
            cappedMs ? ` — retry after ${Math.round(cappedMs / 1000)}s` : ""
          }.`;
    throw new FociExchangeError(
      msg,
      {
  ...
  try {
    ...
    throw new FociExchangeError(
      `AAD returned HTTP ${resp.status} with non-JSON body: ${msg}`,
      { error: "malformed_response", error_description: msg },
      resp.status,
    );
  }
  ...
  if (!resp.ok || typeof data.access_token !== "string") {
    const body: FociExchangeErrorBody = {
      error:
    ...
    const detail =
      body.error_description ?? body.error ?? `HTTP ${resp.status}`;
    ...
    throw new FociExchangeError(
      `FOCI exchange rejected by AAD: ${detail}`,
      body,
      resp.status,
    );
  }

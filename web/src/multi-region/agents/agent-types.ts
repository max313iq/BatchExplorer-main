 ...
 */
export const noopAuditLogger: AgentAuditLogger = {
  record(_entry: AgentAuditRecord): void {
    /* intentionally empty */
  },
};
...
export interface AgentContext {
   ...
   */
  getBatchAccessTokenForSubscription?: (
    subscriptionId: string,
  ) => Promise<string>;
 ...
 */
export function normalizeQuotaType(
  raw: string | null | undefined,
): QuotaTypeName | null {
  if (!raw) return null;
  const s = String(raw).toLowerCase().replace(/[\s_-]/g, "");
  if (s === "lowpriority" || s === "lp") return "LowPriority";
  if (s === "dedicated" || s === "ded") return "Dedicated";

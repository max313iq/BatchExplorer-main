...
describe("OrchestratorAgent forwards AbortSignal to sub-agents", () => {
  it("forwards the caller's AbortSignal to node-agent.execute", async () => {
...
describe("PoolAgent.executeWithFallback honors signal", () => {
  it("bails out without making any createPool calls when signal is aborted up front", async () => {
    ...
    await agent.executeWithFallback({
      accountIds: ["a1"],

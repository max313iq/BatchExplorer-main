...
>;
const mockedListPools = batchService.listPools as jest.MockedFunction<
  typeof batchService.listPools
>;
...
const makeAccount = (id: string, region = "eastus"): ManagedAccount => ({
  id,
  accountName: `acct-${id}`,
  resourceGroup: "rg",
  subscriptionId: "sub1",
...
const makeAccountInfo = (id: string, lpFree: number): AccountInfo => ({
  id,
  accountName: `acct-${id}`,
  subscriptionId: "sub1",
  region: "eastus",
...
function makeContext(): { ctx: AgentContext; store: MultiRegionStore } {
...
describe("PoolAgent.execute — basic pool creation", () => {
  it("creates pools across multiple accounts and reports completion", async () => {
    ...
    expect((sentConfig as Record<string, unknown>).targetDedicatedNodes).toBe(0);
  ...
  it("warns and skips when an accountId does not exist in the store", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
  ...
  it("records a failure with AzureRequestError details when createPool throws", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
    mockedCreatePool.mockRejectedValueOnce(
      new AzureRequestError("quota exceeded", 400, "QuotaExceeded", null),
    );
    ...
    const failures = result.summary.failures as Array<{ error: string }>;
  ...
  it("returns 'partial' status when some accounts succeed and others fail", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
    store.addAccount(makeAccount("a2", "westus"));
    mockedCreatePool
  ...
  it("falls back to default poolId 'pool' when poolConfig.id is missing", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
  ...
  it("uses an explicit token provider when supplied to the constructor", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
  ...
  it("cancel() halts the loop after the current iteration", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
    store.addAccount(makeAccount("a2", "westus"));
    ...
    mockedCreatePool.mockImplementationOnce(async () => {
      agent.cancel();
    });
...
describe("PoolAgent.executeWithFallback — smart pool creation", () => {
  it("skips accounts with zero free LP quota", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
  ...
  it("creates a pool, polls for steady state, then accounts for actual node usage", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
    ...
    expect(mockedCreatePool).toHaveBeenCalledTimes(1);
    const [, sentConfig] = mockedCreatePool.mock.calls[0];
    const cfg = sentConfig as Record<string, unknown>;
    ...
    expect((cfg.id as string).startsWith("gpu-nc6s-v3-")).toBe(true);
  ...
  it("falls back to the next VM size on a capacity error", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
    store.setAccountInfos([makeAccountInfo("a1", 12)]);
    ...
    mockedCreatePool
      .mockRejectedValueOnce(
        new AzureRequestError(
          "AllocationFailed: not enough capacity",
          500,
          "AllocationFailed",
          null,
        ),
  ...
  it("aborts on a non-retryable 400 error without trying other VM sizes", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
    store.setAccountInfos([makeAccountInfo("a1", 12)]);

    mockedCreatePool.mockRejectedValueOnce(
      new AzureRequestError("bad request", 400, "InvalidArgument", null),
    );
  ...
  it("aborts on a generic non-capacity error without fallback", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
    store.setAccountInfos([makeAccountInfo("a1", 12)]);
  ...
  it("skips a VM size that does not fit in the remaining quota", async () => {
    const { ctx, store } = makeContext();
    store.addAccount(makeAccount("a1"));
  ...
  it("warns and skips an unknown account id", async () => {
    const { ctx, store } = makeContext();
    const agent = new PoolAgent(ctx);
    const result = await agent.executeWithFallback({
      accountIds: ["nope"],
    ...
    expect(
      warns.some(
        (l) =>
          ...
          l.message.includes("No free LP quota"),
      ),
    ).toBe(true);
  });
});

...
const mockedListNodes = listNodes as jest.MockedFunction<typeof listNodes>;
const mockedPerformNodeAction =
  performNodeAction as jest.MockedFunction<typeof performNodeAction>;
const mockedRemoveNodes = removeNodes as jest.MockedFunction<typeof removeNodes>;

// ---------------------------------------------------------------------------
// Test fixtures + helpers
// ---------------------------------------------------------------------------

const makeAccount = (id: string, overrides: Partial<ManagedAccount> = {}): ManagedAccount => ({
  id,
  accountName: `acct-${id}`,
  resourceGroup: "rg",
...
const makeManagedNode = (id: string, overrides: Partial<ManagedNode> = {}): ManagedNode => ({
  id,
  accountId: "a1",
  accountName: "acct-a1",
  region: "eastus",
...
const makeBatchPool = (id: string, overrides: Partial<BatchPool> = {}): BatchPool => ({
  id,
  vmSize: "Standard_A1",
...
const makeBatchNode = (id: string, overrides: Partial<BatchNode> = {}): BatchNode => ({
  id,
  state: "idle",
...
const makeContext = (store: MultiRegionStore): AgentContext => ({
  store,
...
describe("NodeAgent", () => {
  ...
  describe("list_nodes (no actionType)", () => {
    it("happy path: aggregates nodes across accounts and pools, marks completed", async () => {
      ...
      const result = await agent.execute({ accountIds: ["a1"] });
    ...
    it("auto-discovers accountIds when none provided (uses created accounts)", async () => {
      const store = new MultiRegionStore();
    ...
    it("uses tokenProvider override when supplied", async () => {
      ...
      const agent = new NodeAgent(makeContext(store));
      await agent.execute({ accountIds: ["a1"], tokenProvider });

      expect(tokenProvider.getToken).toHaveBeenCalled();
      expect(mockedListPools).toHaveBeenCalledWith(
        expect.stringContaining("acct-a1.eastus.batch.azure.com"),
        "custom-token",
      );
    });

    it("error path: account-level failure marks status partial and increments errors", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      store.addAccount(makeAccount("a2"));

      mockedListPools.mockImplementation((endpoint: string) => {
      ...
      const agent = new NodeAgent(makeContext(store));
      const result = await agent.execute({ accountIds: ["a1", "a2"] });
    ...
    it("error path: pool-level listNodes failure is swallowed (no account error)", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
    ...
    it("skips unknown accountIds gracefully", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));

      mockedListPools.mockResolvedValue([]);
      const agent = new NodeAgent(makeContext(store));
      const result = await agent.execute({ accountIds: ["unknown-id"] });
    ...
    it("maps node-level errors and startTaskInfo failures into ManagedNode.error", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      ...
      const agent = new NodeAgent(makeContext(store));
      await agent.execute({ accountIds: ["a1"] });
    ...
    it("supports cancellation between account iterations", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
  ...
  describe("reboot action", () => {
    it("happy path: calls performNodeAction and updates node state to rebooting", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      ...
      const agent = new NodeAgent(makeContext(store));
      const result = await agent.execute({ actionType: "reboot", nodeIds: ["n1"] });

      expect(result.status).toBe("completed");
      expect(result.summary).toMatchObject({ total: 1, succeeded: 1, failed: 0 });
      expect(mockedPerformNodeAction).toHaveBeenCalledWith(
        expect.stringContaining("acct-a1.eastus.batch.azure.com"),
        "pool-1",
        "node-n1",
        "reboot",
    ...
    it("error path: failed reboot returns failed status and logs error", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      store.setNodes([makeManagedNode("n1")]);
      ...
      expect(store.getState().agentStatuses.node).toBe("error");
      expect(
        store.getState().agentLogs.some((l) => l.level === "error" && /reboot kaboom/.test(l.message)),
      ).toBe(true);
    });
  });

  describe("delete action (remove)", () => {
    it("happy path: calls removeNodes and removes node from store", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      ...
      const agent = new NodeAgent(makeContext(store));
      const result = await agent.execute({ actionType: "delete", nodeIds: ["n1"] });

      expect(result.status).toBe("completed");
      expect(mockedRemoveNodes).toHaveBeenCalledWith(
        expect.any(String),
        "pool-1",
        ["node-n1"],
        "batch-token",
    ...
    it("error path: rejection from removeNodes yields failed", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      store.setNodes([makeManagedNode("n1")]);
  ...
  describe("reimage action", () => {
    it("happy path: updates node state to reimaging", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      store.setNodes([makeManagedNode("n1")]);
      mockedPerformNodeAction.mockResolvedValue();
      ...
      const agent = new NodeAgent(makeContext(store));
      const result = await agent.execute({ actionType: "reimage", nodeIds: ["n1"] });

      expect(result.status).toBe("completed");
      expect(store.getState().nodes[0].state).toBe("reimaging");
    });

    it("error path: rejection yields failed status", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      store.setNodes([makeManagedNode("n1")]);
  ...
  describe("disableScheduling action", () => {
    it("happy path: sets schedulingState to disabled", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
    ...
    it("error path: failed call increments failed counter", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      store.setNodes([makeManagedNode("n1"), makeManagedNode("n2")]);
  ...
  describe("enableScheduling action", () => {
    it("happy path: sets schedulingState to enabled and uses tokenProvider", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
    ...
    it("error path: missing node is silently skipped, no failure recorded", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
  ...
  describe("cancellation", () => {
    it("breaks out of the action loop when cancel() is called between iterations", async () => {
      const store = new MultiRegionStore();
      store.addAccount(makeAccount("a1"));
      store.setNodes([
        makeManagedNode("n1"),

...
function makeContext(): AgentContext {
  ...
  const scheduler = {
    run: jest.fn(<T,>(_key: string, fn: () => Promise<T>) => fn()),
...
const okResult = (summary: Record<string, unknown> = {}): AgentResult => ({
  status: "completed",
  summary,
});
...
describe("OrchestratorAgent", () => {
  ...
  describe("execute() dispatch", () => {
    it("dispatches create_accounts to the provisioner", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      ...
      // payload merged in.
      expect(provisionerStub.execute).toHaveBeenCalledWith(
        expect.objectContaining(payload),
      );
      expect(result.status).toBe("completed");
    });

    it("dispatches filter_accounts to the filter agent", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      const payload = { filters: { regions: ["westus"] } };

      await orch.execute({ action: "filter_accounts", payload });
    ...
    it("dispatches list_nodes to the node agent", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      const payload = { accountIds: ["a1"] };

      await orch.execute({ action: "list_nodes", payload });

      // Orchestrator now spreads payload + appends a signal (audit fix #1).
      expect(nodeStub.execute).toHaveBeenCalledWith(
        expect.objectContaining({ accountIds: ["a1"] }),
      );
      const callArg = nodeStub.execute.mock.calls[0][0];
      expect(callArg.signal).toBeInstanceOf(AbortSignal);
    });

    it("dispatches node_action to the node agent", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      const payload = { actionType: "reboot", nodeIds: ["n1"] };

      await orch.execute({ action: "node_action", payload });

      expect(nodeStub.execute).toHaveBeenCalledWith(
        expect.objectContaining({ actionType: "reboot", nodeIds: ["n1"] }),
      );
      const callArg = nodeStub.execute.mock.calls[0][0];
      expect(callArg.signal).toBeInstanceOf(AbortSignal);
    });

    it("dispatches create_pools through filter then pool agent", async () => {
      const ctx = makeContext();
      filterStub.execute.mockResolvedValue(
        okResult({
      ...
      await orch.execute({ action: "create_pools", payload });
    ...
    it("creates and dispatches to the workflow agent for run_workflow", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      const cfg = {
        subscriptionId: "sub",
      ...
      await orch.execute({ action: "run_workflow", payload: cfg });
    ...
    it("returns failed for an unknown action", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      ...
      expect((result.summary as { error: string }).error).toMatch(
        /Unknown action/,
      );
    });
  });
  ...
  describe("status reporting", () => {
    it("transitions orchestrator status idle -> running -> completed on success", async () => {
      const ctx = makeContext();
      ...
      let midflightStatus: string | undefined;
      filterStub.execute.mockImplementationOnce(async () => {
    ...
    it("sets orchestrator status to error when the action throws", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      ...
      expect(result.status).toBe("failed");
      expect((result.summary as { error: string }).error).toMatch(/boom/);
      expect(ctx.store.getState().agentStatuses.orchestrator).toBe("error");
    });

    it("appends a log entry on dispatch and on error", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      ...
      expect(
        after.some(
          (l) => l.level === "error" && l.message.includes("kaboom"),
        ),
      ).toBe(true);
    });

    it("records a running activity that ends as failed on error", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      ...
      const filterActivity = activities.find(
        (a) => a.action === "filter_accounts",
      );
      expect(filterActivity).toBeDefined();
      expect(filterActivity?.status).toBe("failed");
      expect(filterActivity?.error).toMatch(/nope/);
    });
  });
  ...
  describe("cancellation", () => {
    ...
    it("propagates cancel to the workflow agent if one is active", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);

      // Hold the workflow execute open so the orchestrator instance is set.
      let releaseWorkflow!: (r: AgentResult) => void;
      workflowStub.execute.mockImplementationOnce(
        () =>
    ...
    it("marks an in-flight activity as cancelling when cancel(activityId) is called", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
      ...
      expect(activity?.status).toBe("cancelling");
    });
  });

  describe("error handling", () => {
    it("returns a failed result when a sub-agent throws", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);
    ...
    it("logs the error to agentLogs", async () => {
      const ctx = makeContext();
      const orch = new OrchestratorAgent(ctx);

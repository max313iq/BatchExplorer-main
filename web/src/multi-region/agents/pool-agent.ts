...
export class PoolAgent implements Agent {
  ...
  constructor(
    ...
    this._tokenProvider = tokenProvider ?? _ctx.getBatchAccessToken.bind(_ctx);
  }
   ...
   */
  private async _resolveToken(subscriptionId?: string): Promise<string> {
  ...
  async execute(params: Record<string, unknown>): Promise<AgentResult> {
    ...
    try {
      ...
      const idempotencyKey =
        typeof cfg.idempotencyKey === "string" && cfg.idempotencyKey.length > 0
          ? cfg.idempotencyKey
          : undefined;
      ...
      store.addLog({
        ...
        message: `${
          dryRun ? "[dry-run] " : ""
        }Starting pool creation on ${input.accountIds.length} accounts`,
      });
      ...
      const basePoolId = (input.poolConfig.id as string) ?? "pool";
      ...
      for (const accountId of input.accountIds) {
        ...
        const vmSizeForId =
          (input.poolConfig.vmSize as string | undefined) ?? "";
        const startTaskJson = (() => {
          try {
            return JSON.stringify(input.poolConfig.startTask ?? null);
          } catch {
            return "null";
          }
        })();
        ...
        const poolId = idempotencyKey
          ? `${basePoolId}-${shortHash8(
              `${idempotencyKey}|${vmSizeForId}|${startTaskJson}`,
            )}`
          : basePoolId;
        const poolConfig = {
          ...input.poolConfig,
        ...
        try {
          ...
          await scheduler.run(accountId, async () => {
            ...
            const token = await this._resolveToken(account.subscriptionId);
            ...
            if (dryRun) {
              ...
              await createPool(endpoint, poolConfig, token);
            }
          });
          ...
          store.addLog({
            ...
            } pool "${poolId}" on ${account.accountName} (${account.region})`,
          });
          created++;
          this._audit.record({
  ...
  async executeWithFallback(params: {
    ...
    try {
      ...
      store.addLog({
        ...
        message: `${
          dryRun ? "[dry-run] " : ""
        }Smart pool creation on ${accountIds.length} accounts with ${vmSizes.length} VM size(s)`,
      });
      ...
      const accountsWithQuota = accountIds.filter((id) => {
        ...
        return (info?.lowPriorityCoresFree ?? 0) > 0;
      });
      if (accountsWithQuota.length === 0) {
        ...
        const noQuotaFailures = accountIds.map((id) => {
          ...
          const vmFamily = vmSizes[0] ?? "?";
          const requestedSlots = 1;
          const availableSlots = info?.lowPriorityCoresFree ?? 0;
          return {
            kind: "no_quota" as const,
            account: acct?.accountName ?? id,
            accountName: acct?.accountName ?? id,
            region: acct?.region ?? "?",
            vmFamily,
        ...
        return {
          ...
          summary: {
            ...
            created: 0,
            failed: noQuotaFailures.length,
            failures: noQuotaFailures,
            ...(dryRun ? { dryRun: true } : {}),
          },
        };
      }
    ...
    for (const accountId of accountIds) {
      ...
      for (let vmIdx = 0; vmIdx < vmSizes.length; vmIdx++) {
        if (this._isCancelled(signal)) break;
        if (abortRun) break;
        if (regionPermanentlyBlocked) break;
        ...
        try {
          ...
          await scheduler.run(accountId, async () => {
            ...
            if (dryRun) {
              ...
              await createPool(endpoint, currentConfig, token);
            }
          });
          ...
          store.addLog({
            ...
            message: `${
              dryRun ? "[dry-run] would have created" : "Created"
            } pool "${currentPoolId}" (${vmSize}, ${maxNodes} LP nodes) on ${account.accountName}`,
          });
          totalCreated++;
          accountCreated = true;
          this._audit.record({
          ...
          const accountEndpointStr = accountEndpoint(
            account.accountName,
            account.region,
          );
          const resizeResult = await this._waitForPoolSteady(
            accountEndpointStr,
            currentPoolId,
            await this._resolveToken(account.subscriptionId),
      ...
      if (!accountCreated && !regionPermanentlyBlocked && !abortRun) {
        const alreadyHasEntryForAccount = failures.some(
          (f) => f.accountName === account.accountName,
        );
        if (!alreadyHasEntryForAccount) {
          ...
          failures.push({
            ...
            region: account.region,
            error:
              "All VM sizes exhausted — capacity/quota insufficient or every size blacklisted",
            kind: "exhausted",
          });
          totalFailed++;
        }
   ...
   */
  static POLL_INTERVAL_MS = 15000; // overridable from tests
  private async _waitForPoolSteady(
    ...
    for (let i = 0; i < maxPolls; i++) {
      ...
      try {
        ...
        const freshToken = await this._resolveToken(subscriptionId);
        ...
        const currentDedicated = (pool as any).currentDedicatedNodes ?? 0;
  ...
  private _addPoolInfoToStore(
    ...
    const newPoolInfo: PoolInfo = {
      ...
      targetLowPriorityNodes: (config.targetLowPriorityNodes as number) ?? 0,

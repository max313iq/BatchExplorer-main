...
const makePool = (id: string, accountId = "a1"): ManagedPool => ({
  id,
  accountId,
  poolId: `pool-${id}`,
  provisioningState: "pending",
...
const makeNode = (id: string): ManagedNode => ({
  id,
  accountId: "a1",
  accountName: "acct-a1",
  region: "eastus",
...
const makeAuditEntry = (id: string): AuditEntry => ({
  id,
...
const makeAzureAccount = (homeAccountId: string): AzureLoginAccount => ({
  homeAccountId,
...
describe("MultiRegionStore — construction & subscriptions", () => {
  ...
  it("listener errors do not break notification fan-out", () => {
    ...
    store.onChange(ok);
    expect(() =>
      store.setSubscriptions([{ subscriptionId: "s", displayName: "x" }]),
    ).not.toThrow();
...
describe("MultiRegionStore — accounts / pools / nodes", () => {
  ...
  it("setNodes, updateNode, removeNode manage nodes slice", () => {
    ...
    expect(store.getState().nodes.find((n) => n.id === "n1")?.state).toBe(
      "running",
    );
  ...
  it("retryFailedAccounts and retryFailedPools reset failed entries to pending", () => {
    const store = new MultiRegionStore();
    store.addAccount({ ...makeAccount("a1"), provisioningState: "failed" });
    store.addAccount({ ...makeAccount("a2"), provisioningState: "created" });
    const ids = store.retryFailedAccounts();
    expect(ids).toEqual(["a1"]);
    expect(store.getState().accounts.find((a) => a.id === "a1")?.provisioningState).toBe(
      "pending",
    );

    store.addPool({ ...makePool("p1"), provisioningState: "failed" });
...
describe("MultiRegionStore — user preferences", () => {
  ...
  it("saveUserPreferences performs a partial merge into localStorage", () => {
    ...
    expect(after.autoRefreshEnabled).toBe(
      DEFAULT_USER_PREFERENCES.autoRefreshEnabled,
    );
    ...
    expect(JSON.parse(raw as string).sidebarCollapsed).toBe(true);
  });
...
describe("MultiRegionStore — Azure accounts & tenants", () => {
  it("setAzureAccounts replaces; upsertAzureAccount inserts then updates", () => {
    ...
    const updated = { ...makeAzureAccount("h1"), name: "Updated Name" };
    store.upsertAzureAccount(updated);
    expect(store.getState().azureAccounts).toHaveLength(2);
    expect(
      store.getState().azureAccounts.find((a) => a.homeAccountId === "h1")?.name,
    ).toBe("Updated Name");
  });
  ...
  it("setTenantUsers / addTenantUser / updateTenantUser maintain per-tenant lists", () => {
    ...
    const u1Updated: GraphUser = { ...u1, displayName: "Renamed" };
    store.addTenantUser("t1", u1Updated);
    expect(store.getState().tenantUsers["t1"]).toHaveLength(2);
    expect(
      store.getState().tenantUsers["t1"].find((u) => u.id === "u1")?.displayName,
    ).toBe("Renamed");

    store.updateTenantUser("t1", "u2", { displayName: "U2 Updated" });
    expect(
      store.getState().tenantUsers["t1"].find((u) => u.id === "u2")?.displayName,
    ).toBe("U2 Updated");
  });
...
describe("MultiRegionStore — throttle stats", () => {
  it("setThrottleStatusEntry stores a new entry; pushThrottleTransition appends history", () => {
    const store = new MultiRegionStore();
    const entry: ThrottleStatusEntry = {
      state: "closed",
      refillPerSec: 10,
      recentThrottles: 0,
    };
...
describe("MultiRegionStore — activities", () => {
  ...
  it("addChildActivity links to parent", () => {
    ...
    expect(parentEntry?.children).toContain(child);
  });
  ...
  it("pauseActivity and resumeActivity toggle status; isActivityPaused reflects state", () => {
    ...
    expect(store.getState().activities.find((a) => a.id === id)?.status).toBe(
      "paused",
    );

...
// We expose the live `location.search` via a sibling hook so tests can assert
// on the URL produced by the setter.
// ---------------------------------------------------------------------------
...
function useHarness<T extends Record<string, unknown>>(
    initial: T,
    options?: Parameters<typeof useUrlState>[1],
): HarnessResult<T> {
    const [state, setState] = useUrlState(
        initial as Parameters<typeof useUrlState>[0],
        options,
    );
...
describe("useUrlState", () => {
    ...
    describe("initial values", () => {
        it("returns initial values when the URL has no params", () => {
            const { result } = renderHook(
                () => useHarness({ q: "", page: "1", tags: [] as string[] }),
                { wrapper: makeWrapper(["/"]) },
            );
        ...
        it("reads existing params from the URL on mount", () => {
            const { result } = renderHook(
                () => useHarness({ q: "", page: "1" }),
                { wrapper: makeWrapper(["/?q=hello&page=3"]) },
            );
        ...
        it("deserializes array values from comma-delimited strings on mount", () => {
            const { result } = renderHook(
                () => useHarness({ tags: [] as string[] }),
                { wrapper: makeWrapper(["/?tags=a,b,c"]) },
            );
    ...
    describe("empty value stripping", () => {
        ...
        it("removes a key when value is undefined", () => {
            const { result } = renderHook(
                () => useHarness({ q: "" as string | undefined }),
                { wrapper: makeWrapper(["/?q=hello"]) },
            );
    ...
    describe("keys option", () => {
        it("only reads keys listed in the `keys` option", () => {
            const { result } = renderHook(
                ...
                { wrapper: makeWrapper(["/?q=hello&page=5"]) },
            );
    ...
    describe("custom serialize / deserialize", () => {
        it("uses a custom serializer when writing to the URL", () => {
            const serialize = jest.fn((v: unknown) => `S:${String(v)}`);
            const { result } = renderHook(
                () => useHarness({ q: "" }, { serialize }),
                { wrapper: makeWrapper(["/"]) },
            );
        ...
        it("uses a custom deserializer when reading from the URL", () => {
            const deserialize = jest.fn(
                (raw: string, key: string) => `D[${key}]:${raw}`,
            );
            const { result } = renderHook(
                () => useHarness({ q: "" }, { deserialize }),
                { wrapper: makeWrapper(["/?q=raw"]) },
            );
        ...
        it("supports setState(prev => ...) form", () => {
            ...
            act(() => {
                result.current.setState((prev) => ({
                    count: String(parseInt(prev.count as string, 10) + 1),
                }));
            });
            ...
            act(() => {
                result.current.setState((prev) => ({
                    count: String(parseInt(prev.count as string, 10) + 5),
                }));
            });
...
describe("useUrlParam", () => {
    function useParamHarness(key: string, initial = "") {

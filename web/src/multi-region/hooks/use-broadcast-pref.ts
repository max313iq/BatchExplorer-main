...
interface BroadcastMessage<T> {
  /** Origin id of the sending hook instance; used to filter self-echoes. */
  origin: string;
  /** Storage key, doubles as the pref name. */
  key: string;
  /** The new value. */
  value: T;
}
...
export function useBroadcastPref<T>(
  ...
  const [state, setState, reset] = usePersistedState<T>(key, initialValue, options);

  const originRef = React.useRef<string | null>(null);
  if (originRef.current === null) originRef.current = nextOriginId();
  ...
  React.useEffect(() => {
    ...
    const onMessage = (e: MessageEvent<BroadcastMessage<T>>) => {
      const msg = e.data;
      if (!msg) return;
      if (msg.origin === originRef.current) return; // self-echo
  ...
  const setStateAndBroadcast = React.useCallback<

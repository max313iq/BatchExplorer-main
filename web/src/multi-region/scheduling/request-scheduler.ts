 ...
 */
import { globalRetryBudget, type RetryBudget } from "./retry-budget";
 ...
 */
export const DEFAULT_SCHEDULER_OPTIONS: RequestSchedulerOptions = {
  concurrency: 1,
  delayMs: 2000,
  retryAttempts: 5,
...
export class RequestScheduler {
  ...
  private readonly _retryBudget: RetryBudget | null;
  ...
  constructor(options: RequestSchedulerOptions = {}) {
    ...
    const backoff =
      options.retryBackoffSeconds && options.retryBackoffSeconds.length > 0
    ...
    this._retryBudget =
      options.retryBudget === null
        ? null
        : (options.retryBudget ?? globalRetryBudget());
  }

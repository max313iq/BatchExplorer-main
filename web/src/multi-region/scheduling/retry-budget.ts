...
export class RetryBudget {
  ...
  constructor(opts?: RetryBudgetOptions) {
    this._limit = Math.max(1, opts?.limit ?? DEFAULT_LIMIT);
    this._windowMs = Math.max(1, opts?.windowMs ?? DEFAULT_WINDOW_MS);
  ...
  private _evict(): void {
    ...
    let firstInWindow = 0;
    while (
      firstInWindow < this._stamps.length &&
...
let _shared: RetryBudget | null = null;
 ...
 */
export function globalRetryBudget(): RetryBudget {
  if (!_shared) {
 ...
 */
export function _setGlobalRetryBudgetForTest(b: RetryBudget | null): void {
  _shared = b;
}

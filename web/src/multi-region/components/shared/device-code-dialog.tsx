...
type Phase =
  | { kind: "starting" }
  | { kind: "polling"; challenge: DeviceCodeChallenge; polls: number }
  | { kind: "success"; upn?: string }
 ...
 * v2 claim shape.
 */
function pickDisplayName(claims: Record<string, unknown> | null | undefined): string | undefined {
...
export const DeviceCodeDialog: React.FC<DeviceCodeDialogProps> = ({
  ...
  const [copyFeedback, setCopyFeedback] = React.useState<"code" | "url" | null>(null);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const challenge = await startDeviceCodeFlow({
          tenantId,
          clientId,
          scopes:
        ...
        const pollTicker = window.setInterval(() => {
          setPhase((p) =>
            p.kind === "polling" ? { ...p, polls: p.polls + 1 } : p,
          );
        }, Math.max(1, challenge.interval) * 1000);

        try {
          const result = await pollDeviceCodeFlow(challenge, {
            tenantId,
            clientId,
            signal: controller.signal,
          });
  ...
  const handleOpenInTab = (url: string): void => {
    try {
  ...
  const remainingMs =
    phase.kind === "polling" ? phase.challenge.expires_at - now : 0;

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Device code sign-in</DialogTitle>
          <DialogDescription>
            Visit the URL on another device (your phone, or another browser
            tab) and enter the code below to complete sign-in.
          </DialogDescription>
        </DialogHeader>
        ...
        {phase.kind === "polling" && (
          <div className="flex flex-col gap-4">
            {/* User code — big, monospaced, copy button. Operator types this
                on the secondary device, so legibility matters more than
                anything else in this dialog. */}
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center gap-2">
                <code
                  className="flex-1 rounded-md border border-border bg-card/40 px-3 py-3 text-center font-mono text-2xl font-bold tracking-[0.3em] text-foreground"
                  aria-label="Device code"
                >
                  {phase.challenge.user_code}
                </code>
                <Button
                  variant="outline"
                  size="icon"
                  onClick={() => handleCopy(phase.challenge.user_code, "code")}
            ...
            {/* Verification URL — copy + open-in-tab. The open-in-tab button
                may be blocked when the operator chose device-code BECAUSE
                popups are blocked — copy is the reliable fallback. */}
            <div className="flex flex-col gap-1.5">
              <span className="text-2xs font-semibold uppercase tracking-wider text-muted-foreground">
                Verification URL
              </span>
              <div className="flex items-center gap-2">
                <code
                  className="flex-1 truncate rounded-md border border-border bg-card/40 px-3 py-2 font-mono text-xs text-foreground"
                  title={phase.challenge.verification_uri}
                >
                  {phase.challenge.verification_uri}
                </code>
                <Button
                  variant="outline"
                  size="icon-sm"
                  onClick={() =>
                    handleCopy(phase.challenge.verification_uri, "url")
                  }
                ...
                <Button
                  variant="outline"
                  size="icon-sm"
                  onClick={() => handleOpenInTab(phase.challenge.verification_uri)}
            ...
            {/* Status row — discreet spinner + poll counter + countdown. */}
            <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
              <span className="flex items-center gap-2">
                <Loader2
                  ...
                  aria-hidden="true"
                />
                Waiting for sign-in… ({phase.polls} poll
                {phase.polls === 1 ? "" : "s"})
              </span>
              <span
                className={cn(
        ...
        <DialogFooter>
          {phase.kind === "error" && (
            <Button variant="default" size="sm" onClick={handleTryAgain}>
              Try again
            </Button>
          )}
          <Button
            ...
            disabled={phase.kind === "success"}
          >
            {phase.kind === "success" ? "Closing…" : "Cancel"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>

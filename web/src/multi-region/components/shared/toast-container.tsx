...
const TYPE_CLASS: Record<ToastNotification["type"], string> = {
...
export const ToastContainer: React.FC = () => {
  ...
  return (
    <div
      className="pointer-events-none fixed right-4 top-14 z-[10000] flex w-full max-w-sm flex-col gap-2"
      role="status"
      aria-live="polite"
    >
      {(state.notifications ?? [])
        ...
        .map((n) => {
          ...
          return (
            <div
              key={n.id}
              className={cn(
                "pointer-events-auto flex items-start gap-3 rounded-md border px-3 py-2.5 shadow-lg",
            ...
            >
              ...
              <button
                ...
                className="-mr-0.5 -mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded text-current/70 transition-colors hover:bg-current/10 hover:text-current focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current/40"
              >

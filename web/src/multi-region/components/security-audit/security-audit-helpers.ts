...
export const SEVERITY_BADGE_VARIANT: Record<
  Severity,
  "destructive" | "warning" | "info" | "outline" | "secondary"
> = {
...
// ----------------------------------------------------------------------
// Input shapes — narrow subsets of the ARM payloads we consume.
// We deliberately type only the fields we touch; unknown extras are
// permitted via the `Record<string, unknown>` index access.
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Resource-id parser. Pulls the resource group out of an ARM id so we
// can render it in the finding row without an extra API call. Falls
// back to the empty string on malformed ids (defensive — we never
// throw out of the audit).
// ----------------------------------------------------------------------
...
export function parseArmId(armId: string): ParsedArmId {
  ...
  const providerNamespace =
    provIdx >= 0 && provIdx + 1 < parts.length ? parts[provIdx + 1]! : "";
  const resourceType =
    provIdx >= 0 && provIdx + 2 < parts.length ? parts[provIdx + 2]! : "";
  const resourceName =
    provIdx >= 0 && provIdx + 3 < parts.length ? parts[provIdx + 3]! : "";
  return {
 ...
 */
export function portalUrlFor(armId: string): string {
 ...
 */
export function evaluateStorageAccount(
  sa: StorageAccountResource,
  subscriptionName: string,
): Finding[] {
  const out: Finding[] = [];
  ...
  const defaultAction = (acls.defaultAction ?? "").toLowerCase();
  ...
  if (
    ...
    out.push(
      makeFinding(ctx, "storage", {
        ruleId: "storage.publicly-accessible",
        title: "Publicly accessible blob storage",
        description:
          "Public blob access is permitted AND the network firewall defaults to Allow.",
        whyItMatters:
          "Anonymous internet readers can enumerate and download any blob container marked Public/Blob/Container. MicroBurst's Invoke-EnumerateAzureBlobs walks exactly this surface — every account that matches this pattern is one container-misconfigure away from a public data leak.",
        remediation:
          'Disable account-level public blob access: az storage account update -n ' +
          sa.name +
          ' -g ' +
    ...
    out.push(
      makeFinding(ctx, "storage", {
        ...
        description:
          "allowBlobPublicAccess=true — a container can be flipped Public without re-checking firewall.",
        whyItMatters:
          "Defense in depth: the account-level flag should explicitly block public blob access so a per-container slip (someone setting a container to PublicAccess=Container) is contained. Microsoft's storage hardening baseline (Azure Security Benchmark v3 / NIST 800-53 AC-6) calls this out as a required deny.",
        remediation:
          "az storage account update -n " +
          sa.name +
          " -g " +
          parsed.resourceGroup +
  ...
  if (props.supportsHttpsTrafficOnly === false) {
    out.push(
      makeFinding(ctx, "storage", {
        ...
        description:
          "supportsHttpsTrafficOnly=false — SAS tokens and account keys can leak over plaintext.",
        whyItMatters:
          "Without HTTPS-only enforcement, a misconfigured client (or an attacker on the same path) can downgrade the request to HTTP. Both the storage SAS query string AND the shared-access key end up in cleartext — these are bearer credentials, so a single capture is full compromise.",
        remediation:
          "az storage account update -n " +
          sa.name +
          " -g " +
  ...
  if (
    props.minimumTlsVersion === "TLS1_0" ||
    ...
    out.push(
      makeFinding(ctx, "storage", {
        ...
        whyItMatters:
          "TLS 1.0/1.1 are deprecated by the IETF (RFC 8996), PCI-DSS, and Microsoft's own retirement policy. Leaving them enabled lets a MITM force a downgrade and exploit BEAST / POODLE / Lucky13. No modern client requires < TLS 1.2.",
        remediation:
          "az storage account update -n " +
          sa.name +
          " -g " +
  ...
  if (!oauthDefault && !sharedKeyBlocked) {
    out.push(
      makeFinding(ctx, "storage", {
        ...
        description:
          "defaultToOAuthAuthentication is not true and allowSharedKeyAccess is not false — clients fall back to account keys.",
        whyItMatters:
          "Account keys are bearer credentials with full control of the storage account. They can't be audited per-user, can't be MFA-protected, and rotation is painful. Preferring AAD/OAuth means every access ties back to a real principal in the audit log.",
        remediation:
          "Set defaultToOAuthAuthentication=true and, if your workloads support it, set allowSharedKeyAccess=false. az storage account update -n " +
          sa.name +
          " -g " +
          parsed.resourceGroup +
  ...
  if (
    ...
    out.push(
      makeFinding(ctx, "storage", {
        ...
        description:
          "networkAcls.defaultAction=Allow with no IP or VNet rules — reachable from any IP.",
        whyItMatters:
          "Even with HTTPS + AAD, an attacker who lands an account key (e.g. via a leaked DevOps variable) can hit the account from anywhere on the public internet. Tying access to a known IP range / VNet shrinks the credential's blast radius.",
        remediation:
          "az storage account network-rule add for each trusted source, then az storage account update --default-action Deny.",
        severity: "info",
      }),
    );
  }
 ...
 */
export function evaluateKeyVault(
  kv: KeyVaultResource,
  subscriptionName: string,
): Finding[] {
  const out: Finding[] = [];
  ...
  if (props.enablePurgeProtection !== true) {
    out.push(
      makeFinding(ctx, "keyvault", {
        ruleId: "keyvault.no-purge-protection",
        title: "Purge protection disabled",
        description:
          "enablePurgeProtection is not true — vault contents can be hard-deleted before the retention window.",
        whyItMatters:
          "Without purge protection an attacker (or a panicked admin) can soft-delete then immediately purge keys/secrets/certificates, bypassing the retention window. Recovery from backup may not be possible if the vault held a CMK that wrapped a Storage / SQL encryption key — those resources become unreadable.",
        remediation:
          "az keyvault update -n " +
          kv.name +
          " -g " +
          parsed.resourceGroup +
  ...
  if (props.enableSoftDelete !== true) {
    out.push(
      makeFinding(ctx, "keyvault", {
        ...
        description:
          "enableSoftDelete is not true — deletes are immediate with no recovery window.",
        whyItMatters:
          "Soft delete is the only undo button for accidental or malicious secret/key deletion. Without it, a single bad pipeline run that wipes the vault is unrecoverable. Microsoft made soft-delete mandatory on new vaults in 2020 — pre-2020 vaults must be upgraded explicitly.",
        remediation:
          "az keyvault update -n " +
          kv.name +
          " -g " +
  ...
  if (publicNetworkAccessOn && defaultAction === "allow") {
    out.push(
      makeFinding(ctx, "keyvault", {
        ...
        description:
          "publicNetworkAccess=Enabled and networkAcls.defaultAction=Allow — reachable from any IP.",
        whyItMatters:
          "A leaked credential, OAuth token, or SAS that grants the vault data-plane role can be used from anywhere on the public internet. Locking the vault to private endpoints or a small VNet/IP allowlist drops the credential's usable blast radius.",
        remediation:
          'az keyvault update -n ' +
          kv.name +
          ' -g ' +
  ...
  if (props.enableRbacAuthorization !== true) {
    out.push(
      makeFinding(ctx, "keyvault", {
        ...
        description:
          "enableRbacAuthorization is not true — vault uses access-policies, which Microsoft is deprecating.",
        whyItMatters:
          "Access policies live on the vault resource itself, can't be audited or assigned at scale via Azure Policy/Bicep, and don't compose with PIM/Conditional Access. RBAC mode uses standard Azure role assignments — same tooling, same audit trail as every other Azure resource.",
        remediation:
          "Migrate to RBAC: az keyvault update -n " +
          kv.name +
          " -g " +
          parsed.resourceGroup +
  ...
  if (
    typeof props.softDeleteRetentionInDays === "number" &&
    ...
    out.push(
      makeFinding(ctx, "keyvault", {
        ...
        whyItMatters:
          "A short retention window means an accidental delete that goes unnoticed for a weekend (or a long deploy freeze) becomes unrecoverable. Microsoft's default is 90 days for a reason — recovery operations rarely happen the same day.",
        remediation:
          "Retention is immutable on most vaults — recreate the vault with --retention-days 90.",
        severity: "medium",
      }),
    );
  }
  ...
  if (accessPolicies.length > 10) {
    out.push(
      makeFinding(ctx, "keyvault", {
        ...
        description:
          "More than 10 access-policy entries — review for stale principals (left-the-company, deleted-SPN).",
        whyItMatters:
          "Access policies don't auto-clean when the principal is deleted in AAD — the entry stays as a dangling objectId. A new AAD principal that gets the same objectId (rare but possible) would inherit access. Wide policy sets are also a signal that the team should migrate to RBAC for proper auditability.",
        remediation:
          "Audit each policy entry — `az keyvault show -n " +
          kv.name +
  ...
  const wildcardPolicy = accessPolicies.find((p) => {
    ...
    const fams: Array<string[] | undefined> = [
      perms.keys,
      perms.secrets,
      perms.certificates,
      perms.storage,
    ...
    return fams.some(
      (arr) =>
        Array.isArray(arr) &&
        arr.some(
          (perm) => perm.toLowerCase() === "all" || perm === "*",
        ),
    );
  });
  if (wildcardPolicy) {
    out.push(
      makeFinding(ctx, "keyvault", {
        ...
        description: `At least one access policy grants ["all"] / ["*"] on keys, secrets, certs, or storage (objectId ${wildcardPolicy.objectId ?? "unknown"}).`,
        whyItMatters:
          'A wildcard permission set grants every operation in the family — including destructive ones (purge, backup-restore, regenerate). Most workloads only need a narrow slice (e.g. "get + list" on secrets). Wildcard is the Key Vault equivalent of "Contributor on /" — almost never the right scope.',
        remediation:
          "Replace wildcard with the explicit set the workload needs. Example for a secrets reader: --secret-permissions get list (drop set/delete/purge/recover/backup/restore).",
        severity: "info",
      }),
    );
  }
...
export function summarizeFindings(
  ...
  scanned: { storage: number; vaults: number },
): FindingsSummary {
  let critical = 0;
  let high = 0;
  let medium = 0;

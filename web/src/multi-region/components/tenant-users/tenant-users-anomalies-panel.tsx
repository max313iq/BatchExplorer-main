...
import type { DirectoryRole } from "../../services/types";
...
function portalUserRolesUrl(tenantId: string, userId: string): string {
  return (
    `https://entra.microsoft.com/#@${encodeURIComponent(tenantId)}` +
...
// =============================================================================
// Detector — finding shape
// =============================================================================

type Severity = "critical" | "high" | "medium";

interface GuestAdminFinding {
  kind: "guest-admin";
  severity: Severity;
  user: AnomalyUserRow;
  roles: DirectoryRole[];
  tier0Roles: DirectoryRole[];
}

interface DisabledWithSubsFinding {
  kind: "disabled-with-subs";
  severity: Severity;
  user: AnomalyUserRow;
}

interface RapidCreateDeleteFinding {
  ...
  deletedAt: string;
  deltaHours: number;
  /** Optional matching deletedRow — used for the portal deep-link. */
  deletedRow?: DeletedUserRow;
}

type AnyFinding =
  | GuestAdminFinding
  | DisabledWithSubsFinding
  | RapidCreateDeleteFinding;
 ...
 */
function detectDisabledWithSubs(
  rows: ReadonlyArray<AnomalyUserRow>,
): DisabledWithSubsFinding[] {
  const out: DisabledWithSubsFinding[] = [];
  for (const u of rows) {
    if (u.accountEnabled) continue;
    if (u.subscriptionCount <= 0) continue;
    out.push({
      kind: "disabled-with-subs",
 ...
 */
function detectRapidCreateDelete(
  auditEntries: ReadonlyArray<AuditEntry>,
  deletedRows: ReadonlyArray<DeletedUserRow>,
): RapidCreateDeleteFinding[] {
  if (deletedRows.length === 0) return [];
  // Index deletes by lowercase UPN.
  const deletedByUpn = new Map<string, DeletedUserRow>();
  for (const d of deletedRows) {
    const k = (d.userPrincipalName ?? "").toLowerCase();
    if (!k) continue;
...
export const TenantUsersAnomaliesPanel: React.FC<
  TenantUsersAnomaliesPanelProps
> = ({ tenantId, homeAccountId, actor, rows, deletedRows, auditEntries }) => {
  const [guestAdminFindings, setGuestAdminFindings] = React.useState<
    GuestAdminFinding[]
  >([]);
  const [guestProbeAt, setGuestProbeAt] = React.useState<number | null>(null);
  ...
  React.useEffect(() => {
    ...
    if (rowFingerprintRef.current !== guestFingerprint) {
      rowFingerprintRef.current = guestFingerprint;
      if (guestProbeAt !== null) setProbeStale(true);
    }
  }, [guestFingerprint, guestProbeAt]);

  // ---- Cheap detectors run every render ----------------------------------
  const disabledWithSubsFindings = React.useMemo(
    () => detectDisabledWithSubs(rows),
    [rows],
  );
  const rapidFindings = React.useMemo(
    () => detectRapidCreateDelete(auditEntries, deletedRows),
    [auditEntries, deletedRows],
  );
  ...
  const handleHunt = React.useCallback(async () => {
    ...
    try {
      ...
      if (seq !== huntSeqRef.current) return;
      const map = await getUserDirectoryRolesBatched(
        tenantId,
        guests.map((g) => g.id),
        token,
      );
      if (seq !== huntSeqRef.current) return;
      const found: GuestAdminFinding[] = [];
      for (const g of guests) {
        const roles = map.get(g.id) ?? [];
        if (roles.length === 0) continue;
        const tier0 = roles.filter((r) =>
          TIER0_ROLE_TEMPLATE_IDS.has(r.roleTemplateId),
        );
        // Even a non-Tier-0 role on a guest is unusual — defenders should
        // see it. Tier-0 escalates to critical.
        const severity: Severity = tier0.length > 0 ? "critical" : "high";
        found.push({
          kind: "guest-admin",
          severity,
          user: g,
      ...
      if (seq === huntSeqRef.current) setProbing(false);
    }
  }, [tenantId, homeAccountId, actor, rows]);

  // ---- Aggregate counts + sort ------------------------------------------
  const allFindings: AnyFinding[] = React.useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      const kindOrder = (k: AnyFinding["kind"]) =>
        k === "guest-admin" ? 0 : k === "rapid-create-delete" ? 1 : 2;
  ...
  const counts = React.useMemo(() => {
    ...
    for (const f of allFindings) c[f.severity] += 1;
    return c;
  }, [allFindings]);
    ...
    <div className="flex flex-col gap-3 rounded-lg border border-border bg-card px-4 py-3 shadow-sm">
          ...
          {guestProbeAt !== null && (
            <span
              className={cn(
                ...
                probeStale ? "text-warning" : "text-muted-foreground",
              )}
              title={new Date(guestProbeAt).toLocaleString()}
            >
              {probeStale ? "stale · " : ""}
              guest probe{" "}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  disabled={probing || !tenantId || !homeAccountId}
                  aria-label="Run guest-admin probe"
                >
                  {probing ? (
              ...
              <TooltipContent side="bottom">
                Issues a Graph $batch GET /users/&#123;id&#125;/memberOf for
                each guest in the current view (≤ 20 sub-requests per call).
                No state-changing primitive is invoked. Corpus:
                _bypass_tenant_switch.md §11 / §12.
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
      ...
      {probeError && (
        <Alert variant="destructive" className="py-2">
          ...
          <AlertDescription className="text-2xs">{probeError}</AlertDescription>
        </Alert>
      )}
      ...
      {allFindings.length === 0 ? (
        <EmptyState
          ...
          description={
            guestProbeAt === null
              ...
              : "Disabled-with-subs and rapid create→delete returned no findings on the current view, and no guest holds a directory role. Re-run the hunt after a filter change to refresh the guest-admin signal."
          }
        />
      ) : (
        <ul
        ...
        >
          {allFindings.map((f, idx) => (
            ...
            >
              <Badge
                variant={SEVERITY_BADGE[f.severity].variant}
                className="shrink-0 uppercase"
              >
                {SEVERITY_BADGE[f.severity].label}
              </Badge>
      ...
      <p className="border-t border-border pt-2 text-2xs text-muted-foreground">
        Corpus references:{" "}
        <code className="font-mono">_bypass_tenant_switch.md</code> §11 / §12
        item 2 (stale-guest privesc),{" "}
        <code className="font-mono">_bypass_modify_delete.md</code> §4.7 (rapid
        create→delete cleanup),{" "}
        <code className="font-mono">_bypass_role_grant.md</code> (Tier-0 role
        ids). State-changing primitives (PATCH/DELETE/POST) are deliberately
        NOT invoked here.
      </p>
    </div>
  );
...
function renderFindingBody(f: AnyFinding, tenantId: string | null): JSX.Element {
  switch (f.kind) {
    case "guest-admin":
      return (
        <>
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <span className="truncate text-xs text-foreground">
              ...
              {f.tier0Roles.length > 0 && (
                <span className="ml-1.5 text-2xs text-destructive">
                  · holds {f.tier0Roles.length} Tier-0 role
                  {f.tier0Roles.length === 1 ? "" : "s"}
                </span>
              )}
            </span>
            <span className="truncate font-mono text-2xs text-muted-foreground">
              {f.user.userPrincipalName || "(no UPN)"}
            </span>
            <span className="mt-0.5 flex flex-wrap gap-1 text-2xs">
          ...
          <div className="flex items-center gap-1">
            {tenantId && (
              <Button
                ...
                size="xs"
                asChild
                aria-label={`Open role assignments for ${f.user.displayName} in Entra Portal`}
              >
                <a
                  href={portalUserRolesUrl(tenantId, f.user.id)}
                  target="_blank"
            ...
            <CopyButton
              value={f.user.userPrincipalName || f.user.id}
              ariaLabel={`Copy UPN for ${f.user.displayName}`}
              alwaysVisible={false}
              iconSize={14}
            />
          </div>
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <span className="truncate text-xs text-foreground">
              ...
              <span className="ml-1.5 text-2xs text-muted-foreground">
                · disabled but owns {f.user.subscriptionCount} subscription
            ...
            <span className="mt-0.5 inline-flex items-center gap-1 text-2xs text-muted-foreground">
              ...
              ghost-owner pattern — clean up the role assignments or
              re-enable, never leave both in place
            </span>
          </div>
          <CopyButton
            value={f.user.userPrincipalName || f.user.id}
    ...
    case "rapid-create-delete":
      return (
        <>
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <span className="truncate text-xs text-foreground">
              <span className="font-mono">{f.upn}</span>
              <span className="ml-1.5 text-2xs text-destructive">
            ...
            <span className="text-2xs text-muted-foreground tabular-nums">
              created {formatRelativeTime(f.createdAt)} · soft-deleted{" "}
              {formatRelativeTime(f.deletedAt)}
            </span>
            <span className="mt-0.5 inline-flex items-center gap-1 text-2xs text-muted-foreground">
              local create_user audit entry + Graph deletedItems match —
              attacker scaffolding cleanup is a known tactic (corpus §4.7)
            </span>
          </div>
          <CopyButton
            value={f.upn}

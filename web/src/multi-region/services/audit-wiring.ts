...
import { auditLog, type AuditLogger } from "./audit-log";

/** Return the singleton wrapped as an `AuditLogger` for the auth pod. */
export function getAuditLoggerForAuth(): AuditLogger {
  return {
    record: (entry) => auditLog.record(entry),
  };
}

/** Return the singleton wrapped as an `AuditLogger` for the agents pod. */
export function getAuditLoggerForAgents(): AuditLogger {
  return {
    record: (entry) => auditLog.record(entry),
  };
}
 ...
 */
export function wireAuditLoggers(
  authSetter: (logger: AuditLogger) => void,
  agentsSetter: (logger: AuditLogger) => void,
): void {
  authSetter(getAuditLoggerForAuth());
  agentsSetter(getAuditLoggerForAgents());
}

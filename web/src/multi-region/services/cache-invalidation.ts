 ...
 */
export function invalidateAllCachesForTenantChange(reason: string): void {
  ...
  try {
    ...
    console.warn(
      `[cache-invalidation] invalidateQuotaCache failed (reason: ${reason})`,
      e,
    );
  }
  ...
  try {
    ...
    console.warn(
      `[cache-invalidation] invalidateEaCapability failed (reason: ${reason})`,
      e,
    );
  }

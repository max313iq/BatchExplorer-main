...
const TONE_VAR: Record<NonNullable<MeteorsProps["tone"]>, string> = {
  primary: "var(--primary)",
  accent: "var(--accent)",
  success: "var(--success)",
  warning: "var(--warning)",
...
export const Meteors: React.FC<MeteorsProps> = ({
  ...
  return (
    <div
      ...
      style={{ ["--meteor-tone" as never]: TONE_VAR[tone] }}
    >
      {meteors.map((m) => (
        <span
          key={m.key}
          className="absolute h-0.5 w-0.5 rotate-[215deg] rounded-full motion-reduce:animate-none"
          style={{
            top: m.top,
            left: m.left,
        ...
        >
          {/* Trailing tail */}
          <span
            className="pointer-events-none absolute -z-10 h-px w-[60px] -translate-y-1/2"
            style={{
              top: "50%",
...
function ensureMeteorKeyframes(): void {
  if (typeof document === "undefined") return;
  if (_meteorKeyframesInjected) return;
